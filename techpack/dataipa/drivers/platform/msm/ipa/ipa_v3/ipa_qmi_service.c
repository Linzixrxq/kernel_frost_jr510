	...
	const void *decoded_msg)
{
	...
	IPAWANIPC("Received INDICATION Request\n");
	...
	IPAWANIPC("qmi_snd_rsp: result %d, err %d\n",
	...
	if (ipa3_qmi_modem_init_fin)	{
		IPAWANIPC("send indication to modem (%d)\n",
	...
	const void *decoded_msg)
{
	...
	IPAWANIPC("Received install filter Request\n");
	...
	if (rc < 0)
		...
		IPAWANIPC("Replied to install filter request\n");
}
	...
	const void *decoded_msg)
{
	...
	IPAWANIPC("Received filter_install_notify Request\n");
	resp.resp.result = IPA_QMI_RESULT_SUCCESS_V01;
	...
	if (rc < 0)
		...
		IPAWANIPC("Responsed filter_install_notify Request\n");
}
	...
	const void *decoded_msg)
{
	...
	resp.resp.result = IPA_QMI_RESULT_SUCCESS_V01;
	IPAWANIPC("Received IPA CONFIG Request\n");
	...
	if (rc < 0)
		...
		IPAWANIPC("Responsed QMI_IPA_CONFIG_RESP_V01\n");
}
	...
	const void *decoded_msg)
{
	...
	IPAWANIPC("Received QMI_IPA_INIT_MODEM_DRIVER_CMPLT_REQ_V01\n");
	...
	if (rc < 0)
		...
		IPAWANIPC("Sent QMI_IPA_INIT_MODEM_DRIVER_CMPLT_RESP_V01\n");
}
...
qmi_client_start:
	...
	IPAWANIPC("wait service available\n");
	return;
...
{
	IPAWANIPC("Stopping all QMI workqueues\n");
