...
static int ipa3_get_clks(struct device *dev)
{
	...
	ret = ipa3_get_busclks(dev);
	if(ret) {
		IPAERR("ipa get snoc/cnoc clk failure, ret = %d", ret);
	}
...
void _ipa_enable_clks_v3_0(void)
{
	...
	if (ipa3_clk) {
		...
		ret = clk_prepare_enable(ipa3_clk);
		ipa3_ctx->clk_enable = ret == 0 ? true : false;
		if(ret) {
			IPAERR("enabling ipa core_clk failure, ret = %d", ret);
			return;
		}
#else
...
int ipa3_extra_get_bus_vote(void)
{
    return (int)ipa3_get_bus_vote();
}
EXPORT_SYMBOL(ipa3_extra_get_bus_vote);
...
void ipa3_enable_clks(void)
{
	...
	if(ipa3_ctx->bus_enable) {
		for (i = 0; i < ipa3_ctx->icc_num_paths; i++) {
			...
			IPAERR("ipa bus enable = %s bus vote failure\n",
					true == ipa3_ctx->bus_enable ? "true" : "false");
	}
...
void _ipa_disable_clks_v3_0(void)
{
	...
	if (ipa3_clk && ipa3_ctx->clk_enable) {
		clk_disable_unprepare(ipa3_clk);
		return;
	}
	IPAERR("ipa core clk enable = %s\n",
			true == ipa3_ctx->clk_enable ? "true" : "false");
}
...
EXPORT_SYMBOL(ipa3_set_clock_plan_from_pm);
...
{
	memset(&data, 0, sizeof(struct ipa_core_data));
	...
	data.ipa_get_pdev = ipa3_get_pdev;
	data.ipa_sys_setup = ipa3_sys_setup;
	data.ipa_sys_teardown = ipa3_sys_teardown;
	data.ipa_sys_update_gsi_hdls = ipa3_sys_update_gsi_hdls;
	data.ipa_get_rt_tbl = ipa3_get_rt_tbl;
	data.ipa_get_bus_vote = ipa3_extra_get_bus_vote;
	data.ipa_set_clock_plan_from_pm = ipa3_set_clock_plan_from_pm;
		...
		struct platform_device *ipa_pdev)
{
	...
	if (result)
		goto fail_bus_reg;
#ifdef CONFIG_ARCH_JLQ
	result = ipa3_get_busclks(&ipa3_ctx->master_pdev->dev);
	if (result)
		IPAERR("ipa get snoc/cnoc clk failure, ret = %d", result);
...
#ifdef CONFIG_ARCH_JLQ
	ipa_drv_res->transport_irq = platform_get_irq_byname(pdev, "gsi-irq");
#else
