...
int cam_mem_mgr_cache_ops(struct cam_mem_cache_ops_cmd *cmd)
{
	...
	struct ion_buffer *buffer = tbl.bufq[idx].dma_buf->priv;

	if (buffer == NULL) {
		CAM_ERR(CAM_MEM, "cache get flags failed with null buffer.");
		rc = -EINVAL;
		goto end;
	}
	dmabuf_flag = buffer->flags;
#else
...
#ifdef JLQ_CAMERA_EDIT
// Use android ion to instead of msm ion.
		heap_id = ION_HEAP_SYSTEM;
#else
