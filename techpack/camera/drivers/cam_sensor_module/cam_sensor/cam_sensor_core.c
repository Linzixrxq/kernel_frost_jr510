	...
	struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	s_ctrl->moduleId_isAvailable = probe_info->moduleId_isAvailable;
	s_ctrl->moduleId_addr_type   = probe_info->moduleId_regAddrType;
	s_ctrl->moduleId_data_type   = probe_info->moduleId_regDataType;
	s_ctrl->sensordata->slave_info.eeprom_slaveAddress = (uint16_t)probe_info->eeprom_slaveAddress;
	s_ctrl->sensordata->slave_info.moduleId_RegAddr    = probe_info->moduleId_RegAddr;
	s_ctrl->sensordata->slave_info.module_id           = probe_info->module_id;

	CAM_DBG(CAM_SENSOR, "ModuleId isAvailable:%d slaveAddress:0x%x RegAddr:0x%x id:0x%x %d %d",
		probe_info->moduleId_isAvailable,
		probe_info->eeprom_slaveAddress,
		probe_info->moduleId_RegAddr,
		probe_info->module_id,
		probe_info->moduleId_regAddrType,
		probe_info->moduleId_regDataType);
#endif
...
int cam_sensor_match_id(struct cam_sensor_ctrl_t *s_ctrl)
{
    ...
    {
        uint32_t moduleid = 0;
        uint32_t module_id = 0x509;
        uint32_t module_id1 = 0x510;
        uint16_t eeprom_slave_addr = 0xa2;
        uint16_t module_id_reg_addr = 0x000D;
        s_ctrl->io_master_info.cci_client->sid = eeprom_slave_addr >> 1;
        rc = camera_io_dev_read(
                &(s_ctrl->io_master_info),
                module_id_reg_addr,&moduleid,
                CAMERA_SENSOR_I2C_TYPE_WORD,
                CAMERA_SENSOR_I2C_TYPE_WORD);
        CAM_INFO(CAM_SENSOR, "moduleid:0x%x", moduleid);
        if(moduleid == module_id)
        {
            chipid = chipid + 1;
        }
        else if (moduleid == module_id1)
        {
            chipid = chipid + 2;
        }
    ...
    {
        uint32_t moduleid = 0;
        uint32_t module_id = 0x0139;
        uint16_t module_id_reg_addr = 0x1415;
        //page OTP
        uint32_t page_addr = 0xfd;
        uint32_t page_reg = 0;
        uint32_t page_write_reg = 0x06;
        ...
        rc = camera_io_dev_read(
                ...
                CAMERA_SENSOR_I2C_TYPE_BYTE);
        CAM_INFO(CAM_SENSOR, "write_reg:0x%x", page_reg);
        ...
        write_reg.delay = 0;
        write_reg.data_mask = 0;
        write_reg.reg_addr = page_addr;
        write_reg.reg_data = page_write_reg;
        //page 6
	...
	if (TRUE == s_ctrl->moduleId_isAvailable)
	{
		uint32_t moduleid = 0;
		s_ctrl->io_master_info.cci_client->sid =
			s_ctrl->sensordata->slave_info.eeprom_slaveAddress >> 1;
		rc = camera_io_dev_read(
			&(s_ctrl->io_master_info),
			s_ctrl->sensordata->slave_info.moduleId_RegAddr,
			&moduleid,
			s_ctrl->moduleId_addr_type,
			s_ctrl->moduleId_data_type);
		if (moduleid != s_ctrl->sensordata->slave_info.module_id)
		{
			CAM_ERR(CAM_SENSOR, "moduleid:0x%x expected moduleid:0x%x", moduleid,s_ctrl->sensordata->slave_info.module_id);
			return -ENODEV;
		}
		s_ctrl->io_master_info.cci_client->sid =
