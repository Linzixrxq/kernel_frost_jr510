...
#define CAM_SENSOR_PINCTRL_STATE_FLASH_HIGH	"flash_high"
#define CAM_SENSOR_PINCTRL_STATE_FLASH_LOW	"flash_low"
#define CAM_SENSOR_PINCTRL_STATE_FLASH_OFF	"flash_off"
#endif
...
#ifdef JLQ_CAMERA_EDIT
	rc = of_property_read_u32(of_node, "gpio-flash", &val);
	if (rc != -EINVAL) {
		if (rc < 0) {
			CAM_ERR(CAM_SENSOR,
				"read gpio-flash failed rc %d", rc);
			goto free_gpio_info;
		} else if (val >= gpio_array_size) {
			CAM_ERR(CAM_SENSOR, "gpio-flash invalid %d", val);
			rc = -EINVAL;
			goto free_gpio_info;
		}
		gpio_num_info->gpio_num[SENSOR_GPIO_FLASH] =
			gconf->cam_gpio_common_tbl[val].gpio;
		gpio_num_info->valid[SENSOR_GPIO_FLASH] = 1;

		CAM_DBG(CAM_SENSOR, "gpio-flash %d",
			gpio_num_info->gpio_num[SENSOR_GPIO_FLASH]);
	}

	rc = of_property_read_u32(of_node, "gpio-torch", &val);
	if (rc != -EINVAL) {
		if (rc < 0) {
			CAM_ERR(CAM_SENSOR,
				"read gpio-torch failed rc %d", rc);
			goto free_gpio_info;
		} else if (val >= gpio_array_size) {
			CAM_ERR(CAM_SENSOR, "gpio-torch invalid %d", val);
			rc = -EINVAL;
			goto free_gpio_info;
		}
		gpio_num_info->gpio_num[SENSOR_GPIO_TORCH] =
			gconf->cam_gpio_common_tbl[val].gpio;
		gpio_num_info->valid[SENSOR_GPIO_TORCH] = 1;

		CAM_DBG(CAM_SENSOR, "gpio-torch %d",
			gpio_num_info->gpio_num[SENSOR_GPIO_TORCH]);
	}
#endif
...
#ifdef JLQ_CAMERA_EDIT
int msm_camera_flash_pinctrl_init(
	struct cam_soc_flash_pinctrl_info *flash_pctrl, struct device *dev)
{

	flash_pctrl->pinctrl = devm_pinctrl_get(dev);
	if (IS_ERR_OR_NULL(flash_pctrl->pinctrl)) {
		CAM_DBG(CAM_SENSOR, "Getting pinctrl handle failed");
		return -EINVAL;
	}
	flash_pctrl->gpio_state_flash_high =
		pinctrl_lookup_state(flash_pctrl->pinctrl,
				CAM_SENSOR_PINCTRL_STATE_FLASH_HIGH);
	if (IS_ERR_OR_NULL(flash_pctrl->gpio_state_flash_high)) {
		CAM_ERR(CAM_SENSOR,
			"Failed to get the flash high state pinctrl handle");
		return -EINVAL;
	}
	flash_pctrl->gpio_state_flash_low
		= pinctrl_lookup_state(flash_pctrl->pinctrl,
				CAM_SENSOR_PINCTRL_STATE_FLASH_LOW);
	if (IS_ERR_OR_NULL(flash_pctrl->gpio_state_flash_low)) {
		CAM_ERR(CAM_SENSOR,
			"Failed to get the flash low state pinctrl handle");
		return -EINVAL;
	}
	flash_pctrl->gpio_state_flash_off
		= pinctrl_lookup_state(flash_pctrl->pinctrl,
				CAM_SENSOR_PINCTRL_STATE_FLASH_OFF);
	if (IS_ERR_OR_NULL(flash_pctrl->gpio_state_flash_off)) {
		CAM_ERR(CAM_SENSOR,
			"Failed to get the flash low state pinctrl handle");
		return -EINVAL;
	}
