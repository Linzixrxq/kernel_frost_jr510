...
static enum
led_brightness led_brightness_get(struct led_classdev *led_cdev)
{
	return led_cdev->brightness;
}

/* For torch on:
   echo 1 > /sys/class/leds/led:torch/brightness
   For torch off:
   echo 0 > /sys/class/leds/led:torch/brightness
   For flash on:
   echo 1 > /sys/class/leds/led:flash/brightness
   For flash on:
...
{
	CAM_DBG(CAM_FLASH,"Enter, Value %d", value);
	int ret = 0;
	struct cam_flash_ctrl *flash_ctrl = NULL;

	if (!strncmp(led_cdev->name, "led:torch", strlen("led:torch"))) {
		flash_ctrl = container_of(led_cdev,
					  struct cam_flash_ctrl,
					  torch_node);
		struct cam_soc_flash_pinctrl_info pinctrl_flash_info =
			flash_ctrl->soc_info.pinctrl_flash_info;
		struct cam_hw_soc_info  *soc_info = &flash_ctrl->soc_info;
		struct msm_camera_gpio_num_info *gpio_num_info =
			flash_ctrl->power_info.gpio_num_info;
		ret = cam_sensor_util_request_gpio_table(soc_info, 1);
		if (ret < 0) {
			CAM_ERR(CAM_FLASH, "Request gpio table failed");
		}
		if (value) {
			ret = cam_flash_handle_reg_gpio(SENSOR_GPIO_FLASH,
							gpio_num_info,
							GPIOF_OUT_INIT_LOW);
			if (ret < 0)
				CAM_ERR(CAM_FLASH, "Error disabling GPIO");

			ret = cam_flash_handle_reg_gpio(SENSOR_GPIO_TORCH,
							gpio_num_info,
							GPIOF_OUT_INIT_HIGH);
			if (ret < 0)
				CAM_ERR(CAM_FLASH, "Error disabling GPIO");
...
int cam_flash_handle_reg_gpio(int seq_type,
...
#ifdef JLQ_CAMERA_EDIT
		if (CAM_FLASH_TYPE_GPIO ==
		    ((struct cam_flash_private_soc *)fctrl->soc_info.soc_private)->flash_type)
			cam_gpio_flash_off(fctrl);
		else
#endif
			cam_flash_off(fctrl);
		fctrl->flash_state = CAM_FLASH_STATE_ACQUIRE;
	...
	struct device *master_dev, void *data)
{
...
#ifdef JLQ_CAMERA_EDIT
	uint32_t flash_type = CAM_FLASH_TYPE_PMIC;
#endif
...
#ifdef JLQ_CAMERA_EDIT
	flash_type = ((struct cam_flash_private_soc *)fctrl->soc_info.soc_private)->flash_type;
	switch (flash_type) {
	case CAM_FLASH_TYPE_I2C: {
	  /* Get CCI master */
	...
	case CAM_FLASH_TYPE_GPIO: {
	  ...
	  fctrl->torch_node.name = "led:torch_0";
	  fctrl->torch_node.brightness_set = led_brightness_set;
	  fctrl->torch_node.brightness_get = led_brightness_get;
	  rc = led_classdev_register(dev, &(fctrl->torch_node));
	  if (rc < 0) {
		  CAM_ERR(CAM_FLASH, "Unable to register torch node");
	  }

	  fctrl->flash_node.name = "led:flash_0";
	  fctrl->flash_node.brightness_set = led_brightness_set;
	  fctrl->flash_node.brightness_get = led_brightness_get;
	  rc = led_classdev_register(dev, &(fctrl->flash_node));
	  if (rc < 0) {
		  CAM_ERR(CAM_FLASH, "Unable to register flash node");
	  }

	  break;
	}
	default:
	case CAM_FLASH_TYPE_PMIC: {
	  /* PMIC Flash */
