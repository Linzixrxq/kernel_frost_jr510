...
#ifdef JLQ_CAMERA_EDIT
static int cam_gpio_flash_ops(struct cam_flash_ctrl *flash_ctrl,
			      enum camera_flash_opcode op)
{
	int ret = 0;
	struct cam_flash_private_soc *soc_private = NULL;
	...
	ret = msm_camera_flash_pinctrl_init(&pinctrl_flash_info,
					    &flash_ctrl->pdev->dev);
	if (ret < 0) {
		/* gpio flash no pintrl*/
	...
	if (op == CAMERA_SENSOR_FLASH_OP_FIRELOW) {
		ret = pinctrl_select_state(
			pinctrl_flash_info.pinctrl,
			pinctrl_flash_info.gpio_state_flash_low);
		if (ret)
			CAM_ERR(CAM_FLASH, "Can't set pin to flash low state");
		...
		ret = pinctrl_select_state(
			pinctrl_flash_info.pinctrl,
			pinctrl_flash_info.gpio_state_flash_high);
		if (ret)
			CAM_ERR(CAM_FLASH, "Can't set pin to flash high state");
	...
	} else if (op == CAMERA_SENSOR_FLASH_OP_OFF) {
		ret = pinctrl_select_state(
			pinctrl_flash_info.pinctrl,
			pinctrl_flash_info.gpio_state_flash_off);
		if (ret)
			CAM_ERR(CAM_FLASH, "Can't set pin to flash off state");
		...
		CAM_ERR(CAM_FLASH, "Invalid Operation: %d", op);
		return -EINVAL;
	}
...
int cam_gpio_flash_off(struct cam_flash_ctrl *flash_ctrl)
{
	if (!flash_ctrl) {
		CAM_ERR(CAM_FLASH, "Flash control Null");
		return -EINVAL;
	...
	cam_gpio_flash_ops(flash_ctrl, CAMERA_SENSOR_FLASH_OP_OFF);

	return 0;
}

int cam_gpio_flash_high(struct cam_flash_ctrl *flash_ctrl)
{
	if (!flash_ctrl) {
		CAM_ERR(CAM_FLASH, "Flash control Null");
		return -EINVAL;
	...
	cam_gpio_flash_ops(flash_ctrl, CAMERA_SENSOR_FLASH_OP_FIREHIGH);

	return 0;
}

int cam_gpio_flash_low(struct cam_flash_ctrl *flash_ctrl)
{
	if (!flash_ctrl) {
		CAM_ERR(CAM_FLASH, "Flash control Null");
		return -EINVAL;
	...
	cam_gpio_flash_ops(flash_ctrl, CAMERA_SENSOR_FLASH_OP_FIRELOW);
...
int cam_flash_pmic_pkt_parser(struct cam_flash_ctrl *fctrl, void *arg)
{
	...
	if (((csl_packet->header.op_code  & 0xFFFFF) ==
		...
		if (fctrl->bridge_intf.crm_cb &&
			...
			rc = fctrl->bridge_intf.crm_cb->add_req(&add_req);
			if  (rc) {
				CAM_ERR(CAM_FLASH, "Failed in adding request: %llu to request manager",
				csl_packet->header.request_id);
			}
			CAM_DBG(CAM_FLASH, "add req %lld to req_mgr, trigger_eof %d",
				add_req.req_id, add_req.trigger_eof);
		}
	}
