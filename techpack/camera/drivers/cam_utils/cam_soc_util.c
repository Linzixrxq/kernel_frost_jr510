	...
	uint32_t rgltr_delay_ms)
{
	...
	CAM_DBG(CAM_UTIL, "rgltr_name: %s voltage min=%d, max=%d",
			rgltr_name,
	...
	irq_handler_t handler, void *irq_data)
{
	...
	if (soc_info->irq_line) {
		// Because of qtang rise edge interrupt is converted to high level trigger,
		// need to change IRQF_TRIGGER_RISING to IRQF_TRIGGER_HIGH when devm_request_irq.
