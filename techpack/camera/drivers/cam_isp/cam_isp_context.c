	...
	uint32_t status, uint32_t event_cause)
{
	...
	CAM_DBG(CAM_ISP, "ctx[%d] : Req %llu : Handled %d deferred buf_dones num_acked=%d, num_acked=%d "
	    " num_fence_map_out=%d", ctx->ctx_id, req->request_id, req_isp->num_deferred_acks,
...
//			struct cam_isp_ctx_req *req_isp;
			...
			/*
			 * Verify consumed address for this request to make sure
			 * we are handing the buf_done for the correct
			 * buffer. Also defer actual buf_done handing, i.e
			 * do not signal the fence as this request way go into
			 * Bubble state eventully.
			 */
...
#ifdef JLQ_CAMERA_EDIT
	__cam_isp_ctx_handle_deferred_buf_done_in_bubble(ctx_isp, req);
#endif //JLQ_CAMERA_EDIT
	/*
...
#ifdef JLQ_CAMERA_EDIT
static int __cam_isp_ctx_buf_done_in_sof(struct cam_isp_context * ctx_isp, void *evt_data)
{
	int rc = 0;
	struct cam_isp_hw_done_event_data *done =
