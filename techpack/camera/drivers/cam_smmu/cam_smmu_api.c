...
// commented by jlq, the interface iommu_get_fault_ids is not implemented by jlq smmu driver.
...
static int cam_smmu_iommu_fault_handler_for_smmuv3(struct iommu_fault *fault,
	void *token)
{
	int idx;
	int    flags;
	...
	if (!fault || !token) {
		CAM_ERR(CAM_SMMU, "Error: NULL pointer!\n");
		return -EINVAL;
	}
	...
	CAM_ERR(CAM_SMMU, "cb_name: %s (%s) iommu fault type: %d", cb_name, cb->name[0], fault->type);

	if (IOMMU_FAULT_PAGE_REQ == fault->type) {
		flags = fault->prm.flags;
		iova  = fault->prm.addr;
		CAM_ERR(CAM_SMMU, "iommu prm: flags: %x, pasid: 0x%x "
				"grpid: 0x%x perm: 0x%x addr: 0x%x",
				fault->prm.flags,
				fault->prm.pasid,
				fault->prm.grpid,
				fault->prm.perm,
	...
	} else if (IOMMU_FAULT_DMA_UNRECOV == fault->type) {
		flags = fault->event.flags;
		iova  = fault->event.addr;
		CAM_ERR(CAM_SMMU, "iommu event: reason: 0x%x flags: 0x%x, pasid: 0x%x "
				"perm: 0x%x addr: 0x%x fetch_addr:%x",
				fault->event.reason,
				fault->event.flags,
				fault->event.pasid,
				fault->event.perm,
	...
	payload->domain = iommu_get_domain_for_dev(cb->dev);
	payload->dev = cb->dev;
	payload->iova = iova;
	payload->flags = flags;
	payload->token = token;
	...
	bool dis_delayed_unmap, struct cam_dma_buff_info **mapping_info)
{
	...
	if (region_id == CAM_SMMU_REGION_SHARED) {
		...
		// There is no DMA_ATTR_DELAYED_UNMAP in jlq kernel
...
#ifdef JLQ_CAMERA_EDIT
//	This is needed by arm smmuv3.
	if (cb->io_support)
		iommu_register_device_fault_handler(cb->dev,
				cam_smmu_iommu_fault_handler_for_smmuv3,
				(void *)cb->name[0]);
#else //JLQ_CAMERA_EDIT
	if (cb->io_support && cb->domain)
