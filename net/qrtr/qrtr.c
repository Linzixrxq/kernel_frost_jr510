...
static const char *get_qrtr_service_name(unsigned int service)
{
	int i = 0;
	const char *name = NULL;
	if(service < 35 && service >= 0){
		name = common_names[service].name;
	} else {
		for (i = 35; i < sizeof(common_names)/sizeof(common_names[0]); ++i) {
...
static void qrtr_log_resume_pkt(struct qrtr_cb *cb, u64 pl_buf)
{
	...
	if (glink_resume_pkt) {
		...
		pr_info("[QRTR RESUME PKT]:src[0x%x:0x%x] dst[0x%x:0x%x] [%08x %08x]: service[0x%x] name[%s]\n",
			cb->src_node, cb->src_port,
			cb->dst_node, cb->dst_port,
			(unsigned int)pl_buf, (unsigned int)(pl_buf >> 32),
			service_id,
			get_qrtr_service_name(service_id));
	}
}
#endif
	...
	if (cb->type == QRTR_TYPE_DATA) {
		...
		if (cb->type == QRTR_TYPE_NEW_SERVER ||
			...
			if (cb->src_node == 0 || cb->src_node == 3) {
				...
				//        cb->type, cb->src_node);
				QRTR_INFO(node->ilc,"qrtr: Modem QMI Readiness RX cmd:0x%x node[0x%x]\n",
				       cb->type, cb->src_node);
			}
		}
	}
