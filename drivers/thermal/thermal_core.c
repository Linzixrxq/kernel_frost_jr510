...
#include <drm/drm_panelthermal.h>
...
static bool thermal_screen_flag=false;
...
static ssize_t
thermal_screen_state_store(struct device *dev,
				      struct device_attribute *attr, const char *buf, size_t len)
{
#if IS_ENABLED(CONFIG_JGKI)
        unsigned int screen;
	unsigned int screen_bringhtness;
         
        pr_err("GC thermal jlq_thermal_brightness_set buf:%s",buf);
        sscanf(buf, "%d",&screen_bringhtness);
        pr_err("GC thermal thermal_screen_state_store:%s: screen_bringhtness:%d", __func__,screen_bringhtness);
        if((screen_bringhtness>0)&&(sm.screen_state == 1))
        {
          pr_err("GC thermal thermal_screen_state_store success");
          jlq_thermal_brightness_set((int)screen_bringhtness);          
        }
        else
        {
          pr_err("GC thermal thermal_screen_state_store fail！  screen_bringhtness:%d sm.screen_state:%d ",screen_bringhtness,sm.screen_state);
        }
#endif   
	return len;
}
...
static DEVICE_ATTR(screen_state, 0664,
		thermal_screen_state_show, thermal_screen_state_store);
...
{
#if IS_ENABLED(CONFIG_JGKI)
     if((jlq_panel_status)&&(thermal_screen_flag==false))
        {      	
        	if (jlq_panel_notifier_register(&sm.thermal_notifier) < 0) {
               		pr_err("GC:Thermal: jlq_panel_notifier_register failed\n");
       		}
                else
                {
                  	thermal_screen_flag=true;
                        pr_err("GC:Thermal: jlq_panel_notifier_register success:%d\n",thermal_screen_flag);
                }
...
static int screen_state_for_thermal_callback(struct notifier_block *nb, unsigned long val, void *data)
{
	...
	unsigned int blank;
        pr_err("GC screen_state_for_thermal_callback:%s: VAL:%d", __func__,val);
	if (val > 2 || !evdata || !evdata->data)
		return 0;

	blank = *(int *)(evdata->data);
        pr_err("GC thermal blank = *(int *)(evdata->data):%s: VAL:%d", __func__,blank);
	switch (blank) {
	case DRM_BLANK_LP1:
		pr_err("thermal%s: DRM_BLANK_LP1\n", __func__);
	case DRM_BLANK_LP2:
		pr_err("thermal%s: DRM_BLANK_LP2\n", __func__);
	case DRM_BLANK_POWERDOWN:
		sm.screen_state = 0;
		pr_err("thermal%s: DRM_BLANK_POWERDOWN\n", __func__);
		break;
	case DRM_BLANK_UNBLANK:
		sm.screen_state = 1;
		pr_err("thermal%s: DRM_BLANK_UNBLANK\n", __func__);
		break;
	default:
		break;
	}
        pr_err("GC thermal sysfs_notify:%s:", __func__);
...
#ifdef CONFIG_ARCH_JLQ
#if IS_ENABLED(CONFIG_JGKI)
  jlq_panel_notifier_unregister(&sm.thermal_notifier);
#endif   
