...
#define TAG_NAME "IPA_FMWK"
#define IPA_FMWKDBG(fmt, args...) \
		do { \
			pr_info(TAG_NAME " %s:%d " fmt, __func__, __LINE__, ## args);\
		} while (0)
...
#define IPA_FMWK_DISPATCH_RETURN_PTR(api, p...) \
	do { \
			if (!ipa_fmwk_ctx) { \
				pr_err("%s:%d IPA framework is not supported\n", \
					__func__, __LINE__); \
			...
			if (ipa_fmwk_ctx->api) { \
				...
				pr_debug("exit %s\n", __func__);\
					...
					WARN(1, "%s was not registered on ipa_fmwk\n",\
							__func__); \
					ret = NULL; \
			} \
		} \
...
struct ipa_fmwk_contex {
	...
	int (*ipa_get_bus_vote)(void);
	int (*ipa_set_clock_plan_from_pm)(int idx);
...
int ipa_fmwk_register_ipa(const struct ipa_core_data *in)
{
	...
	ipa_fmwk_ctx->ipa_get_pdev = in->ipa_get_pdev;
	ipa_fmwk_ctx->ipa_sys_setup = in->ipa_sys_setup;
	ipa_fmwk_ctx->ipa_sys_teardown = in->ipa_sys_teardown;
	ipa_fmwk_ctx->ipa_sys_update_gsi_hdls = in->ipa_sys_update_gsi_hdls;
	ipa_fmwk_ctx->ipa_get_rt_tbl = in->ipa_get_rt_tbl;
	ipa_fmwk_ctx->ipa_get_bus_vote = in->ipa_get_bus_vote;
	ipa_fmwk_ctx->ipa_set_clock_plan_from_pm = in->ipa_set_clock_plan_from_pm;
	...
	void *user_data)
{
	...
	if (!cb_info) {
	    pr_err("ipa_ready_cb_info kmalloc failure");
		mutex_unlock(&ipa_fmwk_ctx->lock);
		return -ENOMEM;
	}
...

{
	...
	IPA_FMWK_DISPATCH_RETURN_PTR(ipa_get_pdev);
...

{
	...
	IPA_FMWK_DISPATCH_RETURN(ipa_sys_setup, sys_in, ipa_bam_or_gsi_hdl,
...
int ipa_sys_teardown(u32 clnt_hdl)
{
	...
	IPA_FMWK_DISPATCH_RETURN(ipa_sys_teardown, clnt_hdl);
	...
	unsigned long gsi_ev_hdl)
{
	...
	IPA_FMWK_DISPATCH_RETURN(ipa_sys_update_gsi_hdls, clnt_hdl,
...
int ipa_get_rt_tbl(struct ipa_ioc_get_rt_tbl *lookup)
{
	...
	IPA_FMWK_DISPATCH_RETURN(ipa_get_rt_tbl, lookup);
...
int ipa_get_bus_vote(void)
{
	...
	IPA_FMWK_DISPATCH_RETURN(ipa_get_bus_vote);

	return ret;
}
EXPORT_SYMBOL(ipa_get_bus_vote);

int ipa_set_clock_plan_from_pm(int idx)
{
	int ret;

	IPA_FMWK_DISPATCH_RETURN(ipa_set_clock_plan_from_pm, idx);

	return ret;
}
EXPORT_SYMBOL(ipa_set_clock_plan_from_pm);
