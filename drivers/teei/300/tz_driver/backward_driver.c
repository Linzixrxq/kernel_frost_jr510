...
#ifdef UT_DMA_ZONE
	handler->param_buf = (void *)__get_free_pages(GFP_KERNEL | GFP_DMA | __GFP_COMP,
				get_order(ROUND_UP(handler->size, SZ_4K)));
#else
	handler->param_buf = (void *)__get_free_pages(GFP_KERNEL | __GFP_COMP,
				get_order(ROUND_UP(handler->size, SZ_4K)));
#endif
...
static int reetime_handle(struct NQ_entry *entry)
{
	...
	if (time_type == GET_UPTIME) {
		...
		boottime = sched_clock();
		tv_usec = boottime / 1000;
		tv_sec = tv_usec / 1000000;
		tv_usec = tv_usec - tv_sec * 1000000;
#else
