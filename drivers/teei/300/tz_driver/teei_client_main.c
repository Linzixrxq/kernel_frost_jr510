...
struct platform_device *utos_pdev;
/**
 * @brief TEEI Agent Driver initialization
...
static int teei_client_init(void)
{
	...
	utos_pdev = platform_device_alloc("utos", 0);
	if (utos_pdev == NULL) {
		IMSG_ERROR("Failed to initial UTOS device!\n");
		return -EINVAL;
	}

	ret_code = platform_device_add(utos_pdev);
	if (ret_code != 0) {
		IMSG_ERROR("Failed to add device!\n");
		return ret_code;
	}

#ifdef TEEI_UNITTEST_SUPPORT
	IMSG_PRINTK("----------- TEEI register ut_tester_driver START ---------------\n");
	ret_code = platform_driver_register(&ut_tester_driver);
	if (ret_code) {
		IMSG_ERROR("unable to register ut_test_driver(%d)\n", ret_code);
		return ret_code;
	}
	IMSG_PRINTK("----------- TEEI register ut_tester_driver END ---------------\n");
#endif
