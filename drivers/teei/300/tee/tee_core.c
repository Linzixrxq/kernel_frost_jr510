...
struct tee_context *teei_client_open_context(struct tee_context *start,
...
EXPORT_SYMBOL_GPL(teei_client_open_context);

void teei_client_close_context(struct tee_context *ctx)
{
	teedev_close_context(ctx);
}
EXPORT_SYMBOL_GPL(teei_client_close_context);

void teei_client_get_version(struct tee_context *ctx,
			struct tee_ioctl_version_data *vers)
{
	ctx->teedev->desc->ops->get_version(ctx->teedev, vers);
}
EXPORT_SYMBOL_GPL(teei_client_get_version);


int teei_client_open_session(struct tee_context *ctx,
			struct tee_ioctl_open_session_arg *arg,
...
EXPORT_SYMBOL_GPL(teei_client_open_session);

int teei_client_close_session(struct tee_context *ctx, u32 session)
{
	if (!ctx->teedev->desc->ops->close_session)
		return -EINVAL;
	return ctx->teedev->desc->ops->close_session(ctx, session);
}
EXPORT_SYMBOL_GPL(teei_client_close_session);

int teei_client_invoke_func(struct tee_context *ctx,
			struct tee_ioctl_invoke_arg *arg,
			struct tee_param *param)
{
	if (!ctx->teedev->desc->ops->invoke_func)
...
EXPORT_SYMBOL_GPL(teei_client_invoke_func);
