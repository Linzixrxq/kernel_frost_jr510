...
//#define SHAREMEM_IN_DTS
#define JLQ_SHARE_MEM_ADD               (0x94e00000)
#define JLQ_SHARE_MEM_SIZE              (0x400000)
...
soter_config_shm_memremap(void **memremaped_shm)
{
	...
	if (reserved_mem == NULL) {
		IMSG_ERROR("cannot alloc reserved_mem struct\n");
		return ERR_PTR(-ENOMEM);
	}

	reserved_mem->base = JLQ_SHARE_MEM_ADD;
	reserved_mem->size = JLQ_SHARE_MEM_SIZE;
#endif
