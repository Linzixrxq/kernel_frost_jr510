				...
				struct dwc2_hsotg_req *hs_req)
{
	...
	usb_gadget_unmap_request_by_dev(hsotg->gadget.dev.parent, req, hs_ep->map_dir);
#else
...
#ifdef CONFIG_JLQ_USB_DWC2
	ret = usb_gadget_map_request_by_dev(hsotg->gadget.dev.parent, req, hs_ep->dir_in);
#else
			     ...
			     int dir_in)
{
	...
	if (idx == 0 && !hs_ep->dir_in &&
	    (ints & (DXEPINT_SETUP | DXEPINT_SETUP_RCVD)))
...
#ifdef CONFIG_JLQ_USB_DWC2
	hsotg->gadget.otg_caps = devm_kzalloc(hsotg->dev,
			sizeof(struct usb_otg_caps), GFP_KERNEL);
	if (!hsotg->gadget.otg_caps)
		return -ENOMEM;

	hsotg->gadget.otg_caps->otg_rev = 0x0200;
	if (hsotg->params.otg_cap == DWC2_CAP_PARAM_NO_HNP_SRP_CAPABLE) {
		hsotg->gadget.otg_caps->hnp_support = 0;
		hsotg->gadget.otg_caps->srp_support = 0;
	} else if (hsotg->params.otg_cap == DWC2_CAP_PARAM_SRP_ONLY_CAPABLE) {
		hsotg->gadget.otg_caps->hnp_support = 0;
		hsotg->gadget.otg_caps->srp_support = 1;
	} else {
		hsotg->gadget.otg_caps->hnp_support = 1;
		hsotg->gadget.otg_caps->srp_support = 1;
	}
#endif
