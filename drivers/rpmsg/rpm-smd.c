...
static int msm_rpm_flush_requests(bool print)
{
	...
	for (t = rb_first(&tr_root); t; t = rb_next(t)) {
		...
		memcpy(name, &type, sizeof(uint32_t));
		...
		if ((msm_rpm_debug_mask & MSM_RPM_LOG_REQUEST_PRETTY)
			...
			pr_warn_ratelimited("trysend s=slp, rsc_type=0x%08X(%s), rsc_id=%u\n", type, name, id);
		}
...
static void msm_rpm_log_request(struct msm_rpm_request *cdata)
{
	...
	pos += scnprintf(buf + pos, buflen - pos, "\n");
	pr_info_ratelimited("request info %s\n", buf);
}
...
static int rpm_smd_suspend(void)
{
	struct cpumask cpumask;
	cpumask_setall(&cpumask);
	msm_rpm_enter_sleep(0, &cpumask);
	return 0;
}

static void rpm_smd_resume(void)
{
	msm_rpm_exit_sleep();

}

static struct syscore_ops rpm_smd_pm_ops = {
	.suspend  = rpm_smd_suspend,
	.resume  = rpm_smd_resume,
};
...
int __init msm_rpm_driver_init(void)
{
	...
	if (ret)
		...
		register_syscore_ops(&rpm_smd_pm_ops);
