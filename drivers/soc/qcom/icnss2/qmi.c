...
/* When testing modem SSR, there is possible qmi send fail, but the crash
 * notification not yet comes to cnss driver, so it is not correct if trigger
 * assert directly, which will cause SSR failure. If issue happens for wlan fw
 * crash, it can be fixed by enable early crash indication, which can avoid such
 * issue, but for modem watchdog, there will be no chance to send out early
 * crash indication from modem side.
 * Here the timer will be started if qmi send fail, and it will be canelled if
 * modem/fw crash comes.
 */
struct timer_list icnss_qmi_assert_timer;
#define ICNSS_QMI_ASSERT_WAIT_TIME  1500

static inline
void icnss_qmi_assert_timer_handler(struct timer_list *icnss_qmi_assert_timer)
{
	ICNSS_ASSERT(ignore_fw_timeout);
}

void icnss_qmi_assert_timer_init(void)
{
	icnss_pr_dbg("init qmi assert timer");

	timer_setup(&icnss_qmi_assert_timer, icnss_qmi_assert_timer_handler, 0);
	icnss_ignore_fw_timeout(false);
}

void icnss_qmi_assert_timer_deinit(void)
{
	icnss_pr_dbg("deinit qmi assert timer");

	icnss_ignore_fw_timeout(true);
	del_timer_sync(&icnss_qmi_assert_timer);
}

void icnss_qmi_assert_timer_start(void)
{
	if (ignore_fw_timeout)
		return;

	icnss_pr_vdbg("start qmi assert timer(%d ms)",
		     ICNSS_QMI_ASSERT_WAIT_TIME);
	del_timer_sync(&icnss_qmi_assert_timer);

	icnss_qmi_assert_timer.expires = jiffies +
		msecs_to_jiffies(ICNSS_QMI_ASSERT_WAIT_TIME);
	add_timer(&icnss_qmi_assert_timer);
}

void icnss_qmi_assert_timer_stop(void)
{
	icnss_pr_vdbg("stop qmi assert timer");

	del_timer(&icnss_qmi_assert_timer);
}

void icnss_ignore_fw_timeout(bool ignore)
{
	icnss_pr_vdbg("set ignore_fw_timeout = %d", ignore);

	if (ignore)
		icnss_qmi_assert_timer_stop();
