		   ...
		   void *ramdump_dev, void *minidump_dev)
{
	...
	pr_debug("pil_do_ramdump enter:[%p][%s]\n",desc->minidump_ss,desc->name);
	...
	if (desc->minidump_ss) {
		...
		pr_debug("Continuing with mini SSR dump [%d][%d][%d]\n",desc->minidump_ss->md_ss_smem_regions_baseptr,desc->minidump_ss->md_ss_toc_init,desc->minidump_ss->md_ss_enable_status);
		/**
	...
	list_for_each_entry(seg, &priv->segs, list)
		count++;
	pr_debug("pil_do_ramdump --count:[%d]\n", count);

	ramdump_segs = kcalloc(count+1, sizeof(*ramdump_segs), GFP_KERNEL);//the segment mem region for dump 0x86900000-0x88000000 need to allocate too
	if (!ramdump_segs)
		return -ENOMEM;
	...
	/*need to dump whole qtang privte region, need dump region 0x86900000-0x88000000,make this region to a segment*/
	/*priv->region_end: 86900000,priv->modem_pa_end: 88000000, s->size: 1700000*/
	s->address = priv->region_end;
	s->size =  priv->modem_pa_end - s->address;
	if(s->size > 0)
		count++;
				  ...
				  const struct elf32_phdr *phdr, int num)
{
	...
	if (!seg){
		pil_err(desc, "pil_init_seg-kmalloc fail\n");
...
static int pil_init_entry_addr(struct pil_priv *priv, const struct pil_mdt *mdt)
{
	...
	pr_debug("entry address %pa,image_relocated:[%d]\n", &entry,image_relocated);
				...
				phys_addr_t max_addr, size_t align)
{
	...
	if (priv->is_region_allocated) {
		pr_debug("pil_alloc_region  region_end:%pa,  region_start:%pa, Size: 0x%x\n",&(priv->region_end),&(priv->region_start),size);
...
static int pil_setup_region(struct pil_priv *priv, const struct pil_mdt *mdt)
{
	...
	pr_debug("pil_setup_region enter,e_phnum[%d][%s]\n",mdt->hdr.e_phnum,priv->desc->name);
	...
	pr_debug("pil_setup_region,[%pa][%pa][%d]\n",&(max_addr_n),&(max_addr_r),relocatable);

	if (relocatable) {
		ret = pil_alloc_region(priv, min_addr_r, max_addr_r, align);
		pr_debug("pil_setup_region->pil_alloc_region exit,[%pa][%pa][%zu]\n",&(priv->region_start),&(priv->region_end),priv->region_size);
	} else {
...
static int pil_init_mmap(struct pil_desc *desc, const struct pil_mdt *mdt)
{
	...
	if (ret){
		pil_err(desc, "pil_boot-pil_init_mmap ret:[%d]\n",ret);
		return ret;
	}

	pil_info(desc, "pil_init_mmap->loading from %pa to %pa\n", &priv->region_start,
							&priv->region_end);
	...
	for (i = 0; i < mdt->hdr.e_phnum; i++) {
		...
		if (IS_ERR(seg)){
			pil_err(desc, "pil_boot-pil_init_seg fail ret:[%d]\n",ret);
			return PTR_ERR(seg);
...
static int pil_parse_devicetree(struct pil_desc *desc)
{
	...
	pr_debug("pil_parse_devicetree  desc->name:[%s]\n",
					desc->name);
	...
	if (0 == strcmp(desc->name, "modem")){
		...
		struct device_node *np3 = NULL;
		u64 prop_size1 = 0;
		u64 prop_size2 = 0;
		u64 prop_size3 = 0;
		const __be32 *addrp1 = NULL;
		const __be32 *addrp2 = NULL;
		const __be32 *addrp3 = NULL;

		np1 = of_parse_phandle(ofnode,"qcom,msadp-region", 0);
		if (np1) {
			addrp1 = of_get_address(np1, 0, &prop_size1, NULL);
			if (!addrp1) {
				pr_err("Failed to get assigned-addresses or property\n");
				ret = -EINVAL;
				goto out;
			}

			priv->msadp_pa = of_translate_address(np1, addrp1);
			if (priv->msadp_pa == OF_BAD_ADDR) {
				pr_err("Failed to translate MSA PA from device-tree\n");
				ret = -EINVAL;
				goto out;
			}

			priv->msadp_va = memremap(priv->msadp_pa,
						(unsigned long)prop_size1, MEMREMAP_WT);
			if (!priv->msadp_va) {
				pr_err("MSA PA ioremap failed: phy addr: %pa\n",
						 &priv->msadp_pa);
				ret = -EINVAL;
				goto out;
			}
			...
			priv->msadp_mem_size = prop_size1;
		}

		pr_debug("msadp_region_dt_parse pa: %pa, MSA Memory Size: 0x%x\n",
				 &priv->msadp_pa, priv->msadp_mem_size);

		np2 = of_parse_phandle(ofnode,"qcom,qbl-region", 0);
		if (np2) {
			addrp2 = of_get_address(np2, 0, &prop_size2, NULL);
			if (!addrp2) {
				pr_err("Failed to get assigned-addresses or property\n");
				ret = -EINVAL;
				goto out;
			...
			priv->qbl_pa = of_translate_address(np2, addrp2);
			if (priv->qbl_pa == OF_BAD_ADDR) {
				pr_err("Failed to translate MSA PA from device-tree\n");
				ret = -EINVAL;
				goto out;
			}

			priv->qbl_va = memremap(priv->qbl_pa,
						(unsigned long)prop_size2, MEMREMAP_WT);
			if (!priv->qbl_va) {
				pr_err("MSA PA ioremap failed: phy addr: %pa\n",
						 &priv->qbl_pa);
				ret = -EINVAL;
			...
			priv->qbl_mem_size = prop_size2;
		}
		priv->qbl_pa_end = priv->qbl_pa + priv->qbl_mem_size;
		priv->modem_mdt_pa = priv->qbl_pa_end - SZ_64K;
		priv->modem_mdt_va = memremap(priv->modem_mdt_pa,
						SZ_64K, MEMREMAP_WT);
		if (!priv->modem_mdt_va) {
				pr_err("modem_mdt_va ioremap failed: phy addr: %pa\n",
						 &priv->modem_mdt_pa);
				ret = -EINVAL;
				goto out;
		}

		pr_debug("qbl_region_dt_parse pa: %pa,pa_end: %pa, modem_mdt_pa: %pa, qbl Memory Size: 0x%x\n",
				 &priv->qbl_pa, &priv->qbl_pa_end, &(priv->modem_mdt_pa), priv->qbl_mem_size);

		np3 = of_parse_phandle(ofnode,"memory-region", 0);
		if (np3) {
			addrp3 = of_get_address(np3, 0, &prop_size3, NULL);
			if (!addrp3) {
				pr_err("Failed to get assigned-addresses or property\n");
				ret = -EINVAL;
				goto out;
			...
			priv->modem_pa = of_translate_address(np3, addrp3);
			if (priv->modem_pa == OF_BAD_ADDR) {
				pr_err("Failed to translate MSA PA from device-tree\n");
				ret = -EINVAL;
				goto out;
			}
			priv->modem_va = memremap(priv->modem_pa,
						(unsigned long)prop_size3, MEMREMAP_WT);
			if (!priv->modem_va) {
				pr_err("modem PA ioremap failed: phy addr: %pa\n",
						 &priv->modem_pa);
				ret = -EINVAL;
				goto out;
			}

			priv->modem_mem_size = prop_size3;
			priv->modem_pa_end = priv->modem_pa + priv->modem_mem_size;
		}

		pr_debug("modem_region_dt_parse  pa: %pa, modem Memory Size: 0x%x\n",
				 &priv->modem_pa, priv->modem_mem_size);
...
int pil_boot(struct pil_desc *desc)
{
	...
	struct pil_tz_data *drv = container_of(desc, struct pil_tz_data, desc);
	...
	if (!drv->self_auth && drv->tz_verify) {
		snprintf(fw_name, sizeof(fw_name), "%s.sig", desc->fw_name);
		ret = request_firmware(&fw_sign, fw_name, desc->dev);
		if (ret) {
			pil_info(desc, "Failed to locate %s(rc:%d)\n", fw_name, ret);
		}
	}
	...
	if (ret){
		pil_err(desc, "pil_boot-pil_init_mmap fail\n");
		goto release_fw;
	}
	...
	if (pil_wq && !(desc->sequential_loading)) {
		...
		if (ret){
			pil_err(desc, "pil_load_segs fail\n");
			goto err_deinit_image;
		}
	} else {
		list_for_each_entry(seg, &desc->priv->segs, list) {
			...
			if (ret){
				pil_err(desc, "pil_load_seg fail\n");
				goto err_deinit_image;
			}
		}
	}
	...
	if (fw_sign) {
		size_t size = 0;
		trace_pil_event("before_prepare_header", desc);
		if (desc->tz_vi.session_id < 0) {
			desc->tz_vi.session_id = tz_verifier_init();
			if (desc->tz_vi.session_id < 0) {
				pil_err(desc, "%s verify get session fail\n", desc->fw_name);
				ret = -EBUSY;
				goto err_release_verifier;
			}
		}
		if (desc->tz_vi.shm_id >= 0) {
			tz_verifier_get_shm_info(desc->tz_vi.shm_id, NULL, &size);
		}

		desc->tz_vi.argsize = TZ_SHM_SIZE(ehdr->e_phnum);

		if (size < TZ_SHM_SIZE(ehdr->e_phnum)) {
			tz_verifier_free_shm(desc->tz_vi.shm_id);
			desc->tz_vi.shm_id = -1;
		}
		if (desc->tz_vi.shm_id < 0) {
			desc->tz_vi.shm_id = tz_verifier_alloc_shm(desc->tz_vi.argsize);
			if (desc->tz_vi.shm_id < 0) {
				pil_err(desc, "%s verify alloc shm fail:%d\n", desc->fw_name, desc->tz_vi.shm_id);
				ret = -ENOMEM;
				goto err_release_verifier;
			}
		}
		desc->tz_vi.image_info = kzalloc(desc->tz_vi.argsize, GFP_KERNEL);
		if (!desc->tz_vi.image_info) {
			pil_err(desc, "%s verify alloc mem %d fail\n", desc->fw_name, desc->tz_vi.argsize);
			ret = -ENOMEM;
			goto err_release_verifier;
		}
		...
		if (!sig_header) {
			pil_err(desc, "%s verify alloc metadata mem %d fail\n", desc->fw_name, fw_sign->size);
			ret = -ENOMEM;
			goto err_release_verifier;
		}
		memcpy(sig_header, fw_sign->data, fw_sign->size);

		desc->tz_vi.image_info->ss_prop.ssid = SS_MODEM;
		desc->tz_vi.image_info->ss_prop.para_num = 4;
		desc->tz_vi.image_info->ss_prop.paras[0].offset = RMB_PMI_META_DATA;
		desc->tz_vi.image_info->ss_prop.paras[0].value = priv->modem_mdt_pa;
		desc->tz_vi.image_info->ss_prop.paras[1].offset = RMB_PMI_CODE_START;
		desc->tz_vi.image_info->ss_prop.paras[1].value = priv->modem_pa;
		desc->tz_vi.image_info->ss_prop.paras[2].offset = RMB_PMI_CODE_LENGTH;
		desc->tz_vi.image_info->ss_prop.paras[2].value = priv->region_size;
		desc->tz_vi.image_info->ss_prop.paras[3].offset = RMB_MBA_COMMAND;
		desc->tz_vi.image_info->ss_prop.paras[3].value = 0x03;
		desc->tz_vi.image_info->image_seg_num = ehdr->e_phnum;
		desc->tz_vi.image_info->header_paddr = virt_to_phys(sig_header);
		desc->tz_vi.image_info->header_size = fw_sign ? fw_sign->size : 0;
		list_for_each_entry(seg, &desc->priv->segs, list) {
			desc->tz_vi.image_info->ism[seg->num].paddr = seg->paddr;
			desc->tz_vi.image_info->ism[seg->num].sz = seg->filesz;
		}
	}
	...
	pil_info(desc, "[%s] Brought out of reset\n",desc->name);
	desc->modem_ssr = false;

err_release_verifier:
	pil_err(desc, "out! %s session:%d shmid:%d image_info:%x\n", desc->name, desc->tz_vi.session_id, desc->tz_vi.shm_id, desc->tz_vi.image_info);
	kfree(desc->tz_vi.image_info);
	desc->tz_vi.image_info = NULL;
	kfree(sig_header);

	/*
	tz_verifier_free_shm(desc->tz_vi.shm_id);
	desc->tz_vi.shm_id = -1;
	tz_verifier_deinit(desc->tz_vi.session_id, 1);
	desc->tz_vi.session_id = -1;
	*/
...
int pil_desc_init(struct pil_desc *desc)
{
	...
	if (of_property_read_u32(ofnode, "qcom,minidump-id",
		...
		if (g_md_toc && g_md_toc->md_toc_init == true) {
			...
			if (collect_aux_minidump_ids(desc) < 0)
				pil_err(desc, "Failed to get aux %s minidump ids\n",
				       desc->name);
		}
	}
	...
	if (ret){
		pil_err(desc, "pil_parse_devicetree fail\n");
		goto err_parse_dt;
	}
	...
	if (!priv->ws) {
		pil_err(desc, "wakeup_source_register fail\n");
		return -ENOMEM;
		goto err;
	}
					...
					resource_size_t *out_size)
{
	...
	pr_debug("pil: pil_get_resource:[%lx]",(unsigned long)res.start);
