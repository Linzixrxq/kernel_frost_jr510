...
static int g_headphone_type = HEADPHONE;

static struct msm_fm_close_info fm_info;

struct msm_fm_close_info *get_fm_info()
{
    return &fm_info;
}
EXPORT_SYMBOL_GPL(get_fm_info);


bool fm_playbck_is_opened()
{
    return fm_info.opened > 0?true:false;
}
	...
	switch(port_id){
		case MSM_FRONTEND_DAI_MULTIMEDIA1:
			if(session_type == SESSION_TYPE_RX)
			    fdai_id = JLQ_FRONTEND_DAI_RX_DEEPBUFFER;
			else if(session_type == SESSION_TYPE_TX)
			    fdai_id = JLQ_FRONTEND_DAI_TX_AUDIO_RECORD;
			break;
		case MSM_FRONTEND_DAI_MULTIMEDIA9:
			if(session_type == SESSION_TYPE_TX)
				fdai_id = JLQ_FRONTEND_DAI_TX_VOICE_RECORD;
			else if(session_type == SESSION_TYPE_RX)
				fdai_id = JLQ_FRONTEND_DAI_RX_INCALL_MUSIC;
			break;
		case MSM_FRONTEND_DAI_MULTIMEDIA10:
			if(session_type == SESSION_TYPE_TX)
				fdai_id = JLQ_FRONTEND_DAI_TX_VOIP_UPLINK;
			else
				fdai_id = JLQ_FRONTEND_DAI_RX_VOIP_DOWNLINK;
			break;
		case MSM_FRONTEND_DAI_MULTIMEDIA3:
			if(session_type == SESSION_TYPE_TX)
			    fdai_id = JLQ_FRONTEND_DAI_TX_AUDIO_RECORD_EXT2;
			break;
		case MSM_FRONTEND_DAI_MULTIMEDIA5:
			if(session_type == SESSION_TYPE_RX)
			    fdai_id = JLQ_FRONTEND_DAI_RX_LOWLATENCY;
			else if(session_type == SESSION_TYPE_TX)
			    fdai_id = JLQ_FRONTEND_DAI_TX_LOWLATENCY_RECORD;
			break;
		case MSM_FRONTEND_DAI_VOICEMMODE1:
		case MSM_FRONTEND_DAI_VOICEMMODE2:
		case MSM_FRONTEND_DAI_VOIP:
			if(session_type == SESSION_TYPE_RX)
			    fdai_id = JLQ_FRONTEND_DAI_RX_VOICE_DOWNLINK;
			else if(session_type == SESSION_TYPE_TX)
			    fdai_id = JLQ_FRONTEND_DAI_TX_VOICE_UPLINK;
			break;
		case MSM_FRONTEND_DAI_LSM1:
		case MSM_FRONTEND_DAI_LSM2:
			fdai_id = JLQ_FRONTEND_DAI_TX_VT;
			break;
		default:
		...
		pr_info("%s: port_id:%d session_type:%d fdai_id:%d\n", __func__, port_id,session_type,fdai_id);
		return fdai_id;
};
EXPORT_SYMBOL_GPL(jlqaudio_get_dsp_fe_id);
...
uint8_t jlqaudio_get_dsp_be_id(u16 port_id)
{
	...
	switch (port_id) {
		#if 0
		case SLIMBUS_0_RX: return JLQ_BACKEND_DAI_RX_EARPIECE;
		case SLIMBUS_1_RX: return JLQ_BACKEND_DAI_RX_HEADSET;
		case SLIMBUS_2_RX: return JLQ_BACKEND_DAI_RX_WSA_SPEAKER;
		case SLIMBUS_0_TX: return JLQ_BACKEND_DAI_TX_BUILTIN_MIC;
		case SLIMBUS_1_TX: return JLQ_BACKEND_DAI_TX_HEADSET_MIC;
		case SLIMBUS_2_TX: return JLQ_BACKEND_DAI_TX_VA_MIC;
		#endif

		case AFE_PORT_ID_WSA_CODEC_DMA_RX_0:
		case MSM_BACKEND_DAI_RX_CDC_DMA_RX_1:
		case AFE_PORT_ID_RX_CODEC_DMA_RX_1: return JLQ_BACKEND_DAI_RX_SPEAKER;
		case MSM_BACKEND_DAI_RX_CDC_DMA_RX_0:
		case AFE_PORT_ID_RX_CODEC_DMA_RX_0: {
			if(g_headphone_type ==  HEADPHONE)
				return JLQ_BACKEND_DAI_RX_HEADPHONE;
			else if(g_headphone_type ==  HEADSET)
				return JLQ_BACKEND_DAI_RX_HEADSET;
		}
		case AFE_PORT_ID_RX_CODEC_DMA_RX_2: return JLQ_BACKEND_DAI_RX_HANDSET;
		case AFE_PORT_ID_TX_CODEC_DMA_TX_3: return JLQ_BACKEND_DAI_TX_PRIMARY_MIC;
		case AFE_PORT_ID_TX_CODEC_DMA_TX_4: return JLQ_BACKEND_DAI_TX_HEADSET_MIC;
		case AFE_PORT_ID_VA_CODEC_DMA_TX_0: return JLQ_BACKEND_DAI_TX_VA_MIC;

		case MSM_BACKEND_DAI_SLIMBUS_8_TX:
		case SLIMBUS_8_TX:
		{
				return JLQ_BACKEND_DAI_TX_FM;
		}
		case SLIMBUS_7_RX: return JLQ_BACKEND_DAI_RX_SCO_DOWNLINK;
		case SLIMBUS_7_TX: return JLQ_BACKEND_DAI_TX_SCO_UPLINK;
		case VOICE_RECORD_RX:
		case VOICE_RECORD_TX: return JLQ_BACKEND_DAI_TX_VOICE_RECORD;
		case VOICE_PLAYBACK_TX: return JLQ_BACKEND_DAI_RX_INCALL_MUSIC;
...
EXPORT_SYMBOL_GPL(jlqaudio_get_dsp_be_id);

int jlqaudio_set_headphone_type(int type)
{
    if(type != HEADPHONE && type != HEADSET ){
        pr_err("%s: invalid headphone type %d\n", __func__,type);
		return -1;
    }
    pr_info("%s: headphone_type %d\n", __func__,type);
    g_headphone_type = type;
    return 0;
}
EXPORT_SYMBOL_GPL(jlqaudio_set_headphone_type);
