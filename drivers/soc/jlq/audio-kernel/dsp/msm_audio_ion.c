				 ...
				 size_t *plen, void **vaddr)
{
	...
	pr_debug("%s: dma_buf = 0x%x, paddr = 0x%x, plen = %d, vaddr = 0x%x, \n",
				__func__, dma_buf, paddr, (int)*plen, vaddr);
...
u32 query_audio_heap_id(struct ion_heap_data *hdata,int count)
{
	int i=0;
	char AUDIO_HEAP_NAME[] = "adsp_mem_2_region";

	for(i=0; i<count; i++){
		if(strstr(hdata[i].name,AUDIO_HEAP_NAME))
			return hdata[i].heap_id;
	}
	return 0;
}
			...
			dma_addr_t *paddr, size_t *plen, void **vaddr)
{
	...
	heaps_cnt = ion_query_heaps_kernel(hdata,32);
	audio_heap_id = query_audio_heap_id(hdata,heaps_cnt);
	if(!audio_heap_id){
		pr_err("%s: can not find audio heap\n", __func__);
		return -EINVAL;
	}
	pr_debug("%s:find audio heap[%d] \n", __func__,audio_heap_id);

	*dma_buf = ion_alloc(bufsz, ION_HEAP(audio_heap_id), 0);
	...
	pr_debug("%s: mapped address = 0x%px, size=%zd\n", __func__,
		*vaddr, bufsz);
...
static int msm_audio_ion_probe(struct platform_device *pdev)
{
	...
	u64 smmu_sid = 0;
	u64 smmu_sid_mask = 0;
	const char *msm_audio_ion_dt = "jlq,smmu-enabled";
	const char *msm_audio_ion_smmu = "jlq,smmu-version";
	const char *msm_audio_ion_smmu_sid_mask = "jlq,smmu-sid-mask";
	bool smmu_enabled;
	...
	if (!smmu_enabled) {
		pr_err("%s: SMMU is Disabled\n", __func__);
		goto exit;
	}

	q6_state = apr_get_q6_state();
	if (q6_state == APR_SUBSYS_DOWN) {
		pr_err("defering %s, adsp_state %d\n",
			__func__, q6_state);
		return -EPROBE_DEFER;
	}
	pr_err("%s: adsp is ready\n", __func__);

	rc = of_property_read_u32(dev->of_node,
				msm_audio_ion_smmu,
				&msm_audio_ion_data.smmu_version);
	if (rc) {
		pr_err("%s: jlq,smmu_version missing in DT node\n",
			__func__);
		return rc;
	}
	pr_err("%s: SMMU is Enabled. SMMU version is (%d)",
	...
	if (rc) {
		dev_info(dev,
			"%s: jlq,smmu-sid-mask missing in DT node, using default\n",
			__func__);
...
MODULE_SOFTDEP("pre: ion_cma_heap");
