...
inline static int voice_set_packet_exchange_mode_and_config(uint32_t session_id,
						     uint32_t mode);
...
inline static int voice_alloc_and_map_oob_mem(struct voice_data *v);
...
inline static int voice_send_dual_control_cmd(struct voice_data *v)
{
	int ret = 0;
...
inline static int voice_create_mvm_cvs_session(struct voice_data *v)
{
	int ret = 0;
...
static int voice_send_open_voice_cmd(struct voice_data *v)
{
	struct apr_hdr mvm_open_voice_cmd;
	int ret = 0;
	void *apr_mvm;
	u16 mvm_handle;
	...
	mvm_open_voice_cmd.hdr_field = APR_HDR_FIELD(APR_MSG_TYPE_SEQ_CMD,
						APR_HDR_LEN(APR_HDR_SIZE),
						APR_PKT_VER);
	mvm_open_voice_cmd.pkt_size = APR_PKT_SIZE(APR_HDR_SIZE,
				sizeof(mvm_open_voice_cmd) - APR_HDR_SIZE);
	pr_info("send mvm_open_voice_cmd pkt size = %d\n",
				mvm_open_voice_cmd.pkt_size);
	mvm_open_voice_cmd.src_port =
				voice_get_idx_for_session(v->session_id);
	mvm_open_voice_cmd.dest_port = mvm_handle;
	mvm_open_voice_cmd.src_svc = APR_SVC_ADSP_MVM;
	mvm_open_voice_cmd.src_domain = APR_DOMAIN_APPS;
	mvm_open_voice_cmd.dest_svc = APR_SVC_ADSP_MVM;
	mvm_open_voice_cmd.dest_domain = APR_DOMAIN_ADSP;
	mvm_open_voice_cmd.token = 0;
	mvm_open_voice_cmd.opcode = VSS_IMVM_CMD_OPEN_VOICE;

	v->mvm_state = CMD_STATUS_FAIL;
	v->async_err = 0;
	ret = apr_send_pkt(apr_mvm, (uint32_t *) &mvm_open_voice_cmd);
	if (ret < 0) {
		pr_err("Fail in sending VSS_IMVM_CMD_OPEN_VOICE\n");
		goto fail;
	}
	ret = wait_event_timeout(v->mvm_wait,
				 (v->mvm_state == CMD_STATUS_SUCCESS),
...
static int voice_send_start_voice_cmd(struct voice_data *v)
{
	...
	mvm_start_voice_cmd.src_svc = APR_SVC_ADSP_MVM;
	mvm_start_voice_cmd.src_domain = APR_DOMAIN_APPS;
	mvm_start_voice_cmd.dest_svc = APR_SVC_ADSP_MVM;
	mvm_start_voice_cmd.dest_domain = APR_DOMAIN_ADSP;
...
static int voice_send_stop_voice_cmd(struct voice_data *v)
{
	...
	mvm_stop_voice_cmd.src_svc = APR_SVC_ADSP_MVM;
	mvm_stop_voice_cmd.src_domain = APR_DOMAIN_APPS;
	mvm_stop_voice_cmd.dest_svc = APR_SVC_ADSP_MVM;
	mvm_stop_voice_cmd.dest_domain = APR_DOMAIN_ADSP;
...
inline static int voice_setup_vocproc(struct voice_data *v)
{
...
static int voice_send_netid_timing_cmd(struct voice_data *v)
{
	...
	mvm_set_voice_timing.hdr.opcode = VSS_ICOMMON_CMD_SET_VOICE_TIMING_V2;
...
inline static void voc_update_session_params(struct voice_data *v)
{
	/* reset LCH mode */
	v->lch_mode = 0;
				     ...
				     uint16_t mute_flag, uint32_t ramp_duration)
{
	...
	pr_info("%s: direction:%d  mute_flag:%d\n", __func__,direction,mute_flag);
	...
	cvs_mute_cmd.hdr.src_svc = APR_SVC_ADSP_CVS;
...
static int voice_send_vol_step_cmd(struct voice_data *v)
{
	...
	cvp_vol_step_cmd.hdr.src_svc = APR_SVC_ADSP_CVP;
...
int voc_end_voice_call(uint32_t session_id)
{
	...
	if (v->voc_state == VOC_RUN || v->voc_state == VOC_ERROR ||
		...
		pr_info("%s: End voice called in state %d\n",
			__func__, v->voc_state);
	}
 ...
 *       command to open voice call task
 *
 * @session_id: voice session ID to send this command
 *
 * Returns 0 on success or error on failure
 */
int voc_open_voice_call(uint32_t session_id)
{
	struct voice_data *v = voice_get_session(session_id);
	int ret = 0;
	...
	if ((v->voc_state == VOC_INIT) ||
		...
		ret = voice_send_open_voice_cmd(v);
		if (ret < 0) {
...
#endif
		v->voc_state = VOC_OPEN;

	} else {
		pr_info("%s: Error: open voice called in state %d\n",
			__func__, v->voc_state);
...
EXPORT_SYMBOL(voc_open_voice_call);
