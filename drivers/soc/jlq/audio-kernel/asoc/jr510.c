...
#endif
#include <soc/audio-codec-port-types.h>
#include "jr510-port-config.h"
#include <soc/jlq-info.h>
#include "msm_dailink.h"


#define DRV_NAME "jr510-asoc-snd"
#define __CHIPSET__ "JR510 "
...
//TODO:Audio kernel AOSP change - start -
/*delete static,export this interface to machine driver*/
struct snd_soc_component *soc_find_component(
	const struct snd_soc_dai_link_component *dlc);
//TODO:Audio kernel AOSP change - end -
...
static struct dev_config cdc_dma_tx_cfg[] = {
	...
	[VA_CDC_DMA_TX_0] = {SAMPLING_RATE_16KHZ, SNDRV_PCM_FORMAT_S16_LE, 1},
	[VA_CDC_DMA_TX_1] = {SAMPLING_RATE_16KHZ, SNDRV_PCM_FORMAT_S16_LE, 1},
	[VA_CDC_DMA_TX_2] = {SAMPLING_RATE_16KHZ, SNDRV_PCM_FORMAT_S16_LE, 1},
};
...
static struct snd_soc_card snd_soc_card_jr510_msm;
...
static int jr510_send_island_va_config(int32_t be_id)
{
	int rc = 0;
	int port_id = 0xFFFF;
				...
				struct snd_pcm_hw_params *params)
{
	...
	pr_debug("%s: format = %d, rate = %d channel = %d\n",
		  __func__, params_format(params), params_rate(params),params_channels(params));
...
static int jr510_tdm_snd_hw_params(struct snd_pcm_substream *substream,
				    struct snd_pcm_hw_params *params)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_dai *cpu_dai = rtd->cpu_dai;
...
static int jr510_tdm_snd_startup(struct snd_pcm_substream *substream)
{
	int ret = 0;
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_dai *cpu_dai = rtd->cpu_dai;
...
static void jr510_tdm_snd_shutdown(struct snd_pcm_substream *substream)
{
	int ret = 0;
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_dai *cpu_dai = rtd->cpu_dai;
...
static int jr510_aux_snd_startup(struct snd_pcm_substream *substream)
{
	int ret = 0;
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_dai *cpu_dai = rtd->cpu_dai;
...
static void jr510_aux_snd_shutdown(struct snd_pcm_substream *substream)
{
	int ret = 0;
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_dai *cpu_dai = rtd->cpu_dai;
...
static int msm_snd_cdc_dma_startup(struct snd_pcm_substream *substream)
{
	...
	case MSM_BACKEND_DAI_VA_CDC_DMA_TX_2:
		...
		ret = jr510_send_island_va_config(dai_link->id);
		if (ret)
			     ...
			     struct snd_pcm_hw_params *params)
{
	...
	pr_info("%s, cpu_dai->name:%s, cpu_dai->id:%d, codec_dai->name:%s, codec_dai->id:%d \n",
	        __func__, cpu_dai->name, cpu_dai->id, rtd->codec_dai->name, rtd->codec_dai->id);

	dev_dbg(rtd->dev, "%s: %s_tx_dai_id_%d\n", __func__,
		    cpu_dai->name, cpu_dai->id);

    /*slave's codec set_channel in the msm_wcn_init, here for cpu dai.*/
	switch(cpu_dai->id){
		case SLIMBUS_7_TX:
			rx_ch[0] = 159;
			rx_ch_cnt = 1;
		...
		case SLIMBUS_8_TX:
			...
			break;
		default:
			dev_err(rtd->dev, "%s: invalid cpu_dai:%d\n",
			__func__, cpu_dai->id);
			goto err;
		break;
	}
...
static struct snd_soc_ops jr510_aux_be_ops = {
	.startup = jr510_aux_snd_startup,
	.shutdown = jr510_aux_snd_shutdown
};

static struct snd_soc_ops jr510_tdm_be_ops = {
	.hw_params = jr510_tdm_snd_hw_params,
	.startup = jr510_tdm_snd_startup,
	.shutdown = jr510_tdm_snd_shutdown
};
...
static void jlq_add_awpa_snd_controls(struct snd_soc_component *component)
{
	int ret;
	pr_info("%s: jlq_int_aw87xxx_snd_controls\n",__func__);
	ret = aw87xxx_add_codec_controls((void *)component);
	if (ret < 0) {
		pr_err("%s: add awinic pa snd controls failed: %d\n",
			__func__, ret);
	}
}
#endif
...
static void *def_wcd_mbhc_cal(void)
{
	...
	btn_high[0] = 75;
	btn_high[1] = 225;//2019.12.13 longcheer puqirui edit ,default is 150
	btn_high[2] = 450;//2019.12.13 longcheer puqirui edit ,default is 237
	btn_high[3] = 500;
	btn_high[4] = 500;
	btn_high[5] = 500;
	btn_high[6] = 500;
...
static struct snd_soc_dai_link msm_common_dai_links[] = {
	...
	{/* hw:x,5  audio-record-voip & audio-playback-voip */
...
static struct snd_soc_dai_link msm_jr510_dai_links[
...
static const struct of_device_id jr510_asoc_machine_of_match[]  = {
	{ .compatible = "jlq,jr510-asoc-snd",
	  .data = "codec"},
	{ .compatible = "jlq,jr510-asoc-snd-stub",
	  .data = "stub_codec"},
	{},
};
...
static struct snd_soc_card *populate_snd_card_dailinks(struct device *dev)
{
	...
	match = of_match_node(jr510_asoc_machine_of_match, dev->of_node);
	if (!match) {
	...
	if (!strcmp(match->data, "codec")) {
		card = &snd_soc_card_jr510_msm;

		memcpy(msm_jr510_dai_links + total_links,
		...
		rc = of_property_read_u32(dev->of_node, "jlq,va-bolero-codec",
					  &va_bolero_codec);
		if (rc) {
		...
		rc = of_property_read_u32(dev->of_node, "jlq,mi2s-audio-intf",
					  &mi2s_audio_intf);
		if (rc) {
		...
		rc = of_property_read_u32(dev->of_node, "jlq,afe-rxtx-lb",
				&val);
		if (!rc && val) {
			memcpy(msm_jr510_dai_links + total_links,
		...
		rc = of_property_read_u32(dev->of_node, "jlq,tdm-audio-intf",
				&val);
		if (!rc && val) {
			memcpy(msm_jr510_dai_links + total_links,
		...
		rc = of_property_read_u32(dev->of_node, "jlq,wcn-btfm",
					  &wcn_btfm_intf);
		//wcn_btfm_intf = 1;
		if (rc) {
...
#if IS_ENABLED(CONFIG_SND_SOC_AW87XXX)
	jlq_add_awpa_snd_controls(component);
#endif
				...
				struct snd_soc_card *card)
{
	...
	for (i = 0; i < wsa_dev_cnt; i++) {
		...
		dlc->name = NULL;
		dlc->dai_name = "wsa-virtual-dai";

		if (snd_soc_find_dai(dlc)) {
			pr_info("%s found wsa-virtual-dai",__func__);
...
codec_aux_dev:
	...
	for (i = 0; i < codec_aux_dev_cnt; i++) {
		...
		dlc->name = NULL;
		dlc->dai_name = "wcd937x-virtual-dai";

		if (snd_soc_find_dai(dlc)) {
			pr_info("%s found codec wcd937x-virtual-dai",__func__);
...
aux_dev_register:
...
#if IS_ENABLED(CONFIG_SND_SOC_SIA81XX)
	ret = soc_sia81xx_init(pdev,
			&msm_aux_dev[wsa_max_devs + codec_aux_dev_cnt], sia81xx_num,
	...
	if(ret != 0){
		...
		dev_err(&pdev->dev,
				"%s: failed to init sia81xx, ret = %d\n",
				__func__, ret);
		ret = 0;
	}
#endif
...
static void msm_i2s_auxpcm_init(struct platform_device *pdev)
{
	...
	if (ret) {
		dev_dbg(&pdev->dev, "%s: no jlq,msm-mi2s-master in DT node\n",
			__func__);
	} else {
		for (count = 0; count < MI2S_MAX; count++) {
...
static int jr510_ssr_enable(struct device *dev, void *data)
{
	...
	if (!strcmp(card->name, "jr510-stub-snd-card")) {
		/* TODO */
		dev_dbg(dev, "%s: TODO\n", __func__);
	}
...
static void jr510_ssr_disable(struct device *dev, void *data)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct snd_soc_card *card = platform_get_drvdata(pdev);
...
static const struct snd_event_ops jr510_ssr_ops = {
	.enable = jr510_ssr_enable,
	.disable = jr510_ssr_disable,
};
...
static int msm_audio_ssr_register(struct device *dev)
{
	...
	for (i = 0; ; i++) {
		node = of_parse_phandle(np, "jlq,msm_audio_ssr_devs", i);
		if (!node)
			break;
	...
	ret = snd_event_master_register(dev, &jr510_ssr_ops,
					ssr_clients, NULL);
	if (!ret)
...
static int msm_asoc_machine_probe(struct platform_device *pdev)
{
	...
	ret = snd_soc_of_parse_card_name(card, "jlq,model");
	if (ret) {
	...
	ret = snd_soc_of_parse_audio_routing(card, "jlq,audio-routing");
	if (ret) {
	...
	if (!pdata->hph_en0_gpio_p) {
		dev_dbg(&pdev->dev, "%s: property %s not detected in node %s\n",
			__func__, "jlq,hph-en0-gpio",
			pdev->dev.of_node->full_name);
	}

	ret = of_property_read_string(pdev->dev.of_node,
		"jlq,mbhc-audio-jack-type", &mbhc_audio_jack_type);
	if (ret) {
		dev_dbg(&pdev->dev, "%s: Looking up %s property in node %s failed\n",
			__func__, "jlq,mbhc-audio-jack-type",
			pdev->dev.of_node->full_name);
	...
	if (!pdata->us_euro_gpio_p) {
		dev_dbg(&pdev->dev, "property %s not detected in node %s",
			"jlq,us-euro-gpios", pdev->dev.of_node->full_name);
	} else {
	...
	//add hph-select
	ret = of_property_read_u32(card->dev->of_node, "jlq,hph-select-level", &hph_select_level);
	if(ret){
		dev_dbg(&pdev->dev, "%s: missing jlq,hph-select-level in dt node\n",__func__);
		ret = 0;
	}

	pdata->hph_select_gpio = of_get_named_gpio(pdev->dev.of_node,
					"jlq,hph-select-gpio", 0);
	if (gpio_is_valid(pdata->hph_select_gpio)) {
		dev_info(&pdev->dev, "%s: hph_select_gpio %d detected in node\n",
		 __func__, pdata->hph_select_gpio);
		ret = gpio_request(pdata->hph_select_gpio, "hph-select");
		if (!ret) {
			gpio_direction_output(pdata->hph_select_gpio, 0);

			value = gpio_get_value(pdata->hph_select_gpio);
			dev_info(&pdev->dev, "%s: gpio_get_value hph_select_gpio,value %d\n",__func__,value);

			gpio_set_value(pdata->hph_select_gpio, 0);
			usleep_range(20,30);

			gpio_set_value(pdata->hph_select_gpio, hph_select_level);
			usleep_range(20,30);

			hph_select_level = gpio_get_value(pdata->hph_select_gpio);
			dev_dbg(&pdev->dev, "%s: hph_select_gpio,value %d\n",__func__,hph_select_level);
		}
		else {
			dev_err(&pdev->dev, "%s: Failed to request hph_select_gpio %d, ret %d\n",
				__func__, pdata->hph_select_gpio,ret);
			ret = 0;
		}
	}
...
ret:
	...
	dev_err(&pdev->dev, "%s: machine probe success.\n",
			 __func__);

	return 0;
err:
	if (gpio_is_valid(pdata->hph_select_gpio))
		gpio_free(pdata->hph_select_gpio);
...
static struct platform_driver jr510_asoc_machine_driver = {
	.driver = {
		.name = DRV_NAME,
		.owner = THIS_MODULE,
		.pm = &snd_soc_pm_ops,
		.of_match_table = jr510_asoc_machine_of_match,
		.suppress_bind_attrs = true,
	},
	.probe = msm_asoc_machine_probe,
	.remove = msm_asoc_machine_remove,
};
//module_platform_driver(jr510_asoc_machine_driver);
static int __init jr510_asoc_machine_init(void)
{
    int r = platform_driver_register(&jr510_asoc_machine_driver);
...
static void __exit jr510_asoc_machine_exit(void)
{
   platform_driver_unregister(&jr510_asoc_machine_driver);
}

late_initcall(jr510_asoc_machine_init);
module_exit(jr510_asoc_machine_exit);


MODULE_DESCRIPTION("ALSA SoC msm");
MODULE_LICENSE("GPL v2");
MODULE_ALIAS("platform:" DRV_NAME);
MODULE_DEVICE_TABLE(of, jr510_asoc_machine_of_match);
MODULE_SOFTDEP("pre: bt_fm_slim stub_dlkm ");
MODULE_SOFTDEP("pre: tinylog_build pinctrl_lpi_dlkm swr_ctrl_dlkm ");
MODULE_SOFTDEP("pre: tx_macro_dlkm va_macro_dlkm wcd937x_slave_dlkm ");
