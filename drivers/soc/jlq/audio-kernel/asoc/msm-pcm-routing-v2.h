...
struct msm_fm_close_info {
	unsigned int fm_tx_beid;
	unsigned int fm_rx_beid;
	unsigned int tx_active;
	unsigned int rx_active;
	bool opened;
};

int jlq_adsp_caldata_update(void);

int msm_pcm_routing_reg_phy_stream(int fedai_id, int stream_id, int session_id,
				   int stream_type);

void msm_pcm_routing_dereg_phy_stream(int fedai_id, int stream_type);

void msm_pcm_routing_reg_psthr_stream(int fedai_id, int session_id,
		int stream_type);
__inline static int msm_pcm_routing_reg_phy_compr_stream(int fedai_id, int perf_mode,
					  int dspst_id, int stream_type,
					  uint32_t compr_passthr){return 0;};

int msm_pcm_routing_reg_phy_stream_v2(int fedai_id, int stream_id,
				      int session_id, int stream_type,
				      struct msm_pcm_routing_evt event_info);

bool msm_pcm_routing_route_is_set(u16 be_id, u16 fe_id);
