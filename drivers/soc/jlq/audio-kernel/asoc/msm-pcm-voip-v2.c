...
static int msm_pcm_close(struct snd_pcm_substream *substream)
{
	...
	pr_info("%s: state(%d) prepare(%d)(%d)\n", __func__,
		prtd->state, prtd->playback_prepare, prtd->capture_prepare);
...
static int msm_pcm_prepare(struct snd_pcm_substream *substream)
{
	...
	struct voip_drv_info *prtd = runtime->private_data;
	struct snd_soc_pcm_runtime *soc_prtd = substream->private_data;
	uint32_t session_id = voc_get_session_id(VOIP_SESSION_NAME);
	...
	if (prtd->playback_prepare && prtd->capture_prepare
		...
		/* open voip session */
		ret = voc_open_voice_call(voc_get_session_id(VOIP_SESSION_NAME));
		if (ret < 0) {
			pr_err("%s(): fail at voc_open_voice_call, ret=%d\n",
				__func__, ret);
			goto done;
		}
		...
		pr_info("%s reg phy session: link id(%d) session id(0x%x).\n",
			__func__, soc_prtd->dai_link->id, session_id);
		ret = msm_pcm_routing_reg_phy_stream(soc_prtd->dai_link->id,
				0, session_id, SNDRV_PCM_STREAM_PLAYBACK);
		ret = msm_pcm_routing_reg_phy_stream(soc_prtd->dai_link->id,
				0, session_id, SNDRV_PCM_STREAM_CAPTURE);
		if (ret) {
			pr_err("%s: connect Rx/Tx routing ret:%d\n", __func__, ret);
		}
