...
static void msm_qti_pp_send_eq_values_(int eq_idx)
{
	...
	ac = q6asm_get_audio_client(fe_dai.session_id);

	if (ac == NULL) {
		pr_err("%s: Could not get audio client for session: %d\n",
		      __func__, fe_dai.session_id);
		goto done;
	}
...
EXPORT_SYMBOL_GPL(msm_qti_pp_send_eq_values);
...
EXPORT_SYMBOL_GPL(msm_qti_pp_send_stereo_to_custom_stereo_cmd);
			    ...
			    struct snd_ctl_elem_value *ucontrol)
{
	...
	ret = adm_set_fm_volume(ucontrol->value.integer.value[0]);

	msm_afe_slimbus_8_lb_vol_ctrl = ucontrol->value.integer.value[0];
...
EXPORT_SYMBOL_GPL(msm_adsp_init_mixer_ctl_pp_event_queue);
...
EXPORT_SYMBOL_GPL(msm_adsp_clean_mixer_ctl_pp_event_queue);
...
EXPORT_SYMBOL_GPL(msm_adsp_inform_mixer_ctl);
...
EXPORT_SYMBOL_GPL(msm_adsp_stream_cmd_info);
...
EXPORT_SYMBOL_GPL(msm_adsp_stream_callback_get);
...
EXPORT_SYMBOL_GPL(msm_adsp_stream_callback_info);
...
EXPORT_SYMBOL_GPL(msm_qti_pp_add_controls);
