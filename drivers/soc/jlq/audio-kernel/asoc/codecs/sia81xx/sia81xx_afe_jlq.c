...
#define DEBUG
#define LOG_FLAG	"sia81xx_afe_jlq"
...
//sia parameters cmd
#define ASM_SMART_PA_VDD_PARA_CMD_SET 0x00008800

/**
 * enable/disable factory alg for record and playback
 */
#define FACTORY_ALG_BYPASS_SET  (0x00009900)

/* payload of FACTORY_ALG_BYPASS_SET */
typedef struct
{
    /**
     * support value:
     * 0 : disable the factory alg
     * 1 : enable the factory alg
     */
    uint32_t enable_disable;
}factory_alg_bypass_set_t;

/**
 * get the status of factory alg
 */
#define FACTORY_ALG_BYPASS_STATUS_GET          (0x00009901)

/**
 * return the status of factory alg
 */
#define FACTORY_ALG_BYPASS_STATUS_GET_EVENT    (0x00009902)

/**
 * payload of FACTORY_ALG_BYPASS_STATUS_GET_EVENT
 */
typedef struct
{
    /**
     ...
     */
    uint32_t status;
}factory_alg_bypass_status_get_event_t;
...
struct dev_vdd_data {
	...
	uint32_t vdd_param[9]; //must be as sizeof(SIA81XX_VDD_PARAM);
} __packed;


struct asm_vdd_param_cmd_set {
	struct apr_hdr hdr;
	struct dev_vdd_data cmd;
} __packed;

struct asm_smart_pa_alg_en_cmd_set {
	struct apr_hdr hdr;
	factory_alg_bypass_set_t alg_bypass_set;
} __packed;

struct asm_smart_pa_alg_en_cmd_get {
	struct apr_hdr hdr;
} __packed;

static factory_alg_bypass_status_get_event_t alg_bypass_status;
	...
	void *priv)
{
	...
	pr_debug("[ debug][%s] %s: opcode:0x%x, payload size:%d\n",
			LOG_FLAG, __func__, data->opcode, data->payload_size);
	...
	switch(data->opcode) {
		...
		case FACTORY_ALG_BYPASS_STATUS_GET_EVENT:
		{
			factory_alg_bypass_status_get_event_t *payload = NULL;
			if(!data->payload_size) {
				pr_err("[  err][%s] %s: invalid payload_size = %d "
					"with opcode = 0x%08x \r\n",
					LOG_FLAG, __func__, data->payload_size, data->opcode);
			...
			payload = (factory_alg_bypass_status_get_event_t *)data->payload;
			alg_bypass_status.status = payload->status;
		...
		case APR_BASIC_RSP_RESULT: 
		{
			...
			pr_debug("[ debug][%s] %s: payload[0]:0x%x, payload[1]:%d\n",
				LOG_FLAG,__func__, payload[0], payload[1]);
			switch (payload[0]) {
				case ASM_SMART_PA_VDD_PARA_CMD_SET:
				case FACTORY_ALG_BYPASS_SET:
				{
					atomic_set(&afe->state, 0);
					wake_up(&afe->wait);
					break;
	...
	SIA81XX_AFE *afe)
{
	...
	if (NULL == afe->apr) {
		afe->apr = apr_register("ADSP", "PSY", sia81xx_afe_callback,
			0xFFFFFFFF, afe);
		if (NULL == afe->apr) {
	...
	SIA81XX_AFE *afe)
{
	...
	if (NULL == afe->cal_block.ion_handle) { 
		...
		if (ret < 0) {
			...
			pr_debug("[ debug][%s] %s: afe[0x%04x] allocate buffer success! "
				"pa_size = %zu \r\n", 
	...
	if (0 == afe->cal_block.dsp_mmap_handle) {
		...
		if (ret != 0) {
			...
			pr_debug("[ debug][%s] %s: afe[0x%04x] map buffer success! "
				"handle = 0x%08x \r\n", 
				LOG_FLAG, __func__, afe->afe_port_id, 
...
#else
	struct asm_vdd_param_cmd_set vdd_param_set;
	struct asm_smart_pa_alg_en_cmd_set pa_alg_en_cmd_set;
#endif
...
#else /* CAL_BLOCK_MAP_DISABLE */
	//send sia parameter to adsp for JR510

	if(param_id == DEFAULT_PARAM_ID) {	//set vdd
		vdd_param_set.hdr.hdr_field = APR_HDR_FIELD(APR_MSG_TYPE_SEQ_CMD,
					APR_HDR_LEN(APR_HDR_SIZE),
					APR_PKT_VER);
		vdd_param_set.hdr.pkt_size = sizeof(vdd_param_set);
		vdd_param_set.hdr.src_port = 0;
		vdd_param_set.hdr.dest_port = 0;
		vdd_param_set.hdr.src_svc = 0;
		vdd_param_set.hdr.src_domain = APR_DOMAIN_APPS;
		vdd_param_set.hdr.dest_svc = APR_SVC_PSY;
		vdd_param_set.hdr.dest_domain = APR_DOMAIN_ADSP;
		vdd_param_set.hdr.token = 0;
		vdd_param_set.hdr.opcode = ASM_SMART_PA_VDD_PARA_CMD_SET;

		vdd_param_set.cmd.opt = OPT_SET_CAL_VAL;
		vdd_param_set.cmd.param_id = param_id;
		vdd_param_set.cmd.payload_size = param_size;
		memcpy((void *)&vdd_param_set.cmd.vdd_param[0],(void *)buf,param_size);

		/* send the cmd to dsp through apr, and wait dsp returned msg */
		ret = sia81xx_apr_send(afe, &vdd_param_set);
		if(0 > ret) {
			goto err;
		}

	}else if(param_id == SIXTH_SIA81XX_RX_ENABLE) {
		pa_alg_en_cmd_set.hdr.hdr_field = APR_HDR_FIELD(APR_MSG_TYPE_SEQ_CMD,
						APR_HDR_LEN(APR_HDR_SIZE),
						APR_PKT_VER);
		pa_alg_en_cmd_set.hdr.pkt_size = sizeof(pa_alg_en_cmd_set);
		pa_alg_en_cmd_set.hdr.src_port = 0;
		pa_alg_en_cmd_set.hdr.dest_port = 0;
		pa_alg_en_cmd_set.hdr.src_svc = 0;
		pa_alg_en_cmd_set.hdr.src_domain = APR_DOMAIN_APPS;
		pa_alg_en_cmd_set.hdr.dest_svc = APR_SVC_PSY;
		pa_alg_en_cmd_set.hdr.dest_domain = APR_DOMAIN_ADSP;
		pa_alg_en_cmd_set.hdr.token = 0;
		pa_alg_en_cmd_set.hdr.opcode = FACTORY_ALG_BYPASS_SET;

		memcpy((void *)&pa_alg_en_cmd_set.alg_bypass_set,(void *)buf,param_size);
		/* send the cmd to dsp through apr, and wait dsp returned msg */
		ret = sia81xx_apr_send(afe, &pa_alg_en_cmd_set);
		if(0 > ret) {
			goto err;
		}
	} else{
...
#else
	struct asm_smart_pa_alg_en_cmd_get pa_alg_en_cmd_get;
#endif
	SIA81XX_AFE *afe = (SIA81XX_AFE *)afe_handle;
...
#else /* CAL_BLOCK_MAP_DISABLE */
	if(param_id == SIXTH_SIA81XX_RX_ENABLE) {
		pa_alg_en_cmd_get.hdr.hdr_field = APR_HDR_FIELD(APR_MSG_TYPE_SEQ_CMD,
							APR_HDR_LEN(APR_HDR_SIZE),
							APR_PKT_VER);
		pa_alg_en_cmd_get.hdr.pkt_size = sizeof(pa_alg_en_cmd_get);
		pa_alg_en_cmd_get.hdr.src_port = 0;
		pa_alg_en_cmd_get.hdr.dest_port = 0;
		pa_alg_en_cmd_get.hdr.src_svc = 0;
		pa_alg_en_cmd_get.hdr.src_domain = APR_DOMAIN_APPS;
		pa_alg_en_cmd_get.hdr.dest_svc = APR_SVC_PSY;
		pa_alg_en_cmd_get.hdr.dest_domain = APR_DOMAIN_ADSP;
		pa_alg_en_cmd_get.hdr.token = 0;
		pa_alg_en_cmd_get.hdr.opcode = FACTORY_ALG_BYPASS_STATUS_GET;;
	} else{
		goto err;
	}

	/* send the cmd to dsp through apr, and wait dsp returned msg */
	ret = sia81xx_apr_send(afe, &pa_alg_en_cmd_get);
	if(0 > ret) {
		goto err;
	}

	/* read data from dsp returned, and copy this to the user buf */
	memcpy(buf, (void *)(&alg_bypass_status), param_size);
#endif
