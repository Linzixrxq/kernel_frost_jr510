	...
	uint32_t hdl)
{
	...
	if (SIA81XX_TASK_ENABLE != atomic_read(&timer->task_state) && timer->cur_run_cnt == 0) {
		atomic_set(&timer->switch_flag, 1);
	} else {
		pr_debug("[debug][%s] %s: task_state : %d,cur_run_cnt %u\r\n",
			LOG_FLAG, __func__, atomic_read(&timer->task_state), timer->cur_run_cnt);
		goto done;
	}
	...
	//pr_debug("[debug][%s] %s: out,cur_run_cnt %u\r\n",
		//LOG_FLAG, __func__, timer->cur_run_cnt);
	...
	uint32_t hdl)
{
	...
	//pr_debug("[debug][%s] %s: enter,cur_run_cnt %u\r\n",
		//LOG_FLAG, __func__, timer->cur_run_cnt);
	...
	if ((i > 6*TIMER_TASK_CYCLE_TIME_MS) && (timer->cur_run_cnt != 0))
		pr_debug("[ debug][%s] %s: out,cur_run_cnt %u, i= %d \r\n",
				LOG_FLAG, __func__, timer->cur_run_cnt, i);
