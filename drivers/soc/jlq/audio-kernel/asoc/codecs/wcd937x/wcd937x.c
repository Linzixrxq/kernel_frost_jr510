...
#ifdef GPIO_USE_TEMP
#define GPIO_BASE                0x3450D000
#define GPIO_SIZE                0x1000
#define GPIO_PORT_DDR0          0x40
#define GPIO_PORT_DR0           0x00

#define GPIO_WCD_RST        15
static void __iomem *gpio_io_base;
#endif

#ifdef SUPPORT_WCD_REGU
static struct regulator *wcd_regu = NULL;
#endif

static int wcd937x_ssr_up = 0;
...
static struct snd_soc_dai_driver wcd937x_virtual_dais[] = {
	{
		.name = "wcd937x-virtual-dai",
...
static int wcd937x_handle_post_irq(void *data)
{
	...
	pr_debug("%s: status 1 0x%x, 0x%x, 0x%x\n",__func__,status1,status2,status3 );
...
static int wcd937x_init_reg(struct snd_soc_component *component)
{
	...
	if(snd_soc_component_read32(component, WCD937X_DIGITAL_EFUSE_REG_16) == 0x01)
		snd_soc_component_update_bits(component, WCD937X_BIAS_VBG_FINE_ADJ,
				0xF0, 0xB0);
	pr_info(" %s: WCD937X_DIGITAL_EFUSE_REG_16 0x%x\n",__func__,snd_soc_component_read32(component, WCD937X_DIGITAL_EFUSE_REG_16));
...
static int wcd937x_rx_clk_disable(struct snd_soc_component *component)
{
	...
	if (wcd937x->rx_clk_cnt == 0) {
		dev_info(wcd937x->dev, "%s:clk already disabled\n", __func__);
		return 0;
	}
	wcd937x->rx_clk_cnt--;
	if (wcd937x->rx_clk_cnt == 0) {
					...
					int event)
{
	...
	dev_info(component->dev, "%s wname: %s event: %d, hph 0x%x, ear 0x%x\n", __func__,
		w->name, event,snd_soc_component_read32(component,WCD937X_ANA_HPH),snd_soc_component_read32(component,WCD937X_ANA_EAR));
	...
	case SND_SOC_DAPM_PRE_PMD:
		wcd_disable_irq(&wcd937x->irq_info,
				WCD937X_IRQ_HPHL_PDM_WD_INT);
		if((snd_soc_component_read32(component,WCD937X_ANA_EAR)&0x80) != 0x80){
			if (wcd937x->update_wcd_event)
				wcd937x->update_wcd_event(wcd937x->handle,
							WCD_BOLERO_EVT_RX_MUTE,
							(WCD_RX1 << 0x10 | 0x1));
				       ...
				       int event)
{
	...
	case SND_SOC_DAPM_PRE_PMD:
		...
		if((snd_soc_component_read32(component,WCD937X_ANA_HPH)&0x80) != 0x80){
			if (wcd937x->update_wcd_event)
				wcd937x->update_wcd_event(wcd937x->handle,
						WCD_BOLERO_EVT_RX_MUTE,
						(WCD_RX1 << 0x10 | 0x1));
...
static int wcd937x_get_logical_addr(struct swr_device *swr_dev)
{
	...
	pr_info("%s: dev num 0x%x, dev 0x%x\n",__func__,devnum,swr_dev);
	return 0;
}
				...
				void *data)
{
	...
	case BOLERO_WCD_EVT_SSR_UP:
		...
		snd_soc_component_update_bits(component, WCD937X_ANA_MBHC_MECH, 0xff, 0x39);
		snd_soc_component_update_bits(component, WCD937X_DIGITAL_INTR_CLEAR_0, 0xff, 0x00);
		snd_soc_component_update_bits(component, WCD937X_DIGITAL_INTR_CLEAR_1, 0xff, 0x00);
		snd_soc_component_update_bits(component, WCD937X_DIGITAL_INTR_CLEAR_2, 0xff, 0x00);

		regcache_mark_dirty(wcd937x->regmap);
		regcache_sync(wcd937x->regmap);

		pr_debug("%s: wcd status 0x%x, clr 0x%x, fsm 0x%x\n",__func__, snd_soc_component_read32(component,WCD937X_DIGITAL_INTR_STATUS_0),snd_soc_component_read32(component,WCD937X_DIGITAL_INTR_CLEAR_0),snd_soc_component_read32(component, WCD937X_MBHC_NEW_FSM_STATUS));
		...
		pr_debug("%s: wcd status after ssr init 0x%x, fsm status 0x%x\n",__func__, snd_soc_component_read32(component,WCD937X_DIGITAL_INTR_STATUS_0),(snd_soc_component_read32(component, WCD937X_MBHC_NEW_FSM_STATUS)));
		break;
	default:
				 ...
				 struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_info("%s: comp1 en 0x%x, comp2 en 0x%x\n",__func__,wcd937x->comp1_enable,wcd937x->comp2_enable);
					 ...
					 int event)
{
	...
	if(!wcd_regu)
	{
		wcd_regu= regulator_get(component->dev, "wcd");
		if (IS_ERR(wcd_regu)) {
			pr_err("fail to get wcd regulator\n");
			return -EINVAL;
		}
		pr_info("%s: wcd_regu_get done 0x%x\n",__func__,wcd_regu);
	}

	ret = regulator_enable(wcd_regu);
	if(ret < 0)
	{
		dev_err(component->dev, "%s: wcd pm enable failed %d\n",__func__, ret);
		return ret;
	}

	pr_info("%s: wcd regulator_enable successs 0x%x\n",__func__,ret);
	...
	if(wcd_regu)
	{
		ret = regulator_disable(wcd_regu);
		if(ret < 0)
		{
			dev_err(component->dev, "%s: wcd pm disable failed %d\n",__func__, ret);
			return ret;
		}
	}
	pr_info("%s: regulator disable success2\n",__func__);
...
static int wcd937x_soc_codec_probe(struct snd_soc_component *component)
{
	...
	wcd_mbhc = &wcd937x->mbhc->wcd_mbhc;
...
static int wcd937x_suspend(struct device *dev)
{
	...
	if (!pdata) {
		...
		if(!wcd_regu)
		{
			wcd_regu= regulator_get(dev, "wcd");
			if (IS_ERR(wcd_regu)) {
				pr_err("fail to get wcd regulator\n");
				return -EINVAL;
			}
		...
		if(ret < 0)
		{
			dev_err(dev, "%s: wcd pm enable failed %d\n",__func__, ret);
			return ret;
		}
	...
	if (test_bit(ALLOW_BUCK_DISABLE, &wcd937x->status_mask)) {
        ...
        pr_err("%s: msm_cdc_disable_ondemand_supply\n",__func__);
		clear_bit(ALLOW_BUCK_DISABLE, &wcd937x->status_mask);
	}
	...
	if(wcd_regu)
	{
		...
		if(ret < 0)
		{
			dev_err(dev, "%s: wcd pm disable failed %d\n",__func__, ret);
			return ret;
		}
	}

	pr_info("%s: regulator disable success\n",__func__);
#endif
...
static int wcd937x_reset(struct device *dev)
{
...
#if defined(GPIO_USE_TEMP)
	value = 0; //unused
	gpio_dir = ioread32(gpio_io_base + GPIO_PORT_DDR0);
	gpio_dir |= (1 << (GPIO_WCD_RST + 16)) | (1 << GPIO_WCD_RST);
	iowrite32(gpio_dir,gpio_io_base + GPIO_PORT_DDR0);

	gpio_val = ioread32(gpio_io_base + GPIO_PORT_DR0);
	gpio_val &= ~(1 << GPIO_WCD_RST);
	gpio_val |= (1 << (GPIO_WCD_RST + 16));
	iowrite32(gpio_val,gpio_io_base + GPIO_PORT_DR0);
	...
	gpio_val = ioread32(gpio_io_base + GPIO_PORT_DR0);
	gpio_val |= (1 << (GPIO_WCD_RST + 16)) | (1 << GPIO_WCD_RST);
	iowrite32(gpio_val,gpio_io_base + GPIO_PORT_DR0);
	/* 20ms sleep required after pulling the reset gpio to HIGH */
	usleep_range(20, 30);
	dev_err(dev, "%s: GPIO_USE_TEMP, reset gpio device node\n",__func__);
#elif defined(WCD_RST_GPIO)
	rc = gpio_direction_output(wcd937x->old_rst_gpio, 0);
	if (rc < 0) {
		dev_err(dev, "%s: Failed to config GPIO%d.\n",__func__,wcd937x->old_rst_gpio);
		return rc;
	}
	value = gpio_get_value(wcd937x->old_rst_gpio);
	dev_info(dev, "%s: gpio_get_value reset gpio,value %d\n",__func__,value);

	gpio_set_value(wcd937x->old_rst_gpio, 0);
	/* 20ms sleep required after pulling the reset gpio to LOW */
	usleep_range(20, 30);

	gpio_set_value(wcd937x->old_rst_gpio, 1);
	/* 20ms sleep required after pulling the reset gpio to HIGH */
	usleep_range(20, 30);
#else
	if (!wcd937x->rst_np) {
					  ...
					  struct wcd937x_micbias_setting *mb)
{
	...
	/* MB1 */
	if (of_find_property(dev->of_node, "jlq,cdc-micbias1-mv",
				    NULL)) {
	...
	/* MB2 */
	if (of_find_property(dev->of_node, "jlq,cdc-micbias2-mv",
				    NULL)) {
		rc = wcd937x_read_of_property_u32(dev,
						  "jlq,cdc-micbias2-mv",
						  &prop_val);
	...
	/* MB3 */
	if (of_find_property(dev->of_node, "jlq,cdc-micbias3-mv",
				    NULL)) {
		rc = wcd937x_read_of_property_u32(dev,
						  "jlq,cdc-micbias3-mv",
						  &prop_val);
...
struct wcd937x_pdata *wcd937x_populate_dt_data(struct device *dev)
{
...
#if defined(GPIO_USE_TEMP)
	gpio_io_base = ioremap_nocache(GPIO_BASE, GPIO_SIZE);
	dev_info(dev, "%s: ioremap for GPIO_BASE\n",__func__);
#elif defined(WCD_RST_GPIO)
	pdata->old_rst_gpio = of_get_named_gpio(dev->of_node,"wcd-rst-gpio", 0);
	if (gpio_is_valid(pdata->old_rst_gpio)) {
		dev_info(dev, "%s: gpio %d request\n",__func__, pdata->old_rst_gpio);
		ret = gpio_request(pdata->old_rst_gpio, "wcd-cdc-rst");
		if (ret) {
			dev_err(dev, "%s: Failed to request gpio %d, ret %d\n",
				__func__, pdata->old_rst_gpio,ret);
			return NULL;
		}
	}
#else
	pdata->rst_np = of_parse_phandle(dev->of_node,
			"jlq,wcd-rst-gpio-node", 0);

	if (!pdata->rst_np) {
		dev_err(dev, "%s: Looking up %s property in node %s failed\n",
				__func__, "jlq,wcd-rst-gpio-node",
	...
	pdata->rx_slave = of_parse_phandle(dev->of_node, "jlq,rx-slave", 0);
	pdata->tx_slave = of_parse_phandle(dev->of_node, "jlq,tx-slave", 0);
...
static int wcd937x_bind(struct device *dev)
{
...
#if defined(GPIO_USE_TEMP)
	dev_info(dev, "%s: GPIO_USE_TEMP,gpio_io_base 0x%x\n",
				__func__,gpio_io_base);
#elif defined(WCD_RST_GPIO)
	wcd937x->old_rst_gpio = pdata->old_rst_gpio;
	dev_info(dev, "%s: WCD_RST_GPIO,old_rst_gpio %d\n",
				__func__,wcd937x->old_rst_gpio);
#else
	...
	ret = wcd937x_parse_port_mapping(dev, "jlq,rx_swr_ch_map", CODEC_RX);
	ret |= wcd937x_parse_port_mapping(dev, "jlq,tx_swr_ch_map", CODEC_TX);
	...
	ret = snd_soc_register_component(dev, &soc_codec_dev_wcd937x,
				     wcd937x_virtual_dais, ARRAY_SIZE(wcd937x_virtual_dais));
	if (ret) {
...
err_bind_all:
#ifdef WCD_RST_GPIO
	if (gpio_is_valid(wcd937x->old_rst_gpio)){
		gpio_free(wcd937x->old_rst_gpio);
		dev_err(dev, "%s: err_bind_all, free old_rst_gpio %d\n",
			__func__,wcd937x->old_rst_gpio);
	}
#endif
...
#if defined(GPIO_USE_TEMP)
	if(gpio_io_base != NULL)
		iounmap(gpio_io_base);
#elif defined(WCD_RST_GPIO)
	if (gpio_is_valid(wcd937x->old_rst_gpio))
		gpio_free(wcd937x->old_rst_gpio);
#else
...
static const struct of_device_id wcd937x_dt_match[] = {
	{ .compatible = "jlq,wcd937x-codec" , .data = "wcd937x" },
	{}
};
				...
				struct component_match **matchptr)
{
	...
	rx_node = of_parse_phandle(np, "jlq,rx-slave", 0);
	if (!rx_node) {
	...
	tx_node = of_parse_phandle(np, "jlq,tx-slave", 0);
	if (!tx_node) {
...
static int __init wcd937x_codec_init(void)
{
    int r = platform_driver_register(&wcd937x_codec_driver);
...
static void __exit wcd937x_codec_exit(void)
{
   platform_driver_unregister(&wcd937x_codec_driver);
}

module_init(wcd937x_codec_init);
module_exit(wcd937x_codec_exit);
