...
static int msm_cdc_pinctrl_probe(struct platform_device *pdev)
{
	...
	if (!of_property_read_bool(pdev->dev.of_node, "jlq,lpi-gpios")) {
	...
	count = of_property_count_u32_elems(pdev->dev.of_node, "jlq,chip-wakeup-reg");
	if (count <= 0)
		goto cdc_tlmm_gpio;
	if (!of_property_read_u32_array(pdev->dev.of_node, "jlq,chip-wakeup-reg",
				chip_wakeup_reg, count)) {
		if (of_property_read_u32_array(pdev->dev.of_node,
					   "jlq,chip-wakeup-maskbit",
...
cdc_tlmm_gpio:
	count = of_property_count_u32_elems(pdev->dev.of_node, "jlq,tlmm-gpio");
	if (count <= 0)
		goto cdc_rst;
	if (!of_property_read_u32_array(pdev->dev.of_node, "jlq,tlmm-gpio",
				tlmm_gpio, count)) {
...
EXPORT_SYMBOL(msm_cdc_pinctrl_drv_init);


void msm_cdc_pinctrl_drv_exit(void)
{
}
EXPORT_SYMBOL(msm_cdc_pinctrl_drv_exit);
