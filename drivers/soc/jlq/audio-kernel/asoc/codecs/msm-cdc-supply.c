				      ...
				      const char *name, bool is_ond)
{
	...
	snprintf(prop_name, CODEC_DT_MAX_PROP_SIZE, "jlq,%s-voltage", name);
	...
	snprintf(prop_name, CODEC_DT_MAX_PROP_SIZE, "jlq,%s-current", name);
	...
	snprintf(prop_name, CODEC_DT_MAX_PROP_SIZE, "jlq,%s-lpm-supported", name);
	rc = of_property_read_u32(dev->of_node, prop_name, &prop_val);
	if (rc) {
			       ...
			       int *total_num_supplies)
{
	...
	const char *cp_prop_name = "jlq,cdc-cp-supplies";
