...
char __iomem *va_top_cfg_base;
char __iomem *va_bolero_if_base;
#ifdef DMAS_TEST
char __iomem *audsys_base;
char __iomem *dmas_base;
char __iomem *dmas_src;
#endif
				  ...
				  u16 event, u32 data)
{
    ...
    pr_info("%s: event 0x%x,   data 0x%x\n",__func__,event,data);
				      ...
				      bool enable)
{
	...
	if (enable) {
		...
		if (va_priv->swr_clk_users == 0) {
			dev_err(va_priv->dev, "%s: reset_swr: %d\n",
...
static int va_macro_swrm_clock(void *handle, bool enable)
{
	...
	pr_info("%s: swrm clock %s tx_swr_clk_cnt: %d va_swr_clk_cnt: %d\n",
...
static void va_macro_mute_update_callback(struct work_struct *work)
{
    ...
    if(decimator == 1)
    {
        writel(8, dmas_base + DMAS0_EN_OFFSET);
        pr_err("%s: dmas en 0x%x\n",__func__,readl(dmas_base + DMAS0_EN_OFFSET));
    }
#endif
}
...
#ifdef BOLERO_SUPPORT_VAD
		snd_soc_component_update_bits(component,
				BOLERO_CDC_VA_VAD_FILTER_CFG, 0xff, 0x01);
		snd_soc_component_update_bits(component,
				BOLERO_CDC_VA_VAD_IRQ_MSK, 0xff, 0x00);
		snd_soc_component_update_bits(component,
				BOLERO_CDC_VA_VAD_CH0_SNR_THR_L, 0xff, 0xfb);
		snd_soc_component_update_bits(component,
				BOLERO_CDC_VA_VAD_CH0_N_FRM, 0xff, 0x00);
		snd_soc_component_update_bits(component,
				BOLERO_CDC_VA_VAD_INTR1_MSK_EN, 0xff, 0x00);
		snd_soc_component_update_bits(component,
				BOLERO_CDC_VA_VAD_INTR2_MSK_EN, 0xff, 0x00);
...
static int va_macro_probe(struct platform_device *pdev)
{
	...
	const char *micb_voltage_str = "jlq,va-vdd-micb-voltage";
	const char *micb_current_str = "jlq,va-vdd-micb-current";
	int ret = 0;
	const char *dmic_sample_rate = "jlq,va-dmic-sample-rate";
	u32 default_clk_id = 0;
	struct clk *lpass_audio_hw_vote = NULL;
	u32 is_used_va_swr_gpio = 0;
	const char *is_used_va_swr_gpio_dt = "jlq,is-used-swr-gpio";
#ifdef HAPS_TEST
	char __iomem *lpm_base;
#endif
...
//for dbg
	va_top_cfg_base = devm_ioremap(&pdev->dev, 0x34511000, 0x1000);
	lpm_base = devm_ioremap(&pdev->dev, 0x34510000, 0x1000);
	if ((!va_top_cfg_base)|(!lpm_base)) {
		dev_err(&pdev->dev, "%s: bolero ioremap failed xxxxxx\n", __func__);
		return -ENOMEM;
	}

#ifdef BOLERO_REGU
    ret = va_regu_get(pdev);
    if(ret < 0)
    {
        dev_err(&pdev->dev, "%s: va_regu_get failed %d\n",__func__, ret);
        return -EPROBE_DEFER;
    }
    ...
    if(ret < 0)
    {
        dev_err(&pdev->dev, "%s: va_regu_enable failed %d\n",__func__, ret);
        return ret;
    }
#endif
...
//bolero reset
    ...
    //close clock
    writel(0xffff0000, va_top_cfg_base + 0x610);
    pr_debug(" va top 0x610 0x%x\n", readl(va_top_cfg_base + 0x610));

    //de-reset -->reset
    writel(0xffff0000, va_top_cfg_base + 0x2a4);
    pr_debug("  va top 0x2a4 0x%x\n", readl(va_top_cfg_base + 0x2a4));

    writel(0xffffffff, va_top_cfg_base + 0x2a4);
    pr_debug("  va top 0x2a4 0x%x\n", readl(va_top_cfg_base + 0x2a4));

    //open clock
    writel(0xffffffff, va_top_cfg_base + 0x610);
    pr_info("  va top_cfg_base + 0x610 0x%x\n", readl(va_top_cfg_base + 0x610));

	va_bolero_if_base = devm_ioremap(&pdev->dev,
					0x340EC000, 0x1000);
	if (!va_bolero_if_base) {
		dev_err(&pdev->dev, "%s: bolero ioremap failed\n", __func__);
		return -ENOMEM;
	}
	pr_debug("%s: devm_ioremap 0x340EC000,bolero if base 0x%x\n",__func__,va_bolero_if_base);

	writel(0x01, va_bolero_if_base + 0x48);
	pr_debug("%s: bolero if base 0x48 0x%x\n",__func__,readl(va_bolero_if_base + 0x48));

	writel(0x10001, va_bolero_if_base + 0x44);
	pr_debug("%s: bolero if base 0x44 0x%x\n",__func__,readl(va_bolero_if_base + 0x44));

	writel(0x10001, va_bolero_if_base + 0x40);
	pr_debug("%s: bolero if base 0x40 0x%x\n",__func__,readl(va_bolero_if_base + 0x40));

	//dmas cfg
#ifdef DMAS_TEST
	audsys_base = devm_ioremap(&pdev->dev, 0x340EA000, 0x1000);
	dmas_src = devm_ioremap(&pdev->dev, 0xd0000000, 0x100000);
	if ((!dmas_src) || (!audsys_base)) {
		dev_err(&pdev->dev, "%s: bolero dmas ioremap failed xxxxxx\n", __func__);
		return -ENOMEM;
	}

	dmas_base = devm_ioremap(&pdev->dev,
					0x340E6000, 0x1000);
	if(!dmas_base){
	dev_err(&pdev->dev, "%s: dmas_base ioremap failed\n", __func__);
			return -ENOMEM;
	}

#if 0
	memset(dmas_src, 0x24,0x1000);

	writel(1, audsys_base + 0x404);    //dmas reset
	pr_err("%s: dmas reset 0x%x\n",__func__,readl(audsys_base + 0x404));

	writel((0x340EC000), dmas_base+DMAS0_CH0_SAR_OFFSET + 0x20*8);
	pr_err("%s: dmas sar 0x%x\n",__func__,readl(dmas_base+DMAS0_CH0_SAR_OFFSET + 0x20*8));

	writel(0xd0000000, dmas_base+DMAS0_CH0_DAR_OFFSET + 0x20*8);
	pr_err("%s: dmas dar 0x%x\n",__func__,readl(dmas_base+DMAS0_CH0_DAR_OFFSET + 0x20*8));

	writel(0x10000000, dmas_base + DMAS0_CH0_CTL0_OFFSET + 0x20*8);
	pr_err("%s: dmas ctrl0 0x%x\n",__func__,readl(dmas_base + DMAS0_CH0_CTL0_OFFSET + 0x20*8));

	temp = readl(dmas_base + DMAS0_CH0_CTL1_OFFSET + 0x20*8);
	writel((temp&0xfffffffc), dmas_base + DMAS0_CH0_CTL1_OFFSET + 0x20*8);

	temp = readl(dmas_base + DMAS0_CH0_CTL1_OFFSET + 0x20*8);
	writel((temp|0xe), dmas_base + DMAS0_CH0_CTL1_OFFSET + 0x20*8);
	pr_err("%s: dmas ctrl1 0x%x\n",__func__,readl(dmas_base + DMAS0_CH0_CTL1_OFFSET + 0x20*8));
...
static int __init va_driver_init(void)
{
    int r = platform_driver_register(&va_macro_driver);
...
static void __exit va_driver_exit(void)
{
   platform_driver_unregister(&va_macro_driver);
}

module_init(va_driver_init);
module_exit(va_driver_exit);
