...
char __iomem *rx_bolero_if_base;
char __iomem *rx_top_cfg_base;
			       ...
			       struct snd_soc_dai *dai)
{
	...
	dev_info(component->dev,
		"%s: dai_name = %s DAI-ID %x rate %d num_ch %d, stream %d\n", __func__,
		 dai->name, dai->id, params_rate(params),
		 params_channels(params),substream->stream);
...
static int rx_macro_digital_mute(struct snd_soc_dai *dai, int mute)
{
	...
	for (j = 0; j < INTERP_MAX; j++) {
		...
		pr_info("%s: dai 0x%x, dsm reg 0x%x,idx 0x%x, cfg0 0x%x,cfg1 0x%x\n",__func__,dai->id,snd_soc_component_read32(component, dsm_reg),j,int_mux_cfg0_val,int_mux_cfg1_val);
...
exit:
	pr_debug("%s: mclk_enable = %u, dapm = %d clk_users= %d, fs 0x%x\n",
		__func__, mclk_enable, dapm, rx_priv->rx_mclk_users,readl(rx_priv->rx_io_base+0x104));
				  ...
				  u16 event, u32 data)
{
	...
	case BOLERO_MACRO_EVT_RX_MUTE:
        ...
        if(val == 0)
        {
            writel(0, dmas_base + DMAS0_EN_OFFSET);
            pr_info("%s: dmas en 0x%x\n",__func__,readl(dmas_base + DMAS0_EN_OFFSET));
        }
#endif
		break;
						  ...
						  struct rx_macro_priv *rx_priv)
{
	...
	for (i = RX_MACRO_AIF1_PB; i < RX_MACRO_MAX_DAIS; i++) {
		if (test_bit(port_id, &rx_priv->active_ch_mask[i]))
			{
				pr_info("%s, 0x%x, 0x%x, 0x%x\n", __func__,port_id,rx_priv->active_ch_mask[i],i);
				return i;
			}
	}
					...
					int interp, int path_type)
{
	...
	dev_info(component->dev, "%s: num_ports: %d, ports[%d %d %d %d]\n",
	...
	dev_info(component->dev, "%s: (new) idle_thr: %d, (cur) idle_thr: %d, num port %d\n",
		__func__, idle_thr, rx_priv->idle_det_cfg.hph_idle_thr,num_ports);
			     ...
			     int interp_idx)
{
	...
	dev_dbg(component->dev, "%s cfg0 0x%x, cfg1 0x%x\n", __func__,int_mux_cfg0_val,int_mux_cfg1_val);
					...
					int event)
{
	...
	dev_dbg(component->dev, "%s %d %s, shift 0x%x\n", __func__, event, w->name,w->shift);
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		if((snd_soc_component_read32(component,reg)&0x20) == 0)
		{
			snd_soc_component_update_bits(component,reg, 0x20, 0x20);
			pr_info("%s: path clk 0x%x, reg 0x%x, 0x144 0x%x, 0x610 0x%x\n",__func__,snd_soc_component_read32(component,reg),reg,readl(rx_top_cfg_base+0x144),readl(rx_top_cfg_base+0x610));
		}
		break;
	case SND_SOC_DAPM_POST_PMU:
		snd_soc_component_write(component, gain_reg,
				...
				int interp_n, int event)
{
	...
	comp = interp_n;
	dev_info(component->dev, "%s: event %d compander %d, enabled %d\n",
	...
	if (SND_SOC_DAPM_EVENT_ON(event)) {
		...
		snd_soc_component_update_bits(component, rx_path_cfg0_reg,
					0x02, 0x02);
		dev_dbg(component->dev, "%s: dapm event on\n", __func__);
	}
	...
	if (SND_SOC_DAPM_EVENT_OFF(event)) {
		...
		dev_dbg(component->dev, "%s: dapm event off\n", __func__);
	}
				...
				int interp_n, int event)
{
	...
	pr_info("%s: inter 0x%x, ear mode 0x%x",__func__,interp_n,rx_priv->is_ear_mode_on);
					...
					struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_debug("%s: detect en 0x%x\n",__func__,rx_priv->idle_det_cfg.hph_idle_detect_en);
			       ...
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	dev_info(component->dev, "%s: Compander %d enable current %d, new %d\n",
			       ...
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_info("%s: ear mode on 0x%x\n",__func__,rx_priv->is_ear_mode_on);
	return 0;
}
			       ...
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_debug("%s: hph hd2 mode 0x%x\n",__func__,rx_priv->hph_hd2_mode);
	return 0;
}
			       ...
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_debug("%s: hph pwr mode 0x%x\n",__func__,rx_priv->hph_pwr_mode);
	return 0;
}
...
static const struct snd_kcontrol_new rx_macro_snd_controls[] = {
	...
	SOC_SINGLE_S8_TLV("RX_RX0 Mix Digital Volume",
		BOLERO_CDC_RX_RX0_RX_VOL_MIX_CTL, -84, 40, digital_gain),
	SOC_SINGLE_S8_TLV("RX_RX1 Mix Digital Volume",
		BOLERO_CDC_RX_RX1_RX_VOL_MIX_CTL, -84, 40, digital_gain),
	SOC_SINGLE_S8_TLV("RX_RX2 Mix Digital Volume",
		BOLERO_CDC_RX_RX2_RX_VOL_MIX_CTL, -84, 40, digital_gain),
...
static int rx_macro_probe(struct platform_device *pdev)
{
	...
	const char *is_used_rx_swr_gpio_dt = "jlq,is-used-swr-gpio";
	u32 temp;
	...
	ret = of_property_read_u32(pdev->dev.of_node, "jlq,rx_mclk_mode_muxsel",
				   &muxsel);
	if (ret) {
		///dev_err(&pdev->dev, "%s: could not find %s entry in dt\n",
			//__func__, "jlq,rx_mclk_mode_muxsel");
		//return ret;
	}
	ret = of_property_read_u32(pdev->dev.of_node, "jlq,default-clk-id",
				   &default_clk_id);
	if (ret) {
		dev_err(&pdev->dev, "%s: could not find %s entry in dt\n",
			__func__, "jlq,default-clk-id");
    ...
    //for dbg
    rx_top_cfg_base = devm_ioremap(&pdev->dev, 0x34511000, 0x1000);
    if (!rx_top_cfg_base) {
        dev_err(&pdev->dev, "%s: rx bolero top ioremap failed xxxxxx\n", __func__);
        return -ENOMEM;
    }

	rx_bolero_if_base = devm_ioremap(&pdev->dev,
					0x340EC000, 0x1000);
	if (!rx_bolero_if_base) {
		dev_err(&pdev->dev, "%s: rx bolero ioremap failed\n", __func__);
		return -ENOMEM;
	}

    temp = readl(rx_bolero_if_base + 0x44);
    writel(temp|0xa000a0, rx_bolero_if_base + 0x44);
    pr_debug("%s: rx bolero if base 0x44 0x%x\n",__func__,readl(rx_bolero_if_base + 0x44));

    temp = readl(rx_bolero_if_base + 0x40);
    writel(temp|0xa000a0, rx_bolero_if_base + 0x40);
    pr_debug("%s: rx bolero if base 0x40 0x%x\n",__func__,readl(rx_bolero_if_base + 0x40));

#ifdef DMAS_TEST
    //dmas cfg
    writel(1, audsys_base + 0x404);    //dmas reset
    pr_info("%s: dmas reset 0x%x\n",__func__,readl(audsys_base + 0x404));

    writel((0xd0000000), dmas_base+DMAS0_CH0_SAR_OFFSET + 0x20*0);    //HPH main fifo
    pr_info("%s: dmas sar 0x%x\n",__func__,readl(dmas_base+DMAS0_CH0_SAR_OFFSET + 0x20*0));

    writel( 0x340EC014, dmas_base+DMAS0_CH0_DAR_OFFSET + 0x20*0);
    pr_info("%s: dmas dar 0x%x\n",__func__,readl(dmas_base+DMAS0_CH0_DAR_OFFSET + 0x20*0));

    writel(0x1000000, dmas_base + DMAS0_CH0_CTL0_OFFSET + 0x20*0);
    pr_info("%s: dmas ctrl0 0x%x\n",__func__,readl(dmas_base + DMAS0_CH0_CTL0_OFFSET + 0x20*0));

    temp = readl(dmas_base + DMAS0_CH0_CTL1_OFFSET + 0x20*0);
    writel((temp&0xfffffffc), dmas_base + DMAS0_CH0_CTL1_OFFSET + 0x20*0); 

    temp = readl(dmas_base + DMAS0_CH0_CTL1_OFFSET + 0x20*0);
    writel((temp|0x2), dmas_base + DMAS0_CH0_CTL1_OFFSET + 0x20*0);
    pr_info("%s: dmas ctrl1 0x%x\n",__func__,readl(dmas_base + DMAS0_CH0_CTL1_OFFSET + 0x20*0));

#endif

    // set clk
    lpass_audio_hw_vote = devm_clk_get(&pdev->dev, "rx_npl_clk");
    ...
    if(lpass_audio_hw_vote)
    {
        ...
        pr_debug("%s: rx clk get rate 0x%x\n",__func__,ret);

        ret = of_property_read_u32(pdev->dev.of_node, "bolero-rx-clk", &bolero_rx_clk);
        if(ret)
        {
            pr_err("%s: read rx bolero-clk error\n",__func__);
        }
        else
        {
            pr_debug(" %s: rx bolero-clk is %x\n", __func__, bolero_rx_clk);
        }

        if(bolero_rx_clk)
        {
            ret = clk_set_rate(lpass_audio_hw_vote, bolero_rx_clk);  //19.2M
            pr_debug("%s: rx clk set rate 0x%x\n",__func__,ret);

            if(ret)
            {
                pr_err("%s: Failed to set bolero clk to bolero_clk rate\n",__func__);
            }
            ret = clk_get_rate(lpass_audio_hw_vote);
        }

        clk_prepare_enable(lpass_audio_hw_vote);
        pr_debug("%s: rx probe 0x144 0x%x, 0x610 0x%x\n",__func__,readl(rx_top_cfg_base+0x144),readl(rx_top_cfg_base+0x610));
    }
	...
	ret = of_property_read_u8_array(pdev->dev.of_node,
				"jlq,rx-bcl-pmic-params", bcl_pmic_params,
				sizeof(bcl_pmic_params));
	if (ret) {
		dev_dbg(&pdev->dev, "%s: could not find %s entry in dt\n",
			__func__, "jlq,rx-bcl-pmic-params");
	} else {
...
{
    int r = platform_driver_register(&rx_macro_driver);
...
{
   platform_driver_unregister(&rx_macro_driver);
}
