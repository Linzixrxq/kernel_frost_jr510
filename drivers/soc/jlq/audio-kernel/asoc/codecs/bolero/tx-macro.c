...
char __iomem *tx_bolero_if_base;
char __iomem *va_io_base1;
	...
	if (mclk_enable) {
		...
		if (tx_priv->tx_mclk_users == 0) {
			...
			regmap_update_bits(regmap,
				...
				0x01, 0x01);
#if 0//def DMAS_TEST(for DMIC TEST)
			writel(9, dmas_base + DMAS0_EN_OFFSET);
			pr_err("%s: tx dmas en 0x%x\n",__func__,readl(dmas_base + DMAS0_EN_OFFSET));
#endif
		}
			      ...
			      struct snd_ctl_elem_value *ucontrol)
{
	...
	dev_dbg(component->dev, "%s: wname: %s, val: 0x%x, reg 0x%x\n", __func__,
		widget->name, val, e->reg);
			       ...
			       struct snd_kcontrol *kcontrol, int event)
{
	...
	dev_dbg(component->dev, "%s(): widget = %s decimator = %u, event 0x%x\n", __func__,
...
#if 0//def DMAS_TEST
        writel(9, dmas_base + 4);
        pr_err("%s: tx dmas disable 0x%x\n",__func__,readl(dmas_base + 4));
#endif
		snd_soc_component_update_bits(component,
	...
	if (enable) {
		...
		if (tx_priv->swr_clk_users == 0) {
			...
			if (tx_priv->reset_swr)
            {
                ...
                writel(0x02,va_io_base1+BOLERO_CDC_VA_CLK_RST_CTRL_SWR_CONTROL-VA_START_OFFSET);
                pr_info("%s: va ahb offset 0x8: 0x%x\n",__func__,readl(va_io_base1+ 0x8));
            }
            ...
            writel(0x01, va_io_base1+BOLERO_CDC_VA_CLK_RST_CTRL_SWR_CONTROL-VA_START_OFFSET);
            pr_info("%s: va ahb offset 0x8: 0x%x\n",__func__,readl(va_io_base1+ 0x8));
			...
			if (tx_priv->reset_swr)
            {
                ...
                writel(0x00, va_io_base1 +BOLERO_CDC_VA_CLK_RST_CTRL_SWR_CONTROL-VA_START_OFFSET);
                pr_info("%s: va ahb offset 0x8: 0x%x\n",__func__,readl(va_io_base1+ 0x8));
            }
...
static int tx_macro_probe(struct platform_device *pdev)
{
	...
	int ret = 0;
	const char *dmic_sample_rate = "jlq,tx-dmic-sample-rate";
	u32 is_used_tx_swr_gpio = 1;
	const char *is_used_tx_swr_gpio_dt = "jlq,is-used-swr-gpio";
	...
	if (msm_cdc_pinctrl_get_state(tx_priv->tx_swr_gpio_p) < 0 &&
			is_used_tx_swr_gpio) {
		dev_err(&pdev->dev, "%s: failed to get swr pin state xxx\n",
			__func__);
		//return -EPROBE_DEFER;
	}
    ...
    va_io_base1 = devm_ioremap(&pdev->dev, 0x34020000, TX_MACRO_MAX_OFFSET);
	if (!va_io_base1) {
		dev_err(&pdev->dev, "%s: tx va ioremap failed\n", __func__);
		return -EINVAL;
	}

	tx_bolero_if_base = devm_ioremap(&pdev->dev,
					0x340EC000, 0x1000);
	if (!tx_bolero_if_base) {
		dev_err(&pdev->dev, "%s: tx bolero ioremap failed\n", __func__);
		return -ENOMEM;
	}

	writel(0x01, tx_bolero_if_base + 0x48);
	pr_debug("%s: tx bolero if base 0x48 0x%x\n",__func__,readl(tx_bolero_if_base + 0x48));

	temp = readl(tx_bolero_if_base + 0x44);
	writel(temp|0x20002, tx_bolero_if_base + 0x44);
	pr_debug("%s: tx bolero if base 0x44 0x%x\n",__func__,readl(tx_bolero_if_base + 0x44));

	temp = readl(tx_bolero_if_base + 0x40);
	writel(temp|0x20002, tx_bolero_if_base + 0x40);
	pr_info("%s: tx bolero if base 0x40 0x%x\n",__func__,readl(tx_bolero_if_base + 0x40));
	...
	writel((0x340EC004), dmas_base+DMAS0_CH0_SAR_OFFSET + 0x20*9);
	pr_err("%s: dmas sar 0x%x\n",__func__,readl(dmas_base+DMAS0_CH0_SAR_OFFSET + 0x20*9));

	writel(0xd0000000, dmas_base+DMAS0_CH0_DAR_OFFSET + 0x20*9);
	pr_err("%s: dmas dar 0x%x\n",__func__,readl(dmas_base+DMAS0_CH0_DAR_OFFSET + 0x20*9));

	writel(0x100000, dmas_base + DMAS0_CH0_CTL0_OFFSET + 0x20*9);
	pr_err("%s: dmas ctrl0 0x%x\n",__func__,readl(dmas_base + DMAS0_CH0_CTL0_OFFSET + 0x20*9));

	temp = readl(dmas_base + DMAS0_CH0_CTL1_OFFSET + 0x20*9);
	writel((temp&0xfffffffc), dmas_base + DMAS0_CH0_CTL1_OFFSET + 0x20*9);

	temp = readl(dmas_base + DMAS0_CH0_CTL1_OFFSET + 0x20*9);
	writel((temp|0x2), dmas_base + DMAS0_CH0_CTL1_OFFSET + 0x20*9);
	pr_err("%s: dmas ctrl1 0x%x\n",__func__,readl(dmas_base + DMAS0_CH0_CTL1_OFFSET + 0x20*9));
...
{
    int r = platform_driver_register(&tx_macro_driver);
...
{
   platform_driver_unregister(&tx_macro_driver);
}
...
MODULE_SOFTDEP("pre: va_macro_dlkm rx_macro_dlkm");
