...
{
	int ret = 0;
	pr_debug("%s: enable 0x%x 0x%x, cnt 0x%x\n",__func__,enable,clk_id,priv->clk_cnt[clk_id]);
					   ...
					   bool enable)
{
	...
	//pr_info("%s: mux1 clk enable 0x%x 0x%x, handle 0x%x,0x%x\n",__func__,enable,clk_id,priv->clk_cnt[clk_id],clk_muxsel);
				...
				bool enable)
{
	...
	pr_debug("%s: before top cfg 0x144 0x%x, top cfg 0x610 0x%x\n",__func__,readl(rsc_top_cfg_base + 0x144),readl(rsc_top_cfg_base + 0x610));
	...
	if (!priv->dev_up && enable) {
		dev_err_ratelimited(priv->dev, "%s: SSR is in progress..\n",
				__func__);
		trace_printk("%s: SSR is in progress..,0x%x, 0x%x\n", __func__,priv->dev_up,enable);
		ret = -EINVAL;
		goto err;
	}
	...
	pr_debug("%s: top cfg 0x144 0x%x, top cfg 0x610 0x%x\n",__func__,readl(rsc_top_cfg_base + 0x144),readl(rsc_top_cfg_base + 0x610));
...
static int bolero_clk_rsc_probe(struct platform_device *pdev)
{
	...
	if (!of_find_property(pdev->dev.of_node, "jlq,fs-gen-sequence",
						  &fs_gen_size)) {
		dev_err(&pdev->dev, "%s: unable to find jlq,fs-gen-sequence property\n",
			__func__);
		ret = -EINVAL;
		goto err;
	}
...
//for dbg
	rsc_top_cfg_base = devm_ioremap(&pdev->dev, 0x34511000, 0x1000);
	if (!rsc_top_cfg_base) {
		dev_err(&pdev->dev, "%s: bolero rsc ioremap failed xxxxxx\n", __func__);
		return -ENOMEM;
	}
	...
	for (i = 0; i < MAX_CLK; i++) {
		...
		for (j = 0; j < clk_cnt; j++) {
			if (!strcmp(clk_src_name[i], clk_name_array[j])) {
				...
				priv->clk[i] = clk;
				dev_dbg(&pdev->dev, "%s: clk get success for clk name %s, clk handle 0x%x\n",
						__func__, clk_src_name[i],priv->clk[i]);
			}
		}
	}
	ret = of_property_read_u32(pdev->dev.of_node,
				 "jlq,rx_mclk_mode_muxsel", &muxsel);
	if (ret) {
		dev_dbg(&pdev->dev, "%s: could not find jlq,rx_mclk_mode_muxsel entry in dt\n",
			__func__);
	} else {
	...
	ret = of_property_read_u32(pdev->dev.of_node,
				"jlq,wsa_mclk_mode_muxsel", &muxsel);
	if (ret) {
		dev_dbg(&pdev->dev, "%s: could not find jlq,wsa_mclk_mode_muxsel entry in dt\n",
			__func__);
	} else {
	...
	ret = of_property_read_u32(pdev->dev.of_node,
				 "jlq,va_mclk_mode_muxsel", &muxsel);
	if (ret) {
		dev_dbg(&pdev->dev, "%s: could not find jlq,va_mclk_mode_muxsel entry in dt\n",
			__func__);
	} else {
