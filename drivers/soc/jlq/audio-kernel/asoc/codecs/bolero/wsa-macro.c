...
static int wsa_macro_probe(struct platform_device *pdev)
{
	...
	const char *is_used_wsa_swr_gpio_dt = "jlq,is-used-swr-gpio";
...
static int __init wsa_driver_init(void)
{
    int r = platform_driver_register(&wsa_macro_driver);
...
static void __exit wsa_driver_exit(void)
{
   platform_driver_unregister(&wsa_macro_driver);
}

module_init(wsa_driver_init);
module_exit(wsa_driver_exit);
