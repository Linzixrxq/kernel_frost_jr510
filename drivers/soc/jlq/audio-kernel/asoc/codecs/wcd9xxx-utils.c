...
static struct mfd_cell tavil_devs[] = {
	{
		.name = "jlq-wcd-pinctrl",
		.of_compatible = "jlq,wcd-pinctrl",
	},
	{
		.name = "tavil_codec",
	},
					  ...
					  struct wcd9xxx_micbias_setting *mb)
{
	...
	if (of_find_property(dev->of_node, "jlq,cdc-micbias-ldoh-v", NULL)) {
		rc = wcd9xxx_read_of_property_u32(dev,
						  "jlq,cdc-micbias-ldoh-v",
						  &prop_val);
		if (!rc)
	...
	/* MB1 */
	if (of_find_property(dev->of_node, "jlq,cdc-micbias-cfilt1-mv",
			     NULL)) {
		rc = wcd9xxx_read_of_property_u32(dev,
						  "jlq,cdc-micbias-cfilt1-mv",
						   &prop_val);
		if (!rc)
			mb->cfilt1_mv = prop_val;

		rc = wcd9xxx_read_of_property_u32(dev,
						"jlq,cdc-micbias1-cfilt-sel",
						&prop_val);
		if (!rc)
			mb->bias1_cfilt_sel = (u8)prop_val;

	} else if (of_find_property(dev->of_node, "jlq,cdc-micbias1-mv",
				    NULL)) {
		rc = wcd9xxx_read_of_property_u32(dev,
						  "jlq,cdc-micbias1-mv",
						  &prop_val);
	...
	/* MB2 */
	if (of_find_property(dev->of_node, "jlq,cdc-micbias-cfilt2-mv",
			     NULL)) {
		rc = wcd9xxx_read_of_property_u32(dev,
						  "jlq,cdc-micbias-cfilt2-mv",
						   &prop_val);
		if (!rc)
			mb->cfilt2_mv = prop_val;

		rc = wcd9xxx_read_of_property_u32(dev,
						"jlq,cdc-micbias2-cfilt-sel",
						&prop_val);
		if (!rc)
			mb->bias2_cfilt_sel = (u8)prop_val;

	} else if (of_find_property(dev->of_node, "jlq,cdc-micbias2-mv",
				    NULL)) {
		rc = wcd9xxx_read_of_property_u32(dev,
						  "jlq,cdc-micbias2-mv",
						  &prop_val);
	...
	/* MB3 */
	if (of_find_property(dev->of_node, "jlq,cdc-micbias-cfilt3-mv",
			     NULL)) {
		rc = wcd9xxx_read_of_property_u32(dev,
						  "jlq,cdc-micbias-cfilt3-mv",
						   &prop_val);
		if (!rc)
			mb->cfilt3_mv = prop_val;

		rc = wcd9xxx_read_of_property_u32(dev,
						"jlq,cdc-micbias3-cfilt-sel",
						&prop_val);
		if (!rc)
			mb->bias3_cfilt_sel = (u8)prop_val;

	} else if (of_find_property(dev->of_node, "jlq,cdc-micbias3-mv",
				    NULL)) {
		rc = wcd9xxx_read_of_property_u32(dev,
						  "jlq,cdc-micbias3-mv",
						  &prop_val);
	...
	/* MB4 */
	if (of_find_property(dev->of_node, "jlq,cdc-micbias4-cfilt-sel",
			     NULL)) {
		rc = wcd9xxx_read_of_property_u32(dev,
						"jlq,cdc-micbias4-cfilt-sel",
						&prop_val);
		if (!rc)
			mb->bias4_cfilt_sel = (u8)prop_val;

	} else if (of_find_property(dev->of_node, "jlq,cdc-micbias4-mv",
				    NULL)) {
		rc = wcd9xxx_read_of_property_u32(dev,
						  "jlq,cdc-micbias4-mv",
						  &prop_val);
	...
	mb->bias1_cap_mode =
	   (of_property_read_bool(dev->of_node, "jlq,cdc-micbias1-ext-cap") ?
	 MICBIAS_EXT_BYP_CAP : MICBIAS_NO_EXT_BYP_CAP);
	mb->bias2_cap_mode =
	   (of_property_read_bool(dev->of_node, "jlq,cdc-micbias2-ext-cap") ?
	    MICBIAS_EXT_BYP_CAP : MICBIAS_NO_EXT_BYP_CAP);
	mb->bias3_cap_mode =
	   (of_property_read_bool(dev->of_node, "jlq,cdc-micbias3-ext-cap") ?
	    MICBIAS_EXT_BYP_CAP : MICBIAS_NO_EXT_BYP_CAP);
	mb->bias4_cap_mode =
	   (of_property_read_bool(dev->of_node, "jlq,cdc-micbias4-ext-cap") ?
	    MICBIAS_EXT_BYP_CAP : MICBIAS_NO_EXT_BYP_CAP);

	mb->bias2_is_headset_only =
		of_property_read_bool(dev->of_node,
				      "jlq,cdc-micbias2-headset-only");
...
struct wcd9xxx_pdata *wcd9xxx_populate_dt_data(struct device *dev)
{
	...
	pdata->has_micb_supply_en_gpio = of_property_read_bool(dev->of_node,
					   "jlq,has-micbias-supply-en-gpio");
	if (pdata->has_micb_supply_en_gpio) {
		pdata->micb_en_ctl = of_parse_phandle(dev->of_node,
				"jlq,micbias-supply-en-gpio-node", 0);
		if (!pdata->micb_en_ctl) {
			dev_err(dev, "%s No entry for %s property in node %s\n",
				__func__, "jlq,micbias-supply-en-gpio-node",
				dev->of_node->full_name);
			goto err_parse_dt_prop;
		}
	}

	if (!(wcd9xxx_read_of_property_u32(dev, "jlq,cdc-mclk-clk-rate",
					   &prop_val)))
	...
	if (!(wcd9xxx_read_of_property_u32(dev, "jlq,cdc-dmic-sample-rate",
					   &prop_val)))
	...
	pdata->dmic_sample_rate = wcd9xxx_validate_dmic_sample_rate(dev,
							...
							pdata->mclk_rate,
							"audio_dmic_rate");
	if (!(wcd9xxx_read_of_property_u32(dev, "jlq,cdc-mad-dmic-rate",
					   &prop_val)))
	...
	if (of_find_property(dev->of_node, "jlq,cdc-ecpp-dmic-rate", NULL)) {
		rc = wcd9xxx_read_of_property_u32(dev,
						  "jlq,cdc-ecpp-dmic-rate",
						  &prop_val);
		if (!rc)
