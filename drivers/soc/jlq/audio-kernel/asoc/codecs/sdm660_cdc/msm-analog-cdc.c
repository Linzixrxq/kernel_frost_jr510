				...
				struct snd_soc_component *component)
{
	struct sdm660_cdc_priv *sdm660_cdc_priv =
		snd_soc_component_get_drvdata(component);
	const char *prop_name = "jlq,cdc-boost-voltage";
...
{
	const char *prop_name = "jlq,cdc-micbias-cfilt-mv";
	int ret;
						...
						struct device *dev)
{
	...
	const char *static_prop_name = "jlq,cdc-static-supplies";
	const char *ond_prop_name = "jlq,cdc-on-demand-supplies";
...
static const struct of_device_id sdm660_codec_of_match[] = {
	{ .compatible = "jlq,pmic-analog-codec", },
	{},
};
...
static int __init msm_anlg_codec_init(void)
{
    int r = platform_driver_register(&msm_anlg_codec_driver);
...
static void __exit msm_anlg_codec_exit(void)
{
   platform_driver_unregister(&msm_anlg_codec_driver);
}

module_init(msm_anlg_codec_init);
module_exit(msm_anlg_codec_exit);
