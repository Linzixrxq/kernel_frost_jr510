...
static const struct of_device_id msm_dig_cdc_of_match[] = {
	{.compatible = "jlq,msm-digital-codec"},
	{},
};
...
static int __init msm_digcodec_init(void)
{
    int r = platform_driver_register(&msm_digcodec_driver);
...
static void __exit msm_digcodec_exit(void)
{
   platform_driver_unregister(&msm_digcodec_driver);
}

module_init(msm_digcodec_init);
module_exit(msm_digcodec_exit);
