...
#else
	/*During slow plugging, the L_DETECT pin(left ear detect) has contacted the headset, but the HPH_L pin (left ear)
	does not touch the headset within mbhc->swap_thr(4) times of slow plugging!
	As a result, earphone interruption has been generated, but left ear is not touched
	resulting in high left ear feedback level and normal right ear feedback level!
	--- > Judgment leading to European standard / American Standard switching
	*/
	...
	if (hphl_adc_res > mbhc->hphl_cross_conn_thr &&
		...
		pr_debug("%s: Cross connection identified, hphl 0x%x, hphr 0x%x\n", __func__,hphl_adc_res,hphr_adc_res);
	} else {
...

{
	...
	pr_debug("%s: plug type is %d found, 0x%x, 0x%x\n", __func__, plug_type,hph_thr,hs_thr);
...
static void wcd_correct_swch_plug(struct work_struct *work)
{
	...
	enum wcd_mbhc_plug_type plug_type = MBHC_PLUG_TYPE_INVALID;
	enum wcd_mbhc_plug_type old_plug_type = MBHC_PLUG_TYPE_INVALID;
	enum wcd_mbhc_plug_type slow_plug_type = MBHC_PLUG_TYPE_INVALID;
	...
	if (wcd_swch_level_remove(mbhc)) {
		pr_err("%s: Switch level is low111\n", __func__);
		goto exit;
	}
	WCD_MBHC_RSC_LOCK(mbhc);
	/* Mask ADC COMPLETE interrupt */
	...
	while(1)
	{
		...
		output_mv = wcd_measure_adc_continuous(mbhc);
		plug_type = wcd_mbhc_get_plug_from_adc(mbhc, output_mv);
		output_mv_2p = wcd_measure_adc_once(mbhc, MUX_CTL_IN2P);

		pr_debug("%s:  Plug type %d, old plug type %d, output mv 0x%x, 2p once 0x%x\n",
					 __func__, plug_type, old_plug_type, output_mv,output_mv_2p);

		if(plug_type == old_plug_type )
		{
			try++;
			if(try == mbhc->swap_thr )
				break;
		}
		else
		{
	...
	*/
    ...
    pr_debug("%s: curr plug 0x%x, plug type 0x%x\n",__func__,mbhc->current_plug,plug_type);
...
correct_plug_type:
	...
	while (!time_after(jiffies, timeout)) {
		...
		pr_info("%s: plug type 0x%x, hs_thr 0x%x, cnt 0x%x, mv 0x%x  \n",__func__,plug_type,hs_threshold,spl_hs_count,output_mv);
...
report:
	...
	/*
	Handle the problem of Android clearing events caused by two consecutive events
	reported in slow plug (headphone < - > headset)
	*/
	...
	if((plug_type == MBHC_PLUG_TYPE_HEADSET ||
		...
		(!wcd_swch_level_remove(mbhc)))
	{
		if((slow_plug_type == MBHC_PLUG_TYPE_HEADSET ||
		 slow_plug_type == MBHC_PLUG_TYPE_HEADPHONE)&&(slow_plug_type != plug_type ))
		{

		pr_debug("%s: plug_type:0x%x, slow plug type 0x%x\n",
				__func__, plug_type,slow_plug_type);

			if(slow_plug_type == MBHC_PLUG_TYPE_HEADPHONE)
...
static irqreturn_t wcd_mbhc_adc_hs_rem_irq(int irq, void *data)
{
	...
	if (mbhc->mbhc_cfg->moisture_en && !moisture_status) {
		...
		pr_err("%s: plug is remove, report unplug\n",__func__);
