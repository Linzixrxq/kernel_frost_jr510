				...
				enum snd_jack_types jack_type)
{
	...
	if (!insertion) {
		/* Report removal */
		mbhc->hph_status &= ~(jack_type|(SND_JACK_LINEOUT));
		/*
...
int wcd_mbhc_start(struct wcd_mbhc *mbhc, struct wcd_mbhc_config *mbhc_cfg)
{
	...
	const char *usb_c_dt = "jlq,msm-mbhc-usbc-audio-supported";
		      ...
		      bool impedance_det_en)
{
	...
	struct snd_soc_card *card = component->card;
	const char *hph_switch = "jlq,msm-mbhc-hphl-swh";
	const char *gnd_switch = "jlq,msm-mbhc-gnd-swh";
	const char *hs_thre = "jlq,msm-mbhc-hs-mic-max-threshold-mv";
	const char *hph_thre = "jlq,msm-mbhc-hs-mic-min-threshold-mv";
	...
	ret = of_property_read_u32_array(card->dev->of_node,
					 "jlq,msm-mbhc-moist-cfg",
					 hph_moist_config, 3);
	if (ret) {
		dev_dbg(card->dev, "%s: no jlq,msm-mbhc-moist-cfg in DT\n",
			__func__);
