...
static const struct of_device_id wcd9xxx_of_match[] = {
	{ .compatible = "jlq,tavil-i2c",
	  .data = (void *)&wcd9xxx_cdc_types[WCD934X]},
	{ .compatible = "jlq,tasha-i2c-pgd",
	  .data = (void *)&wcd9xxx_cdc_types[WCD9335]},
	{ .compatible = "jlq,wcd9xxx-i2c",
						...
						struct slim_device *slim_ifd)
{
	...
	ret = of_property_read_string(dev->of_node, "jlq,cdc-slim-ifd",
				      &slim_ifd->name);
	if (ret) {
		dev_err(dev, "Looking up %s property in node %s failed",
			"jlq,cdc-slim-ifd-dev", dev->of_node->full_name);
		return -ENODEV;
	}
	prop = of_find_property(dev->of_node,
			"jlq,cdc-slim-ifd-elemental-addr", NULL);
	if (!prop) {
		dev_err(dev, "Looking up %s property in node %s failed",
			"jlq,cdc-slim-ifd-elemental-addr",
