...
#include <soc/jlq/ramdump.h>
#include <soc/wcd-dsp-mgr.h>
				    ...
				    int index)
{
	...
	cmpnt->np = np;
	of_property_read_string(np, "jlq,wdsp-cmpnt-dev-name",
				&cmpnt->cdev_name);
done:
	of_node_put(np);
	return cmpnt;
...
static int wdsp_mgr_parse_dt_entries(struct wdsp_mgr_priv *wdsp)
{
	...
	ret = of_property_read_string(dev->of_node, "jlq,img-filename",
				      &wdsp->img_fname);
	if (ret < 0) {
		WDSP_ERR(wdsp, "Reading property %s failed, error = %d",
			 "jlq,img-filename", ret);
		return ret;
	}
