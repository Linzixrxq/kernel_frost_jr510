...
struct rouleur_pdata *rouleur_populate_dt_data(struct device *dev)
{
	...
	pdata->spmi_np = of_parse_phandle(dev->of_node,
					"jlq,pmic-spmi-node", 0);
	if (!pdata->spmi_np) {
		dev_err(dev, "%s: Looking up %s property in node %s failed\n",
				__func__, "jlq,pmic-spmi-node",
				dev->of_node->full_name);
		kfree(pdata);
		return NULL;
	}

	ret = of_property_read_u32(dev->of_node, "jlq,wcd-reset-reg", &reg);
	if (ret) {
	...
	if (of_property_read_u32(dev->of_node, "jlq,foundry-id-reg", &reg))
...
static int rouleur_bind(struct device *dev)
{
	...
	ret = rouleur_parse_port_mapping(dev, "jlq,rx_swr_ch_map", CODEC_RX);
	ret |= rouleur_parse_port_mapping(dev, "jlq,tx_swr_ch_map", CODEC_TX);
...
static const struct of_device_id rouleur_dt_match[] = {
	{ .compatible = "jlq,rouleur-codec" , .data = "rouleur" },
	{}
};
