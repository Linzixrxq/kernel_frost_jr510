...
#define AW87XXX_FW_BIN_ADP_NAME	"aw87xxx_acf_adp.bin"
...
static void aw87xxx_fw_load_init(struct aw87xxx *aw87xxx)
{
	...
	if(aw87xxx->enable_adp)
		snprintf(aw87xxx->fw_name, AW87XXX_FW_NAME_MAX, "%s", AW87XXX_FW_BIN_ADP_NAME);
	else
				...
				struct device_node *dev_node)
{
	...
	//add for workaroud adp mode for jr510_evb
	ret = of_property_read_u32(dev_node, "enable_adp", &enable_adp);
	if (ret < 0) {
		AW_DEV_LOGI(aw87xxx->dev, "enable_adp parse failed, user default[%d], ret=%d",
				enable_adp, ret);
		aw87xxx->enable_adp = enable_adp;
	} else {
		aw87xxx->enable_adp = enable_adp;
		AW_DEV_LOGI(aw87xxx->dev, "parse enable_adp=[%d]",
				aw87xxx->enable_adp);
	}
