...
#include <soc/jlq,audio-ext-clk.h>
...
static int audio_get_pinctrl(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(dev->of_node, "jlq,mclk-clk-reg", &reg);
	if (ret < 0) {
...
static int audio_ref_clk_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(pdev->dev.of_node,
			"jlq,codec-lpass-ext-clk-freq",
			&clk_freq);
	if (!ret)
		clk_priv->clk_cfg.clk_freq_in_hz = clk_freq;

	ret = of_property_read_u32(pdev->dev.of_node,
			"jlq,codec-lpass-clk-id",
			&clk_id);
	if (!ret)
	...
	if (ret)
		dev_dbg(&pdev->dev, "%s: could not find pmic clock names\n",
			__func__);
	/*
	 * property jlq,use-pinctrl to be defined in DTSI to val 1
	 * for clock nodes using pinctrl
	 */
	of_property_read_u32(pdev->dev.of_node, "jlq,use-pinctrl",
			     &use_pinctrl);
