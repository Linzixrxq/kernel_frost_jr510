...
static int msm_pcm_close(struct snd_pcm_substream *substream)
{
	...
	pr_info("instance:%d playback_start:%d capture_start:%d \n",
		     prtd->instance,prtd->playback_start,prtd->capture_start);
...
static int msm_pcm_prepare(struct snd_pcm_substream *substream)
{
	...
	if (prtd->playback_start && prtd->capture_start) {
		session_id = get_session_id(prtd);
		if (session_id){
			voc_open_voice_call(session_id);

			ret = msm_pcm_routing_reg_phy_stream(soc_prtd->dai_link->id,
							0,
							session_id, SNDRV_PCM_STREAM_PLAYBACK);
			ret = msm_pcm_routing_reg_phy_stream(soc_prtd->dai_link->id,
							0,
							session_id, SNDRV_PCM_STREAM_CAPTURE);
			if (ret) {
				pr_debug("%s: connect Rx/Tx routing ret:%d\n", __func__, ret);
			}
...
static int msm_pcm_probe(struct platform_device *pdev)
{
	...
	const char *is_destroy_cvd = "jlq,destroy-cvd";
