...
enum {
    EFFECT_CONFIG_NONE = 0,
    EFFECT_CONFIG_AEC,
    EFFECT_CONFIG_AEC_FOR_XTS,
    EFFECT_CONFIG_NS,
    EFFECT_CONFIG_NS_FOR_XTS,
    EFFECT_CONFIG_MAX
};
...
#define ADM_CMD_ENABLE_EFFECT 0x000100E5

struct adm_cmd_effect_enable {
    struct apr_hdr	hdr;
    u8 effect_type;
    u8 effect_config;
    u8 enable;
...
struct afe_device_config {
	uint16_t channel_num;
	/* config channel number */

	uint16_t bit_width;
	/* config bit width of per sample */

	uint32_t sample_rate;
    /* config sample rate*/

    uint32_t interleave;
    /*1, interleave;0, disinterleave*/
};

struct afe_device_config_t {
	uint8_t channel_num;
	/* config channel number */

	uint8_t sample_bytes;
	/* config bytes of per sample */
...
#define AFE_PORT_CMD_DEVICE_OPEN 0x000100E3

struct afe_cmd_device_open {
	struct apr_hdr hdr;
	uint8_t device_id;
	uint8_t channel_num;
	uint8_t sample_bytes;
...
#define AFE_PORT_CMD_DEVICE_CLOSE  0x000100E4

struct afe_port_cmd_device_close {
	struct apr_hdr hdr;
	uint8_t device_id;
	uint8_t reserved[3];
} __packed;
...
#define ASM_MEDIA_FMT_PCM 0x00013225
...
#define ASM_DEEP_BUFFER_STREAM_SESSION           0

#define ASM_LOW_LATENCY_STREAM_SESSION           1

#define ASM_COMPRESS_OFFLOAD_STREAM_SESSION      2
...
#define ADM_CMD_AUDIO_CAL_UPDATED    0x00002201
struct audiocal_adsp_update{
	struct apr_hdr			hdr;
}__packed;

#define ADM_STREAM_FM_OPEN    0x00004401

struct fm_stream_cmd_open{
	struct apr_hdr			hdr;
	struct asm_pcm_cfg_param cfg_param;
}__packed;


#define ADM_STREAM_FM_RUN    0x00004402

struct fm_stream_cmd_run{
	struct apr_hdr			hdr;
}__packed;

#define ADM_STREAM_FM_CLOSE    0x00004403

struct fm_stream_cmd_close{
	struct apr_hdr			hdr;
}__packed;

#define ADM_STREAM_FM_PAUSE    0x00004405

struct fm_stream_cmd_pause{
	struct apr_hdr			hdr;
}__packed;

#define ADM_IVOLUME_CMD_FM_SET_STEP   0x00004404
#define FM_IVOLUME_RAMP_MS     10

enum{
	FM_IVOLUME_DIRECTION_TX = 0,
	FM_IVOLUME_DIRECTION_RX = 1,
};

struct volume_cmd_step_t {
	uint16_t direction;
	uint16_t ramp_duration_ms;
	uint32_t value;
} __packed;

struct fm_volume_set_step_cmd {
	struct apr_hdr hdr;
	struct volume_cmd_step_t vol_step;
} __packed;
...
#define ASM_VT_OPEN                  0x00003300
#define ASM_VT_HW_CONFIG             0x00003301
#define ASM_VT_READ                  0x00003302
#define ASM_VT_READ_DONE             0x00003303
#define ASM_VT_EVENT                 0x00003304
#define ASM_VT_CLOSE                 0x00003305
#define ASM_VT_SET_MODE              0x00003306
#define ASM_VT_RUN                   0x00003307
#define ASM_VT_PAUSE                 0x00003308

#define VT_IN_AP                     0
#define VT_IN_ADSP                   1

#define CLOSE_VOICE_WAKEUP_PORT      1
#define OPEN_VOICE_WAKEUP_PORT       2
#define CLOSE_VOICE_RECOGNITION_PORT 4
#define OPEN_VOICE_RECOGNITION_PORT  8

enum{
    ASM_VT_RECOGNITION = 0,
	ASM_VT_WAKEUP = 1
};
...
enum {
	JLQ_FRONTEND_DAI_TX_FM = 0,
	JLQ_FRONTEND_DAI_TX_VOICE_UPLINK,
	JLQ_FRONTEND_DAI_TX_AUDIO_RECORD,
	JLQ_FRONTEND_DAI_TX_AUDIO_RECORD_EXT1,
	JLQ_FRONTEND_DAI_TX_AUDIO_RECORD_EXT2,
	JLQ_FRONTEND_DAI_TX_VOICE_RECORD,
	JLQ_FRONTEND_DAI_TX_VT,
	JLQ_FRONTEND_DAI_TX_FM_RECORED,//unuse, just for match adsp define
	JLQ_FRONTEND_DAI_TX_VOIP_UPLINK,
	JLQ_FRONTEND_DAI_TX_LOWLATENCY_RECORD = JLQ_FRONTEND_DAI_TX_AUDIO_RECORD_EXT2,
	JLQ_FRONTEND_DAI_RX_DEEPBUFFER = 0x80,
	JLQ_FRONTEND_DAI_RX_LOWLATENCY,
	JLQ_FRONTEND_DAI_RX_FM,
	JLQ_FRONTEND_DAI_RX_VOICE_DOWNLINK,
	JLQ_FRONTEND_DAI_RX_VOIP_DOWNLINK,
	JLQ_FRONTEND_DAI_RX_INCALL_MUSIC,
	JLQ_FRONTEND_DAI_MAX
};

enum {
	JLQ_BACKEND_DAI_TX_PRIMARY_MIC= 0,
	JLQ_BACKEND_DAI_TX_SECONDARY_MIC,
	JLQ_BACKEND_DAI_TX_DUAL_MIC,
	JLQ_BACKEND_DAI_TX_HEADSET_MIC,
	JLQ_BACKEND_DAI_TX_VA_MIC,
	JLQ_BACKEND_DAI_TX_SCO_UPLINK,
	JLQ_BACKEND_DAI_TX_FM,
	JLQ_BACKEND_DAI_TX_VOICE_RECORD,
	JLQ_BACKEND_DAI_TX_FM_RECORD,
	JLQ_BACKEND_DAI_RX_HANDSET = 0x80,
	JLQ_BACKEND_DAI_RX_HEADSET,
	JLQ_BACKEND_DAI_RX_SPEAKER,
	JLQ_BACKEND_DAI_RX_SPEAKER_WSA,
	JLQ_BACKEND_DAI_RX_SCO_DOWNLINK,
	JLQ_BACKEND_DAI_RX_HEADPHONE,
	JLQ_BACKEND_DAI_RX_INCALL_MUSIC,
	JLQ_BACKEND_DAI_MAX,
};

#define VSS_IMVM_CMD_OPEN_VOICE			0x10011193
