...
static int lpi_pinctrl_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32_array(dev->of_node, "jlq,lpi-offset-tbl",
					 lpi_offset, npins);
	if (ret < 0) {
	...
	slew_reg = 0;
	ret = of_property_read_u32(dev->of_node, "jlq,slew-reg", &slew_reg);
	if (!ret) {
...
static const struct of_device_id lpi_pinctrl_of_match[] = {
	{ .compatible = "jlq,lpi-pinctrl" }, /* Generic */
	{ },
};
...
{
    int r = platform_driver_register(&lpi_pinctrl_driver);
...
static void __exit lpi_pinctrl_exit(void)
{
   platform_driver_unregister(&lpi_pinctrl_driver);
}

module_init(lpi_pinctrl_init);
module_exit(lpi_pinctrl_exit);
