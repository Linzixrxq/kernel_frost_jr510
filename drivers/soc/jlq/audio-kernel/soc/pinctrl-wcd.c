...
static int wcd_pinctrl_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(dev->of_node, "jlq,num-gpios", &npins);
	if (ret) {
		dev_err(dev, "%s: Looking up %s property in node %s failed\n",
			__func__, "jlq,num-gpios", dev->of_node->full_name);
		ret = -EINVAL;
		goto err_priv_alloc;
	}
	if (!npins) {
...
static int __init wcd_pinctrl_init(void)
{
    int r = platform_driver_register(&wcd_pinctrl_driver);
...
static void __exit wcd_pinctrl_exit(void)
{
   platform_driver_unregister(&wcd_pinctrl_driver);
}

module_init(wcd_pinctrl_init);
module_exit(wcd_pinctrl_exit);

MODULE_DESCRIPTION("Inc WCD GPIO pin control driver");
