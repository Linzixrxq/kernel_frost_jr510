...
//used for debug; when i find the reason,i will delete it;
#define SWM_TEMP_DEBUG
...
char __iomem *swr_top_base;
char __iomem *swr_va_base;
char __iomem *swr_rx_base;
static int swr_err_cnt = 0;
static int swrm_wakeup_flag = 0;
static u32 old_temp=0;
static int dev1_swrm_port_en = 0;
static int dev2_swrm_port_en = 0;
static int swrm_port_en = 0;
static struct device *pdev1 =NULL;
static struct device *pdev2 =NULL;
static int need_pdw_up = 0;
				...
				int core_type, bool enable)
{
	...
	mutex_unlock(&swrm->devlock);
	dev_dbg(swrm->dev, "%s: hw_clk_en: %d audio_core_clk_en: %d, type 0x%x,0x610 0x%x, 0x144 0x%x\n",
		__func__, swrm->hw_core_clk_en, swrm->aud_core_clk_en,core_type,readl(swr_top_base + 0x610),readl(swr_top_base + 0x144));
...
static bool swrm_check_link_status(struct swr_mstr_ctrl *swrm, bool active)
{
	...
	if (retry == 0)
		dev_info(swrm->dev, "%s: link status not %s\n", __func__,
...
retry_read:
	*cmd_data = swr_master_read(swrm, SWRM_CMD_FIFO_RD_FIFO_ADDR);
	//dev_dbg(swrm->dev, "%s:swr mstl 0x318 reg: 0x%x, cmd_id: 0x%x, rcmd_id: 0x%x, \
		     ...
		     void *buf, u32 len)
{
	...
	if (!dev_num) {
		dev_err(&master->dev, "%s: invalid slave dev num, 0x1090 0x%x\n", __func__,swr_master_read(swrm, SWRM_MCP_SLV_STATUS));
		return -EINVAL;
	}
	mutex_lock(&swrm->devlock);
	if (!swrm->dev_up) {
	...
	for (i = 0; i < SWR_MSTR_PORT_LEN ; i++) {
		...
		list_for_each_entry(port_req, &mport->port_req_list, list) {
			...
			SWRS_DP_CHANNEL_ENABLE_BANK(port_req->slave_port_id,
					bank));
			dev_dbg(swrm->dev, "%s: mastr mport :%d, reg: 0x%x, bank 0x%x\n",
				__func__, i,
		...
		swr_master_write(swrm,
				SWRM_DP_PORT_CTRL_BANK(i+1, bank),
				value);
		dev_dbg(swrm->dev, "%s: master mport :%d, reg: 0x%x, val: 0x%x\n",
			__func__, i,
...
static void swrm_copy_data_port_config(struct swr_master *master, u8 bank)
{
	...
	for (i = 0; i < SWR_MSTR_PORT_LEN; i++) {
		...
		list_for_each_entry(port_req, &mport->port_req_list, list) {
			...
			//pr_debug("%s: slv id 0x%x, 0x%x, 0x%x, 0x%x\n",__func__,slv_id,port_req->req_ch,port_req->dev_num,bank);
           ...
           //dev_dbg(swrm->dev, "%s: mstr ch_en :%d, reg: 0x%x \n",
			//__func__, port_req->req_ch,
			//SWRS_DP_CHANNEL_ENABLE_BANK(slv_id,bank));
            ...
            //dev_dbg(swrm->dev, "%s: mstr sinterval :%d, reg: 0x%x \n",
			//__func__, mport->sinterval,
			//SWRS_DP_SAMPLE_CONTROL_1_BANK(slv_id,
			//					bank));
			...
			val[len++] = SWR_REG_VAL_PACK(mport->offset1,
					...
					SWRS_DP_OFFSET_CONTROL_1_BANK(slv_id,
								bank));
            //dev_dbg(swrm->dev, "%s: mstr offset1 :%d, reg: 0x%x \n",
			//__func__, mport->offset1,
			//SWRS_DP_OFFSET_CONTROL_1_BANK(slv_id,
			//					bank));
			...
			if (mport->offset2 != SWR_INVALID_PARAM) {
				...
				val[len++] = SWR_REG_VAL_PACK(mport->offset2,
						...
						SWRS_DP_OFFSET_CONTROL_2_BANK(
							slv_id, bank));
            //dev_dbg(swrm->dev, "%s: mstr offset2 :%d, reg: 0x%x \n",
			//__func__, mport->offset2,
			//SWRS_DP_OFFSET_CONTROL_2_BANK(
			//				slv_id, bank));
			}
            ...
            //dev_dbg(swrm->dev, "%s: mstr hparams :%d, reg: 0x%x \n",
			//__func__, hparams,
			//SWRS_DP_HCONTROL_BANK(slv_id,
			//						bank));
			}
            ...
            //dev_dbg(swrm->dev, "%s: mstr word_length :%d, reg: 0x%x \n",
			//__func__, mport->word_length,
			//SWRS_DP_BLOCK_CONTROL_1(slv_id));
			...
			if (mport->blk_pack_mode != SWR_INVALID_PARAM
				...
				val[len++] =
					...
					SWRS_DP_BLOCK_CONTROL_3_BANK(slv_id,
									bank));
			//dev_dbg(swrm->dev, "%s: mstr blk_pack_mode :%d, reg: 0x%x \n",
			//__func__, mport->blk_pack_mode,
			//SWRS_DP_BLOCK_CONTROL_3_BANK(slv_id,
			//						bank));
				...
				val[len++] =
					 SWR_REG_VAL_PACK(mport->blk_grp_count,
						...
						SWRS_DP_BLOCK_CONTROL_2_BANK(slv_id,
									bank));
            //dev_dbg(swrm->dev, "%s: mstr blk_grp_count :%d, reg: 0x%x \n",
			//__func__, mport->blk_grp_count,
			//SWRS_DP_BLOCK_CONTROL_2_BANK(slv_id,
			//						bank));
				...
				val[len++] =
					SWR_REG_VAL_PACK(mport->lane_ctrl,
						...
						SWRS_DP_LANE_CONTROL_BANK(slv_id,
									bank));
            dev_dbg(swrm->dev, "%s: mstr lane_ctrl :%d, reg: 0x%x \n",
			__func__, mport->lane_ctrl,
			SWRS_DP_LANE_CONTROL_BANK(slv_id,
									bank));
			}
			port_req->ch_en = port_req->req_ch;
            pr_debug("%s: mstr ch en 0x%x\n",__func__,port_req->ch_en );
		}
		value = ((mport->req_ch)
				<< SWRM_DP_PORT_CTRL_EN_CHAN_SHFT);
		...
		val[len++] = value;
		//dev_dbg(swrm->dev, "%s: mstr mport :%d, reg: 0x%x, val: 0x%x, bank 0x%x\n",
			//__func__, i,
			//(SWRM_DP_PORT_CTRL_BANK((i + 1), bank)), value, bank);
		...
		if (mport->lane_ctrl != SWR_INVALID_PARAM) {
            ...
            dev_dbg(swrm->dev, "%s: mstr lane_ctrl :%d, reg: 0x%x\n",
			__func__, mport->lane_ctrl,
			(SWRM_DP_PORT_CTRL_2_BANK((i + 1), bank)));
		}
		if (mport->word_length != SWR_INVALID_PARAM) {
		...
		if (mport->blk_grp_count != SWR_INVALID_PARAM) {
            ...
            //dev_dbg(swrm->dev, "%s: mstr blk_grp_count :%d, reg: 0x%x\n",
			//__func__, mport->blk_grp_count,
			//SWRM_DP_BLOCK_CTRL2_BANK((i + 1), bank));
		}
		if (mport->hstart != SWR_INVALID_PARAM
				&& mport->hstop != SWR_INVALID_PARAM) {
...
static int swrm_slvdev_datapath_control(struct swr_master *master, bool enable)
{
	...
	if (enable) {
		if (!test_bit(ENABLE_PENDING, &swrm->port_req_pending)) {
			dev_dbg(swrm->dev, "%s:mstr No pending connect port req, req pending 0x%x\n",
				__func__,swrm->port_req_pending);
			goto exit;
		}
		...
		if (!test_bit(DISABLE_PENDING, &swrm->port_req_pending)) {
			dev_dbg(swrm->dev, "%s:else No pending disconn port req\n",
				__func__);
			goto exit;
		}
		clear_bit(DISABLE_PENDING, &swrm->port_req_pending);
...
#ifdef SWM_TEMP_DEBUG
	if(master->tx_swr == 1)
	{
	writel(0x09, swrm->swrm_dig_base + 0x105c);
	pr_info("%s: tx swr base offset 0x105c 0x%x, swr base 0x%x\n",__func__, readl(swrm->swrm_dig_base + 0x105c),swrm->swrm_dig_base);

	writel(0x00170009, swrm->swrm_dig_base + 0x105c);

	writel(0x0F000003, swrm->swrm_dig_base + 0x1164);

	writel(0x01, swrm->swrm_dig_base + 0x1168);

	writel(0xF0, swrm->swrm_dig_base + 0x1174);

#if 0
	writel(0x011E2005, swrm->swrm_dig_base + 0x300);
	writel(0x011E2005, swrm->swrm_dig_base + 0x304);

	if((readl(swrm->swrm_dig_base+0x30c)&0xf0000) == 0x0){
		while(fifo_retry_times){
			usleep_range(1000, 1050);
			if((readl(swrm->swrm_dig_base+0x30c)&0xf0000) != 0x0)
				break;
			fifo_retry_times--;
			if(fifo_retry_times == 0){
				pr_err("%s: swrm offset 0x30c, 0x%x\n",__func__, readl(swrm->swrm_dig_base+0x30c));
			}
		}
	}

	pr_info("%s: tx swr base offset 0x2005 0x%x\n",__func__, readl(swrm->swrm_dig_base + 0x318));
#endif
	swrm_cmd_fifo_wr_cmd(swrm, 0x01, 1, 0x0, 0x2005);
	swrm_cmd_fifo_rd_cmd(swrm, &val, 1, 0, 0x2005, 0x1);
	pr_info("%s: tx swr base offset 0x2005 0x%x\n",__func__, val);

	//writel(0x0610345B, swrm->swrm_dig_base + 0x300);
	swrm_cmd_fifo_wr_cmd(swrm, 0x06, 1, 0x0, WCD937X_DIGITAL_CDC_DMIC_CTL);

	//writel(0x011100F0, swrm->swrm_dig_base + 0x300);
	swrm_cmd_fifo_wr_cmd(swrm, 0x01, 1, 0x0, 0x00f0);

	//writel(0x01120130, swrm->swrm_dig_base + 0x300);
	swrm_cmd_fifo_wr_cmd(swrm, 0x01, 1, 0x0, 0x0130);

	//writel(0x03130132, swrm->swrm_dig_base + 0x300);
	swrm_cmd_fifo_wr_cmd(swrm, 0x03, 1, 0x0, 0x0132);

	//writel(0x01140138, swrm->swrm_dig_base + 0x300);
	swrm_cmd_fifo_wr_cmd(swrm, 0x01, 1, 0x0, 0x0138);

	//other porrt
	//writel(0x03150230, swrm->swrm_dig_base + 0x300);
	swrm_cmd_fifo_wr_cmd(swrm, 0x03, 1, 0x0, 0x0230);

	//writel(0x03160232, swrm->swrm_dig_base + 0x300);
	swrm_cmd_fifo_wr_cmd(swrm, 0x03, 1, 0x0, 0x0232);

	//writel(0x01170234, swrm->swrm_dig_base + 0x300);
	swrm_cmd_fifo_wr_cmd(swrm, 0x01, 1, 0x0, 0x0234);

	//writel(0x01180238, swrm->swrm_dig_base + 0x300);
	swrm_cmd_fifo_wr_cmd(swrm, 0x01, 1, 0x0, 0x0238);

	//writel(0x04190330, swrm->swrm_dig_base + 0x300);
	swrm_cmd_fifo_wr_cmd(swrm, 0x04, 1, 0x0, 0x0330);

	//writel(0x031A0332, swrm->swrm_dig_base + 0x300);
	swrm_cmd_fifo_wr_cmd(swrm, 0x03, 1, 0x0, 0x0332);

	//writel(0x021B0334, swrm->swrm_dig_base + 0x300);
	swrm_cmd_fifo_wr_cmd(swrm, 0x02, 1, 0x0, 0x0334);

	//writel(0x09FC0070, swrm->swrm_dig_base + 0x300);
	swrm_cmd_fifo_wr_cmd(swrm, 0x09, 0xf, 0xc, 0x0070);
			...
			struct swr_params *portinfo)
{
	...
	pr_debug("%s: swr 0x%x disconnect done\n",__func__,swrm);
...
static void swrm_enable_slave_irq(struct swr_mstr_ctrl *swrm)
{
	...
	//dev_err(swrm->dev, "%s: slave status: 0x%x\n", __func__, status);
...
static irqreturn_t swr_mstr_interrupt_v2(int irq, void *dev)
{
	...
	dev_dbg(swrm->dev, "%s: status_mask: 0x%x, status 0x%x \n", __func__, intr_sts_masked, intr_sts);
	trace_printk("%s: status: 0x%x \n", __func__, intr_sts_masked);
handle_irq:
	for (i = 0; i < SWRM_INTERRUPT_MAX; i++) {
		value = intr_sts_masked & (1 << i);
		...
		case SWRM_INTERRUPT_STATUS_SLAVE_PEND_IRQ:
		    ...
		    swrm_cmd_fifo_rd_cmd(swrm, &temp, 1, 0x0, WCD937X_DIGITAL_INTR_STATUS_0, 1);
		    dev_info(swrm->dev,"%s: Trigger irq to slave device, tx swr base offset 0x46e 0x%x,0x200 0x%x\n",__func__, temp, intr_sts);
		    old_temp = temp;
			...
			if (ret) {
				dev_err_ratelimited(swrm->dev,
				   "%s: no slave alert found.spurious interrupt, status 0x%x\n",
					__func__,status);
				break;
			}
			swrm_cmd_fifo_rd_cmd(swrm, &temp, devnum, 0x0,
			...
			swr_err_cnt = 0;
		    swrm_cmd_fifo_wr_cmd(swrm, 0x0, 1, 0x0,       // trigger interrupt again
						WCD937X_DIGITAL_INTR_CLEAR_0);
		    swrm_cmd_fifo_wr_cmd(swrm, 0x0, 1, 0x0,
						WCD937X_DIGITAL_INTR_CLEAR_1);
		    swrm_cmd_fifo_wr_cmd(swrm, 0x0, 1, 0x0,
						WCD937X_DIGITAL_INTR_CLEAR_2);

		    swrm_cmd_fifo_rd_cmd(swrm, &temp1, 1, 0x0, WCD937X_DIGITAL_INTR_CLEAR_0, 1);
		    pr_info("%s: WCD937X_DIGITAL_INTR_CLEAR_0 status 0x%x\n", __func__,temp1);
		...
		case SWRM_INTERRUPT_STATUS_CLK_STOP_FINISHED_V2:
			if (swrm->state == SWR_MSTR_UP)
				...
				pr_debug("%s: SWR wokeup during clock stop\n",
					__func__);
	...
	if((swr_err_cnt == 2) &&(swrm_wakeup_flag ==1))
	{
		swrm_cmd_fifo_rd_cmd(swrm, &temp, 1, 0x0, WCD937X_DIGITAL_INTR_STATUS_0, 1);
		dev_dbg(swrm->dev, "%s: clear abnormal wakeup interrupt, temp 0x%x, old 0x%x\n", __func__,temp, old_temp);

		if((temp&0xff) == 0)
			goto wcd_no_new_intr;             //no new interrupt from wcd

		swrm_cmd_fifo_rd_cmd(swrm, &temp1, 1, 0x0,
					SWRS_SCP_INT_STATUS_CLEAR_1, 1);
		swrm_cmd_fifo_wr_cmd(swrm, 0x4, 1, 0x0,
					SWRS_SCP_INT_STATUS_CLEAR_1);
		swrm_cmd_fifo_wr_cmd(swrm, 0x0, 1, 0x0,
					SWRS_SCP_INT_STATUS_CLEAR_1);

		list_for_each_entry(swr_dev, &mstr->devices, dev_list) {
			if (swr_dev->dev_num != 1)
				continue;

			if (swr_dev->slave_irq) {
				do {
					pr_info("%s: abnormal interrupt handle  ",__func__);
					swr_dev->slave_irq_pending = 0;
					handle_nested_irq(
						irq_find_mapping(
						swr_dev->slave_irq, 0));
				...
				swrm_enable_slave_irq(swrm);  //enable host IRQ
				swrm_cmd_fifo_wr_cmd(swrm, 0x0, 1, 0x0, WCD937X_DIGITAL_INTR_CLEAR_0);// trigger interrupt again
				swrm_cmd_fifo_rd_cmd(swrm, &temp1, 1, 0x0, WCD937X_DIGITAL_INTR_STATUS_0, 1);
				dev_dbg(swrm->dev, "%s: new wcd status 0x%x\n", __func__,temp1);
			}
		}

wcd_no_new_intr:

		old_temp = temp;    //update status
		swr_err_cnt = 0;
		swrm_wakeup_flag = 0;
	}
...
static irqreturn_t swrm_wakeup_interrupt(int irq, void *dev)
{
	...
	pr_info("%s enter, wake interrupt \n", __func__);
	mutex_lock(&swrm->devlock);
	swrm_wakeup_flag = 1;
				...
				u8 *dev_num)
{
	...
	if (swrm_request_hw_vote(swrm, LPASS_HW_CORE, true)) {
	    dev_err(swrm->dev, "%s: request hw vote fail\n",__func__);
	    hw_core_err = true;
	}
	if (swrm_request_hw_vote(swrm, LPASS_AUDIO_CORE, true)) {
	    dev_err(swrm->dev, "%s: request audio vote fail\n",__func__);
	    aud_core_err = true;
	}
	ret = swrm_clk_request(swrm, true);
	if (ret) {
	    dev_err(swrm->dev, "%s: swrm clk failed\n", __func__);
	    ret = IRQ_NONE;
	    goto err_swr_audio_core_vote;
	}

	if(swrm->state == SWR_MSTR_SSR)
	{
		pr_err("%s: swrm state 0x%x\n",__func__,swrm->state);
	...
	while(1)
	{
		ret = swr_master_read(swrm, SWRM_COMP_STATUS)& 0x03;
		if(ret == 1)
			break;
		...
		if(cnt == 20)
		{
			pr_err("%s: SWRM_COMP_STATUS 0x%x\n",__func__,swr_master_read(swrm, SWRM_COMP_STATUS));
			usleep_range(100, 105);
			break;
		}
	}
...
static int swrm_master_init(struct swr_mstr_ctrl *swrm)
{
	...
	swr_master_write(swrm,SWRM_COMP_SW_RESET,0x1);
    swr_master_write(swrm,SWRM_MCP_BUS_CTRL_ADDR,0x1);
    swr_master_write(swrm,SWRM_MCP_BUS_CTRL_ADDR,0x0);

    dev_dbg(swrm->dev, "%s: read SWRM_COMP_SW_RESET : %d, bus reset %d\n", __func__, swr_master_read(swrm, SWRM_COMP_SW_RESET),swr_master_read(swrm, SWRM_MCP_BUS_CTRL_ADDR));
	...
	if (!swrm_check_link_status(swrm, 0x1)) {
		...
		for (i = 0; i < len; i++) {
			usleep_range(50, 55);
			dev_err(swrm->dev,
				"%s:master xxx reg:0x%x val:0x%x\n",
				__func__,
...
static int swrm_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(pdev->dev.of_node, "jlq,swr_master_id",
				&swrm->master_id);
	if (ret) {
	...
	if (!(of_property_read_u32(pdev->dev.of_node,
			"jlq,swrm-hctl-reg", &swrm_hctl_reg)))
	...
	if (of_property_read_u32(pdev->dev.of_node,
			"jlq,swr-clock-stop-mode0",
			&swrm->clk_stop_mode0_supp)) {
		swrm->clk_stop_mode0_supp = FALSE;
	}

	ret = of_property_read_u32(swrm->dev->of_node, "jlq,swr-num-dev",
				   &swrm->num_dev);
	if (ret) {
		dev_dbg(&pdev->dev, "%s: Looking up %s property failed\n",
			__func__, "jlq,swr-num-dev");
	} else {
	...
	ret = of_property_read_u32(pdev->dev.of_node, "jlq,swr-num-ports",
				&num_ports);
	if (ret) {
	...
	if (!of_find_property(pdev->dev.of_node, "jlq,swr-port-mapping",
				&map_size)) {
	...
	ret = of_property_read_u32_array(pdev->dev.of_node,
				"jlq,swr-port-mapping", temp, 3 * map_length);
	if (ret) {
    ...
    //for dbg
    swr_top_base = devm_ioremap(&pdev->dev, 0x34511000, 0x1000);
    if (!swr_top_base){
        dev_err(&pdev->dev, "%s: bolero swr top base failed xxxxxx\n", __func__);
        return -ENOMEM;
    }

    swr_va_base = devm_ioremap(&pdev->dev, 0x34020000, 0x1000);
	if (!swr_va_base) {
		dev_err(&pdev->dev, "%s: swr va ioremap failed xxxxxx\n", __func__);
		return -ENOMEM;
	}

    swr_rx_base = devm_ioremap(&pdev->dev, 0x34040000, 0x1000);
	if (!swr_va_base) {
		dev_err(&pdev->dev, "%s: swr rx ioremap failed xxxxxx\n", __func__);
		return -ENOMEM;
	}
	...
	ret = of_property_read_u32(pdev->dev.of_node,
				   "jlq,swr-mstr-irq-wakeup-capable",
...
static int swrm_runtime_resume(struct device *dev)
{
	...
	dev_dbg(dev, "%s: pm_runtime: resume, state:%d, swrm 0x%x, 0x%x\n",
		__func__, swrm->state,swrm,swrm->dev_up);
	...
	if ((swrm->state == SWR_MSTR_DOWN) ||
		...
		if (!swrm->clk_stop_mode0_supp || swrm->state == SWR_MSTR_SSR) {
			...
			usleep_range(100, 105);
			if (!swrm_check_link_status(swrm, 0x1))
				dev_err(dev, "%s:failed in connecting, ssr?\n",
					__func__);
...
exit:
	...
	dev_dbg(dev, "%s:0x144 0x%x, 0x610 0x%x \n",__func__,readl(swr_top_base+0x144),readl(swr_top_base+0x610));
	return ret;
}
...
static int swrm_runtime_suspend(struct device *dev)
{
	...
	trace_printk("%s: pm_runtime: suspend state: %d\n",
		__func__, swrm->state);
	dev_dbg(dev, "%s: pm_runtime: suspend state: %d, SWR_MSTR_SSR 0x%x, port en 0x%x, swrm 0x%x\n",
		__func__, swrm->state, SWR_MSTR_SSR,swrm_is_port_en(&swrm->master),swrm);

    //dev_err(dev, "%s:begin 0x144 0x%x, 0x610 0x%x \n",__func__,readl(swr_top_base+0x144),readl(swr_top_base+0x610));
	...
	pr_debug("%s:update dev 0x%x, dev1 0x%x, dev2 0x%x     \n",__func__,&mstr->dev,pdev1,pdev2);
	...
	if ((current_state == SWR_MSTR_UP) ||
		...
		if ((current_state != SWR_MSTR_SSR) &&
			...
			dev_err(dev, "%s ports are enabled\n", __func__);
			trace_printk("%s ports are enabled\n", __func__);
			if(pdev1 == &mstr->dev)
				dev1_swrm_port_en = 1;
			else if (pdev2 == &mstr->dev)
				dev2_swrm_port_en = 1;

			swrm_port_en = 1;//either dev port is enable, do not pd bolero
			dev_info(dev, "%s ports are enabled, 0x%x, 0x%x, 0x%x\n", __func__,dev1_swrm_port_en,dev2_swrm_port_en,swrm_port_en);
			ret = -EBUSY;
			goto exit;
		}
		...
		if((dev1_swrm_port_en == 0)&&(dev2_swrm_port_en == 0))
			swrm_port_en =0;

		dev_info(dev, "%s ports are disabled, 0x%x, 0x%x, 0x%x\n", __func__,dev1_swrm_port_en,dev2_swrm_port_en,swrm_port_en);
		...
		if (!swrm->clk_stop_mode0_supp || swrm->state == SWR_MSTR_SSR) {
			...
			status = swr_master_read(swrm, SWRM_MCP_SLV_STATUS);
			if (!status) {
			    dev_err(swrm->dev, "%s: slaves status is 0x%x\n",__func__, status);
			    goto exit;
			}
			pr_info("%s: swr clock stop!\n",__func__);
		...
		if (!swrm_check_link_status(swrm, 0x0))
			dev_err(dev, "%s:failed in disconnecting, ssr?\n",
				__func__);
...
exit:
	...
	trace_printk("%s: pm_runtime: suspend done state: %d\n",
		__func__, swrm->state);
    dev_dbg(dev,"%s: after 0x144 0x%x, 0x610 0x%x \n",__func__,readl(swr_top_base+0x144),readl(swr_top_base+0x610));
	return ret;
}
#endif /* CONFIG_PM */
...
static int swrm_device_suspend(struct device *dev)
{
	...
	dev_dbg(dev, "%s: swrm state: %d, en 0x%x, dis 0x%x\n", __func__, swrm->state,pm_runtime_enabled(dev),pm_runtime_suspended(dev));
...
int swrm_register_wake_irq(struct swr_mstr_ctrl *swrm)
{
	...
	if (!swrm->ipc_wakeup) {
        ...
        disable_irq_nosync(swrm->wake_irq);
#endif
        pr_info("%s: set wakeup irq wake~~~~~~~~~~~~~~~~0x%x\n",__func__,swrm->wake_irq);
...
int swrm_wcd_notify(struct platform_device *pdev, u32 id, void *data)
{
    ...
    pr_debug("%s: id 0x%x, data 0x%x, map id 0x%x\n",__func__, id, data,SWR_CH_MAP);
	...
	case SWR_REGISTER_WAKE_IRQ:
        pr_debug("%s: SWR_REGISTER_WAKE_IRQ id 0x%x, data 0x%x\n",__func__);
		...
		break;
	case SWR_REGISTER_WAKEUP:   //wake up adsp
        pr_debug("%s: id 0x%x\n",__func__,id);
		msm_aud_evt_blocking_notifier_call_chain(
					SWR_WAKE_IRQ_REGISTER, (void *)swrm);
		break;
				...
				enum swrm_pm_state n)
{
    ...
    //pr_info("%s: enter ,old 0x%x, new 0x%x\n",__func__,o,n);
...
static int swrm_suspend(struct device *dev)
{
	...
	dev_info(dev, "%s: system suspend, state: %d, pm sta 0x%x\n", __func__, swrm->state,swrm->pm_state);
	...
	if (swrm->pm_state == SWRM_PM_SLEEPABLE) {
		...
		if (!(wait_event_timeout(swrm->pm_wq, swrm_pm_cmpxchg(
			...
			dev_err(swrm->dev, "%s: suspend failed state %d, wlock %d\n",
				 __func__, swrm->pm_state,
				 swrm->wlock_holders);
			return -EBUSY;
		} else {
	...
	else if((swrm_port_en == 0)&&(need_pdw_up ==0))      //no port is enabled
	{
...
static int swrm_resume(struct device *dev)
{
	...
	dev_info(dev, "%s: system resume, state: %d, pm state 0x%x\n", __func__, swrm->state,swrm->pm_state);
	if((swrm_port_en == 0)&&(need_pdw_up == 1))	   //no port is enabled
	{
