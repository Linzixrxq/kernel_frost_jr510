...
void clk_common_debug_init(struct clk_hw *hw, struct dentry *dentry)
{
	...
	d = debugfs_create_dir("measure", dentry);
	if (!d) {
		pr_err("%s:%s create debugfs failed\n",
			__func__, clk_hw_get_name(hw));
		return;
	}

	if (rclk->vdd_data.vdd_class)
		debugfs_create_file("clk_rate_max", 0444, d, hw,
				&rate_max_fops);

	if (rclk->ops && rclk->ops->list_rate)
		debugfs_create_file("clk_list_rates", 0444, d, hw,
				&list_rates_fops);

	debugfs_create_file("clk_print_regs", 0444, d, hw,
			&clock_print_hw_fops);

	debugfs_create_file("clk_rate", 0644, d, hw,
			&clock_rate_fops);

	debugfs_create_file("clk_enable", 0644, d, hw,
			&clock_enable_fops);
};
...
void clk_debug_measure_add(struct clk_hw *hw, struct dentry *dentry)
{
	...
	d = debugfs_lookup("measure", dentry);
	if (!d) {
		d = debugfs_create_dir("measure", dentry);
		if (!d) {
			pr_err("%s:%s create debugfs failed\n",
...
static struct gcc_info gcc_info;
void gcc_dump(struct clk_hw *hw, unsigned long rate, unsigned int level)
{
	struct gcc_info *gccinfo = &gcc_info;
	struct clk_regmap *rclk = to_clk_regmap(hw);
	struct clk_vdd_class_data *vdd_data = &rclk->vdd_data;
	struct clk_vdd_class *vdd_class = vdd_data->vdd_class;
	unsigned int max = vdd_class->num_levels - 1;
	unsigned int i;

	for (i = max; i > 0; i--)
		if (vdd_class->level_votes[i])
			break;

	mutex_lock(&gccinfo->mutex);
	if (gccinfo->logbuf)
		ipc_log_string(gccinfo->logbuf,
				"%30s: %8lu khz, [ %1d ]; %8s, [ %1d ]\n",
				clk_hw_get_name(hw), (rate / 1000), level,
				vdd_class->class_name, i);
	mutex_unlock(&gccinfo->mutex);
}
EXPORT_SYMBOL(gcc_dump);

static void gcc_log_start(unsigned int start)
{
	struct gcc_info *gccinfo = &gcc_info;

	mutex_lock(&gccinfo->mutex);
	if (start) {
		gccinfo->logbuf = ipc_log_context_create(GCC_IPC_LOG_PAGES, "gcc", 0);
		if (!gccinfo->logbuf)
			pr_err("create gcc log buf failed\n");
	} else {
		ipc_log_context_destroy(gccinfo->logbuf);
		gccinfo->logbuf = NULL;
	}
	mutex_unlock(&gccinfo->mutex);
...
static ssize_t gcc_debug_write(struct file *filp,
		const char __user *buffer, size_t count, loff_t *ppos)
{
	...
	if (start == gccinfo->start) {
		pr_info("gcc log is already %s\n",
...
static const struct file_operations gcc_debug_fops = {
	.write          = gcc_debug_write,
};

void gcc_ipc_log_init(void)
{
	struct gcc_info *gccinfo = &gcc_info;

	gccinfo->dir = debugfs_create_dir("gcc_debug", NULL);
	if (!gccinfo->dir)
		return;

	if (!debugfs_create_file("log_enable", 0644,
			gccinfo->dir, NULL, &gcc_debug_fops)) {
		debugfs_remove_recursive(gccinfo->dir);
		gccinfo->dir = NULL;
		return;
	}

	mutex_init(&gccinfo->mutex);

	gcc_log_start(1);
}
EXPORT_SYMBOL(gcc_ipc_log_init);
