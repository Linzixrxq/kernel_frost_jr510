			...
			unsigned long new_rate)
{
	...
	gcc_dump(hw, new_rate, new_vdd_level);
			...
			unsigned long cur_rate)
{
	...
	gcc_dump(hw, cur_rate, cur_vdd_level);
...
int clk_prepare_regmap(struct clk_hw *hw)
{
	...
	ret = clk_vote_vdd_level(&rclk->vdd_data, rclk->vdd_data.vdd_level);
	if (ret)
		return ret;

	gcc_dump(hw, rate, rclk->vdd_data.vdd_level);
