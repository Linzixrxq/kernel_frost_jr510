...
static int __maybe_unused clk_smd_rpm_enable_scaling(void)
{
	int ret = 0;
...
DEFINE_CLK_SMD_RPM(bengal, cnoc_clk, cnoc_a_clk, QCOM_SMD_RPM_BUS_CLK, 0);
DEFINE_CLK_SMD_RPM(bengal, snoc_clk, snoc_a_clk, QCOM_SMD_RPM_BUS_CLK, 1);
#if 0
...
static int rpm_smd_clk_probe(struct platform_device *pdev)
{
	...
	if (hw_clk_handoff) {
		for (i = 0; i < desc->num_clks; i++) {
			...
			if (ret) {
				dev_err(&pdev->dev, "probe:clk_smd_rpm_handoff (%d)\n", ret);
				goto err;
			}
		}
	}
	...
	if (ret) {
		dev_err(&pdev->dev, "probe:clk_smd_rpm_enable_scaling (%d)\n", ret);
		goto err;
	}
#endif
	for (i = 0; i < desc->num_clks; i++) {
	...
	ret = devm_of_clk_add_hw_provider(&pdev->dev, qcom_smdrpm_clk_hw_get,
				  (void *)desc);
	if (ret) {
		dev_err(&pdev->dev, "probe:devm_of_clk_add_hw_provider (%d)\n", ret);
		goto err;
	}
	...
	if (is_bengal) {
		...
		/* Hold an active set vote for the cnoc_keepalive_a_clk */
		clk_prepare_enable(bengal_cnoc_a_clk.hw.clk);
		clk_set_rate(bengal_cnoc_a_clk.hw.clk, 19200000);

		/* Hold an active set vote for the snoc_keepalive_a_clk */
		//clk_prepare_enable(bengal_snoc_a_clk.hw.clk);
		//clk_set_rate(bengal_snoc_a_clk.hw.clk, 19200000);
