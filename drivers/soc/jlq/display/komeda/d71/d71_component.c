...
/**
 * d71_format_info_block_height - height in pixels of a block
 ...
 */
static unsigned int d71_format_info_block_height(const struct drm_format_info *info,
					  int plane)
{
			      ...
			      struct komeda_component_state *state)
{
	...
	if (fb->modifier) {
		if ((fb->modifier & AFBC_FORMAT_MOD_BLOCK_SIZE_MASK) ==
			AFBC_FORMAT_MOD_BLOCK_SIZE_32x8 || layer->layer_type ==
			      ...
			      struct komeda_component_state *state)
{
	...
	if (crtc_st->color_mgmt_changed) {
		mask |= IPS_CTRL_IT | IPS_CTRL_FT | IPS_CTRL_RGB;
