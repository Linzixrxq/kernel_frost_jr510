			      ...
			      struct komeda_events *evts)
{
	...
	if (events & KOMEDA_EVENT_FLIP) {
		...
		if (kcrtc->disable_done) {
			...
			DRM_DEBUG("CRTC[%d]: FLIP happen but no pending commit.\n",
...
static void komeda_writeback_queue_job(struct drm_writeback_connector *wb_connector,
		     ...
		     struct drm_crtc_state *old)
{
	...
	if (conn_st && conn_st->writeback_job)
		komeda_writeback_queue_job(&wb_conn->base, conn_st);
...
unsigned long komeda_crtc_get_aclk(struct komeda_crtc_state *kcrtc_st)
{
	...
	return clk_round_rate(mdev->aclk, mdev->aclk_rate);
}
...
komeda_crtc_mode_valid(struct drm_crtc *crtc, const struct drm_display_mode *m)
{
	...
	if (clk_round_rate(mdev->aclk, mdev->aclk_rate) < min_aclk) {
...
static int komeda_crtc_create_ctm_ext_property(struct komeda_crtc *kcrtc)
{
	struct drm_crtc *crtc = &kcrtc->base;
	struct drm_property *prop;
...
static void komeda_crtc_enable_color_mgmt(struct drm_crtc *crtc,
			   ...
			   struct komeda_crtc *kcrtc)
{
	...
	komeda_crtc_enable_color_mgmt(crtc, KOMEDA_COLOR_LUT_SIZE, true, KOMEDA_COLOR_LUT_SIZE);

	err = komeda_crtc_create_protected_mode_property(kcrtc);
	if (err)
		return err;

	err = komeda_crtc_create_ctm_ext_property(kcrtc);
	if (err)
		return err;
