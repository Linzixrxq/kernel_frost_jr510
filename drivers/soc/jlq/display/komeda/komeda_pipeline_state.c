...
komeda_atomic_get_old_private_obj_state(struct drm_atomic_state *state,
...
static struct drm_private_state *
komeda_atomic_get_new_private_obj_state(struct drm_atomic_state *state,
				     struct drm_private_obj *obj)
{
	int i;
			      ...
			      struct drm_atomic_state *state)
{
	...
	priv_st = komeda_atomic_get_old_private_obj_state(state, &pipe->obj);
	if (priv_st)
		return priv_to_pipe_st(priv_st);
	return NULL;
}
			      ...
			      struct drm_atomic_state *state)
{
	...
	priv_st = komeda_atomic_get_new_private_obj_state(state, &pipe->obj);
	if (priv_st)
		return priv_to_pipe_st(priv_st);
	return NULL;
}
			       ...
			       struct drm_atomic_state *state)
{
	...
	priv_st = komeda_atomic_get_new_private_obj_state(state, &c->obj);
	if (priv_st)
		return priv_to_comp_st(priv_st);
	return NULL;
}
			       ...
			       struct drm_atomic_state *state)
{
	...
	priv_st = komeda_atomic_get_old_private_obj_state(state, &c->obj);
	if (priv_st)
		return priv_to_comp_st(priv_st);
	return NULL;
}
...
{
	struct komeda_plane_state *kplane_st = to_kplane_st(plane_st);
		     ...
		     struct drm_display_mode *mode)
{
	...
	if (ad->funcs->mode_set) {
		ret = ad->funcs->mode_set(ad, mode);
		if (ret)
			return ret;
	}
