...
/**
 * komeda_format_info_block_width - width in pixels of block.
 ...
 */
static unsigned int komeda_format_info_block_width(const struct drm_format_info *info,
					 int plane)
{
...
/**
 * komeda_format_info_block_height - height in pixels of a block
 * @info: pixel format info
 * @plane: plane index
 *
 * Returns:
 ...
 */
static unsigned int komeda_format_info_block_height(const struct drm_format_info *info,
					  int plane)
{
	if (!info || plane < 0 || plane >= info->num_planes)
		return 0;
			  ...
			  const struct drm_mode_fb_cmd2 *mode_cmd)
{
	...
	if ((fb->modifier & AFBC_FORMAT_MOD_TILED) && info->is_yuv && !(mode_cmd->flags & DRM_MODE_FB_SECURE))
	{
		if(obj->dma_buf)
		{
			void* vaddr = NULL;
			struct ion_buffer *buffer = obj->dma_buf->priv;
			if (buffer && (buffer->flags & ION_FLAG_CACHED))
				dma_sync_single_for_cpu(fb->dev->dev, to_drm_gem_cma_obj(obj)->paddr, 4,DMA_FROM_DEVICE);
			vaddr = dma_buf_kmap(obj->dma_buf, 0);
			if(vaddr)
			{
				u32 offsetFromHeader =  ((*(unsigned *)vaddr)&0x0FFFF000);
				if(offsetFromHeader != kfb->offset_payload)
				{
					DRM_DEBUG_KMS("Change payload offset from %x to %x \n",kfb->offset_payload,offsetFromHeader);
					kfb->offset_payload = offsetFromHeader;
				}
				dma_buf_kunmap(obj->dma_buf,0,vaddr);
				if (buffer && (buffer->flags & ION_FLAG_CACHED))
					dma_sync_single_for_device(fb->dev->dev, to_drm_gem_cma_obj(obj)->paddr, 4,DMA_FROM_DEVICE);
			}
		}
		else
		{
			DRM_DEBUG_KMS("obj->dma_buf is null  %s \n",__func__);
		}
	}
			       ...
			       const struct drm_mode_fb_cmd2 *mode_cmd)
{
	...
	for (i = 0; i < info->num_planes; i++) {
		...
		block_h = komeda_format_info_block_height(info, i);
			       ...
			       u32 src_x, u32 src_y, u32 src_w, u32 src_h)
{
	...
	const struct drm_format_info *info = fb->format;
	u32 block_w = komeda_format_info_block_width(fb->format, 0);
	u32 block_h = komeda_format_info_block_height(fb->format, 0);
...
komeda_fb_get_pixel_addr(struct komeda_fb *kfb, int x, int y, int plane)
{
	...
	if (!fb->modifier) {
		block_w = komeda_format_info_block_width(fb->format, plane);
