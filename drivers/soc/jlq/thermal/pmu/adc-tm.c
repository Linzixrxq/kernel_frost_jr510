...
static int adc_tm_get_all_iio_channel(struct platform_device *pdev, struct iio_channel **channels)
{
	struct device_node *node = pdev->dev.of_node;
	int ret;
	int i;
	const char *names[QCOM_ADC_TM_MAX];

	ret = of_property_read_string_array(node, "io-channel-names",
			(const char**)names, QCOM_ADC_TM_MAX);
	if (ret < 0)
		return ret;
	for (i = 0; i < ret; i++ ) {
		channels[i] = iio_channel_get(&pdev->dev, names[i]);
		if (IS_ERR(channels[i]))
...
static int adc_tm_probe(struct platform_device *pdev)
{
	...
	struct iio_channel *channels[QCOM_ADC_TM_MAX];
	...
	indio_chan_count = adc_tm_get_all_iio_channel(pdev,
		(struct iio_channel **)channels);
	if (indio_chan_count < 0)
		return indio_chan_count;
//	if (IS_ERR(channels))
