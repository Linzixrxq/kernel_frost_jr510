...
-------------------------------------------------------------------------------
#  @file cam_thermal.c
#  @brief jlq camera thermal and bwlm test driver
#******************************************************************************/
...
static int cam_bwlm_store_iter(struct device *dev, void *buf) {
    ...
    if (therm_dev) {
        ...
        if (temp != therm_dev->temp) {
            struct thermal_zone_device *tzd = NULL;
            therm_dev->temp = temp;
            tzd = thermal_zone_get_zone_by_name("qtang2-camera-therm");
            if (tzd)
...
static struct platform_driver cam_thermal_driver = {
    ...
    .driver = {
        .name = "jlq,qtang2-camera-test-tsens",
