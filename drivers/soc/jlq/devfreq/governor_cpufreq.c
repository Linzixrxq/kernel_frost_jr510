...
static int update_node(struct devfreq_node *node)
{
	...
	if (!df) {
		//pr_err_ratelimited("df ... not ready\n");
		return 0;
	}
...
static void add_policy(struct cpufreq_policy *policy)
{
	...
	if (state[policy->cpu]) {
		state[policy->cpu]->freq = policy->cur * 1000;
		...
		new_state->freq = policy->cur * 1000;
...
static int cpufreq_gov_suspend_cb(struct notifier_block *nfb,
...
static unsigned int cpu_to_dev_freq(struct devfreq *df, unsigned int cpu)
{
	...
	while (map->cpu_hz && map->cpu_hz < cpu_hz)
		map++;
	if (!map->cpu_hz)
		map--;
				...
				struct freq_map *map)
{
	...
	while (map->cpu_hz && cnt < len) {
		cnt += scnprintf(buf + cnt, len - cnt, "%8u\t%11u\n",
				map->cpu_hz, map->target_freq);
		map++;
	}
	if (cnt < len)
					...
					unsigned int event, void *data)
{
	...
	case DEVFREQ_GOV_START:
		...
		pr_info("Enabled dev CPUfreq governor\n");
		break;

	case DEVFREQ_GOV_STOP:

		devfreq_cpufreq_gov_stop(devfreq);
		pr_info("Disabled dev CPUfreq governor\n");
		break;
	}
...
static int devfreq_cpufreq_probe(struct platform_device *pdev)
{
	int ret;
	struct device_node *of_par, *of_child;

	if (cpufreq_cpu_get(0) == NULL) {
		pr_info("cpufreq not ready, defer probe\n");
		return -EPROBE_DEFER;
	}
	...
	pm_notifier(cpufreq_gov_suspend_cb, 0);
	return 0;
}

static int __exit devfreq_cpufreq_remove(struct platform_device *pdev)
{
	int ret, cpu;
...
static const struct of_device_id devfreq_cpufreq_match[] = {
	{.compatible = "jlq,devfreq-cpufreq"},
	{},
};
static struct platform_driver devfreq_cpufreq_driver = {
	.driver = {
		.name   = "jlq-devfreq-cpufreq",
		.owner = THIS_MODULE,
		.of_match_table = devfreq_cpufreq_match,
	},
	.probe          = devfreq_cpufreq_probe,
	.remove         = devfreq_cpufreq_remove,
};

static int __init devfreq_cpufreq_init(void)
{
	platform_driver_register(&devfreq_cpufreq_driver);
	return 0;
}

static void __exit devfreq_cpufreq_exit(void)
{
	platform_driver_unregister(&devfreq_cpufreq_driver);
}

module_init(devfreq_cpufreq_init);
