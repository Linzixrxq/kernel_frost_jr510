...
{
	if (hwmon->get_bw_hwmon_mode(hwmon))
...
static unsigned long hwmon_get_freq(struct hwmon_node *node, unsigned long *freq, unsigned long *ab)
{
	...
	//TBD
	//node->max_mbps = DEFAULT_VALUE /100 * io_percent;
	node->max_mbps *= 5;
	//
	...
	if (meas_mbps > node->hist_max_mbps || !node->hist_mem_cnt) {
		/* If new max or no history */
		node->hist_max_mbps = meas_mbps;
		node->hist_mem_cnt  = node->hist_memory;
	...
	if (node->wake == UP_WAKE) {
		req_mbps += ((meas_mbps - node->prev_req) * node->up_scale) / 100;
		/*
		 ...
		 * historic peak.
		 */
		if (req_mbps > node->hist_max_mbps && meas_mbps < node->hist_max_mbps)
	 ...
	 */
	if (meas_mbps >= hyst_lo_tol && meas_mbps > MIN_MBPS && !node->max_mbps) {
		node->hyst_peak++;
		if (node->hyst_peak >= node->hyst_trigger_count  || node->hyst_en)
	...
	if (meas_mbps < MIN_MBPS) {
		hw->up_wake_mbps   = (max(MIN_MBPS, req_mbps) * (100 + node->up_thres)) / 100;
		hw->down_wake_mbps = 0;
		thres = mbps_to_bytes(max(MIN_MBPS, req_mbps / 2), node->sample_ms);
	} else {
		/*
	...
	if (adj_mbps > node->prev_ab) {
		...
		new_bw = adj_mbps * node->decay_rate + node->prev_ab * (100 - node->decay_rate);
		new_bw /= 100;
	}
...
{
	struct hwmon_node *node = NULL;
	struct hwmon_node *found = NULL;

	mutex_lock(&list_lock);
	list_for_each_entry(node, &hwmon_list, list)
		if (node->hw->of_node == df->dev.parent->of_node) {
			found = node;
			break;
		}
...
static int gov_start_monitor(struct devfreq *df, bool init)
{
...
static void gov_stop_monitor(struct devfreq *df, bool init)
{
	struct hwmon_node *node = df->data;
	struct bw_hwmon *hw = node->hw;
...
static int gov_start(struct devfreq *df)
{
	...
	//Store hw node as devfreq-dev df-data for governor
	hw->df = df;
	node->orig_data = df->data;
	df->data = node;

	ret = gov_start_monitor(df, true);
	if (ret < 0)
		goto err_start;
...
static int gov_resume(struct devfreq *df)
{
	...
	return gov_start_monitor(df, false);
}

static int devfreq_busmon_bw_get_freq(struct devfreq *df, unsigned long *freq)
{
	struct hwmon_node *node = df->data;
	...
	hwmon_get_freq(node, freq, node->dev_ab);

	return 0;
}

static int devfreq_busmon_bw_ev_handler(struct devfreq *df, unsigned int event, void *data)
{
	int ret = 0;
	...
	case DEVFREQ_GOV_START:
		polling_ms = df->profile->polling_ms;
		polling_ms = max(MIN_MS, polling_ms);
		polling_ms = min(MAX_MS, polling_ms);
		df->profile->polling_ms = polling_ms;
		ret = gov_start(df);
		if (ret < 0)
			goto out;
		dev_dbg(df->dev.parent, "Enabled dev BW HW monitor governor\n");
		break;
	...
	case DEVFREQ_GOV_INTERVAL:
		node = df->data;
		polling_ms = *(unsigned int *)data;
		if (polling_ms < node->sample_ms) {
			ret = -EINVAL;
			goto out;
		}
		polling_ms = max(MIN_MS, polling_ms);
		...
		hw = node->hw;
		hw->suspend_hwmon(hw);
		devfreq_interval_update(df, &polling_ms);
		ret = hw->resume_hwmon(hw);
		if (ret < 0) {
			dev_err(df->dev.parent, "Unable to resume HW monitor (%d)\n", ret);
			goto out;
		}
		break;
	...
	case DEVFREQ_GOV_SUSPEND:
		ret = gov_suspend(df);
		if (ret < 0) {
			dev_err(df->dev.parent, "Unable to suspend BW HW mon governor (%d)\n", ret);
			goto out;
		}
	...
	case DEVFREQ_GOV_RESUME:
		ret = gov_resume(df);
		if (ret < 0) {
			dev_err(df->dev.parent, "Unable to resume BW HW mon governor (%d)\n", ret);
			goto out;
		}
...
static struct devfreq_governor devfreq_gov_bw_hwmon = {
	.name = "gov-busmon-bw",
	.immutable = 1,
	.get_target_freq = devfreq_busmon_bw_get_freq,
	.event_handler   = devfreq_busmon_bw_ev_handler,
};
...
int register_busmon_bw(struct device *dev, struct bw_hwmon *hwmon)
{
	...
	if (hwmon->gov) {
		...
		if (!attr_grp)
			return -ENOMEM;
		hwmon->gov->get_target_freq = devfreq_busmon_bw_get_freq;
		hwmon->gov->event_handler   = devfreq_busmon_bw_ev_handler;
	...
	node->use_ab = 1;
	node->mbps_zones[0] = 0;
	node->sample_ms = node->hw->get_bw_hwmon_period(hwmon);
...
EXPORT_SYMBOL(register_busmon_bw);

int unregister_busmon_bw(struct device *dev, struct bw_hwmon *hwmon)
{
	int ret = 0;
	//TBD
	return ret;
}
EXPORT_SYMBOL(unregister_busmon_bw);

MODULE_DESCRIPTION("DDR controller ports freq voting governor based on busmon-bw");
MODULE_LICENSE("GPL");
