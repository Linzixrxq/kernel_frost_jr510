...
module_param(disable_on_suspend, bool, 0644);
...
module_param(detach_debounce_delay_ms, uint, 0644);
...
static const unsigned int pi_extcon_cable[] = {
	EXTCON_USB,
	EXTCON_USB_HOST,
	EXTCON_NONE,
};
...
static int piusb_update_power_supply(struct power_supply *psy, int limit)
{
	...
	if (psy->desc->set_property)
		return psy->desc->set_property(psy,
			POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT, &ret);
...
static irqreturn_t piusb_irq(int irq, void *data)
{
	...
	if ((pi_usb->reg_data.port_status & STS_MODE_MASK) != STS_MODE_AUDIO) {
...
static int piusb_gpio_config(struct pi_usb_type_c *pi, bool enable)
{
	...
	ret = devm_gpio_request(&pi->client->dev, pi->enb_gpio,	"pi_typec_enb_gpio");
	if (ret) {
...
static int piusb_extcon_init(struct pi_usb_type_c *pi)
{
	int rc = 0;

	/* extcon registration */
	pi->edev = devm_extcon_dev_allocate(&pi->client->dev, pi_extcon_cable);
	if (IS_ERR(pi->edev)) {
		rc = PTR_ERR(pi->edev);
		dev_err(&pi->client->dev, "failed to allocate extcon device rc=%d\n", rc);
		return rc;
	}

	rc = devm_extcon_dev_register(&pi->client->dev, pi->edev);
	if (rc < 0)
		dev_err(&pi->client->dev, "failed to register extcon device rc=%d\n", rc);
		...
		struct device_attribute *attr, char *buf)
{
	int rc;
	int data_length = sizeof(pi_usb->reg_data);
	uint16_t saddr = pi_usb->client->addr;
	...
	mutex_lock(&pi_usb->mutex);
	rc = i2c_transfer(pi_usb->client->adapter, msgs, 1);
	if (rc < 0) {
		mutex_unlock(&pi_usb->mutex);
		/* i2c read may fail if device not enabled or not present */
		dev_err(&pi_usb->client->dev, "i2c read from 0x%x failed %d\n",
			saddr, rc);
		pi_usb->attach_state = false;
		return -ENXIO;
	}
	...
	return snprintf(buf, PAGE_SIZE, "0x%x regs:%x, %x, %x, %x\n", saddr,
		    pi_usb->reg_data.dev_id, pi_usb->reg_data.control,
		    pi_usb->reg_data.intr_status, pi_usb->reg_data.port_status);
}

static DEVICE_ATTR_RO(register_data);

static struct attribute *PI5USB30216C_attr[] = {
	&dev_attr_register_data.attr,
	NULL
};

static const struct attribute_group PI5USB30216C_attr_group = {
	.attrs = PI5USB30216C_attr,
};
...
static int piusb_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
...
#endif
	pi_usb->irq_gpio = of_get_named_gpio(np, "qpio-irq", 0);
	if (pi_usb->irq_gpio < 0) {
		dev_err(&i2c->dev, "no irq gpio\n");
		ret = -EINVAL;
		goto out;
	}
	pi_usb->irq = gpio_to_irq(pi_usb->irq_gpio);
	...
	ret = piusb_extcon_init(pi_usb);
	if (ret)
		goto i2c_disable;

	mutex_init(&pi_usb->mutex);
	/* Update initial state to USB */
	piusb_irq(pi_usb->irq, pi_usb);

	ret = devm_request_threaded_irq(&i2c->dev, pi_usb->irq, NULL, piusb_irq,
	...
	enable_irq_wake(pi_usb->irq);

	INIT_DELAYED_WORK(&pi_usb->handle_detach_work,
				  piusb_handle_detach_work);

	ret = sysfs_create_group(&i2c->dev.kobj, &PI5USB30216C_attr_group);
	if (ret) {
		dev_err(&i2c->dev, "sysfs create group failed\n");
		goto i2c_disable;
	}
...
static int piusb_i2c_suspend(struct device *dev)
{
	...
	if (disable_on_suspend && gpio_is_valid(pi->enb_gpio))
		gpio_set_value(pi->enb_gpio, !pi->enb_gpio_polarity);
...
static int __init piusb_driver_init(void)
{
	return i2c_add_driver(&piusb_driver);
}

static void __exit piusb_driver_exit(void)
{
	i2c_del_driver(&piusb_driver);
}
module_init(piusb_driver_init);
module_exit(piusb_driver_exit);
