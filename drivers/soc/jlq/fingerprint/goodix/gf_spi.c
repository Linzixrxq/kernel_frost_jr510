...
static int check_hwid() {
	if (fp_sensor_vendor != VENDOR_GOODIX) {
		return -1;
	}
...
{
	if (check_hwid() < 0) {
		pr_warn("gf_init: The FP sensor vendor is not goodix.\n");
		return 0;
	}
...
{
	if (check_hwid() < 0) {
		pr_warn("gf_exit: The FP sensor vendor is not goodix.\n");
		return;
	}
