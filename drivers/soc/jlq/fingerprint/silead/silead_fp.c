...
static
#endif
fp_debug_level_t sil_debug_level = DBG_LOG; // ALL_LOG
...
static int check_hwid() {
	if (fp_sensor_vendor != VENDOR_SILEAD) {
		return -1;
	}
...
{
	if (check_hwid() < 0) {
		pr_warn("silfp_dev_init: The FP sensor vendor is not silead.\n");
		return 0;
	}
