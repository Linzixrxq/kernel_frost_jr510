 ...
 */
static void jcmvote_set_any(struct jcmvotable *votable, int client_id,
 ...
 *	Must be called with the votable->lock held
 */
static void jcmvote_min(struct jcmvotable *votable, int client_id,
				int *eff_res, int *eff_id)
{
	int i;
 ...
 *	Must be called with the votable->lock held
 */
static void jcmvote_max(struct jcmvotable *votable, int client_id,
				int *eff_res, int *eff_id)
{
	int i;
...
static int get_client_id(struct jcmvotable *votable, const char *client_str)
{
	int i;
...
static char *get_client_str(struct jcmvotable *votable, int client_id)
{
...
{
	mutex_lock(&votable->jcmvote_lock);
}

void unlock_jcmvotable(struct jcmvotable *votable)
{
	mutex_unlock(&votable->jcmvote_lock);
}

/**
 * is_override_jcmvote_enabled() -
 * is_override_jcmvote_enabled_locked() -
...
bool is_override_jcmvote_enabled(struct jcmvotable *votable)
{
	...
	lock_jcmvotable(votable);
	enable = is_override_jcmvote_enabled_locked(votable);
	unlock_jcmvotable(votable);

	return enable;
}

/**
 * is_client_jcmvote_enabled() -
 * is_client_jcmvote_enabled_locked() -
...
bool is_client_jcmvote_enabled(struct jcmvotable *votable, const char *client_str)
{
	...
	lock_jcmvotable(votable);
	enabled = is_client_jcmvote_enabled_locked(votable, client_str);
	unlock_jcmvotable(votable);
	return enabled;
}

/**
 * jcmvote_get_client_vote() -
 * jcmvote_get_client_jcmvote_locked() -
 ...
 */
int jcmvote_get_client_jcmvote_locked(struct jcmvotable *votable, const char *client_str)
{
	int client_id;
	...
	if ((votable->type != JCMVOTE_SET_ANY)
...
int jcmvote_get_client_vote(struct jcmvotable *votable, const char *client_str)
{
	...
	lock_jcmvotable(votable);
	value = jcmvote_get_client_jcmvote_locked(votable, client_str);
	unlock_jcmvotable(votable);
	return value;
}

/**
 * jcmvote_get_effective_result() -
 * jcmvote_get_effective_result_locked() -
...
int jcmvote_get_effective_result(struct jcmvotable *votable)
{
	...
	lock_jcmvotable(votable);
	value = jcmvote_get_effective_result_locked(votable);
	unlock_jcmvotable(votable);
	return value;
}

/**
 * jcmvote_get_effective_client() -
 * jcmvote_get_effective_client_locked() -
...
const char *jcmvote_get_effective_client(struct jcmvotable *votable)
{
	...
	lock_jcmvotable(votable);
	client_str = jcmvote_get_effective_client_locked(votable);
	unlock_jcmvotable(votable);
	return client_str;
}
 ...
 *	or zero.
 */
int jcmvote(struct jcmvotable *votable, const char *client_str, bool enabled, int val)
{
	...
	switch (votable->type) {
	case JCMVOTE_MIN:
		jcmvote_min(votable, client_id, &effective_result, &effective_id);
		break;
	case JCMVOTE_MAX:
		jcmvote_max(votable, client_id, &effective_result, &effective_id);
		break;
	case JCMVOTE_SET_ANY:
		jcmvote_set_any(votable, client_id,
...
int jcmvote_rerun_election(struct jcmvotable *votable)
{
	...
	lock_jcmvotable(votable);
	effective_result = jcmvote_get_effective_result_locked(votable);
	if (votable->callback)
...
{
	struct jcmvotable *votable = data;
...
{
	struct jcmvotable *votable = m->private;
	int i;
	...
	effective_client_str = jcmvote_get_effective_client_locked(votable);
	seq_printf(m, "%s: effective=%s type=%s v=%d\n",
			votable->name,
			effective_client_str ? effective_client_str : "none",
			type_str,
			jcmvote_get_effective_result_locked(votable));
	unlock_jcmvotable(votable);
...
{
	struct jcmvotable *votable = inode->i_private;
				...
				void *data)
{
	...
	votable = find_jcmvotable(name);
	if (votable)
		return ERR_PTR(-EEXIST);

	if (debug_root == NULL) {
		debug_root = debugfs_create_dir("jcmvotable", NULL);
		if (!debug_root) {
	...
	if (votable_type >= NUM_JCMVOTABLE_TYPES) {
		pr_err("Invalid votable_type specified for voter\n");
		return ERR_PTR(-EINVAL);
	}

	votable = kzalloc(sizeof(struct jcmvotable), GFP_KERNEL);
	if (!votable)
		return ERR_PTR(-ENOMEM);
	...
	votable->data = data;
	votable->override_result = -EINVAL;
	mutex_init(&votable->jcmvote_lock);
