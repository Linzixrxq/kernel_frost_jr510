...
#define SD7601_VBUS_DEFUALT_VOLT_MV	5000
#define SD7601_IIC_LOCK_TIMEOUT  usecs_to_jiffies(200) //200US

static const unsigned int sd7601_extcon_cables[] = {
	EXTCON_USB,
	EXTCON_CHG_USB_SDP,
	EXTCON_CHG_USB_CDP,
	EXTCON_CHG_USB_DCP,
...
struct sd7601_chip {
	...
	struct delayed_work sd7601_batt_pro_work;
	...
	struct power_supply *sd7601_usb_psy;
...
#define SD7601_PR_ERR  (1 << 0)
#define SD7601_PR_WARN (1 << 1)
#define SD7601_PR_INFO (1 << 2)
#define SD7601_PR_DBG  (1 << 3)

#define SD7601_PR(_level, fmt, ...) \
	do { \
		if ((_level) & chip->debug_mask) { \
			dev_err(chip->dev, "%s:" fmt, __func__, ##__VA_ARGS__); \
		} else { \
			dev_dbg(chip->dev, "%s:" fmt, __func__, ##__VA_ARGS__); \
		}\
	} while(0)

//NOTE: pointer "chip", must have a struct sd7601_chip *chip in context
#define sd7601_err(fmt, ...)   SD7601_PR(SD7601_PR_ERR, fmt, ##__VA_ARGS__);
#define sd7601_warn(fmt, ...)   SD7601_PR(SD7601_PR_WARN, fmt, ##__VA_ARGS__);
#define sd7601_info(fmt, ...)  SD7601_PR(SD7601_PR_INFO, fmt, ##__VA_ARGS__);
#define sd7601_dbg(fmt, ...)   SD7601_PR(SD7601_PR_DBG, fmt, ##__VA_ARGS__);

static int32_t	sd7601_psy_notify(struct sd7601_chip *chip);
static int32_t	sd7601_extcon_clear_types(struct sd7601_chip *chip);
static int32_t	sd7601_extcon_notify(struct sd7601_chip *chip, unsigned int extcon_id, bool data);
...
static int32_t sd7601_read_byte(struct sd7601_chip *chip, uint8_t reg, uint8_t *data)
{
	...
	if (time_is_after_eq_jiffies(chip->last_update + SD7601_IIC_LOCK_TIMEOUT))
		udelay(200);
	for (i = 0; i < 4; i++) {
	...
	*data = (uint8_t)ret;
//	sd7601_dbg("%s: reg[0x%02X]  val[0x%02x]\n", __func__, reg, (uint8_t)ret);
	return 0;
}

static int32_t sd7601_write_byte(
		struct sd7601_chip *chip, int32_t reg, uint8_t val)
{
	int32_t ret = 0;
	int32_t i = 0;
//	sd7601_dbg("%s: reg[0x%02X]  val[0x%02x]\n", __func__, reg, val);
...
static int32_t sd7601_update(
	struct sd7601_chip *chip, uint8_t RegNum,
...
static int32_t sd7601_enter_debug_mode(struct sd7601_chip *chip, int32_t enable)
{
	uint8_t i    = 0;
	int32_t ret  = 0;
	uint8_t data = 0;
...
static int32_t sd7601_enable_sysovp(struct sd7601_chip *chip, int32_t enable)
{
	int32_t ret  = 0;
	uint8_t i    = 0;
	uint8_t data = 0;
...
static int sd7601_get_main_psy_prop(struct sd7601_chip *chip,
	...
	union power_supply_propval *val)
{
	...
	if (IS_ERR_OR_NULL(main_psy)) {
		sd7601_dbg("Cannot find power supply- \"%s\"\n",
				chip->battery_profile_psy);
		return -ENODEV;
	}
	ret = power_supply_get_property(main_psy, psp, val);
	if (ret < 0) {
		sd7601_dbg("Cannot read psp:%d value from \"%s\"\n",
...
{
	return sd7601_update(chip, SD7601_R00, !!enable,
...
static int32_t sd7601_set_input_curr_lim(
		struct sd7601_chip *chip, uint32_t ilimit_ma)
{
	uint8_t  data_reg = 0;
	...
	return sd7601_update(chip, SD7601_R00, data_reg,
...
static int32_t sd7601_get_input_curr_lim(struct sd7601_chip *chip)
{
	...
	data_reg = (data_reg & (CON0_IINLIM_MASK << CON0_IINLIM_SHIFT)) >>
			CON0_IINLIM_SHIFT;
...
static int sd7601_set_otg_en(struct sd7601_chip *chip, int32_t enable)
{
	return sd7601_update(chip, SD7601_R01, !!enable,
...
static int32_t sd7601_set_charger_en(struct sd7601_chip *chip, int32_t enable)
{
	...
	ret =  sd7601_update(chip, SD7601_R01, !!enable,
...
static int32_t sd7601_set_sys_min_volt(struct sd7601_chip *chip, uint32_t vsys_mv)
{
	...
	return sd7601_update(chip, SD7601_R01, data_reg,
				CON1_SYS_V_LIMIT_MASK, CON1_SYS_V_LIMIT_SHIFT);
}

int32_t sd7601_set_boost_ilim(struct sd7601_chip *chip, uint32_t boost_ilimit)
{
	uint8_t data_reg = 0;
	...
	return sd7601_update(chip, SD7601_R02, data_reg,
...
static int32_t sd7601_set_iprechg_current(
		struct sd7601_chip *chip, uint32_t iprechg_curr)
{
	uint8_t data_reg = 0;
	uint32_t data_temp = 0;
...
static int32_t sd7601_get_iprechg_current(struct sd7601_chip *chip)
{
	uint8_t data_reg = 0;
	int32_t ret = 0;
...
static int32_t sd7601_set_vreg_volt(struct sd7601_chip *chip, uint32_t vreg_chg_vol)
{
	...
	data_reg = (data_temp - VREG_VOL_MIN + (VREG_VOL_STEP/2)) / VREG_VOL_STEP;
...
static int32_t sd7601_set_vreg_volt(struct sd7601_chip *chip, uint32_t vreg_chg_vol)
{
	...
	sd7601_update(chip, SD7601_R0D, data_reg & 1, COND_CV_SP_MASK, COND_CV_SPP_SHIFT);
	return sd7601_update(chip, SD7601_R04, data_reg >> 1, CON4_VREG_MASK, CON4_VREG_SHIFT);
}
...
static int32_t sd7601_get_vreg_volt(struct sd7601_chip *chip)
{
	...
	if (ret < 0)
		return ret;
	ret = sd7601_read_byte(chip, SD7601_R0D, &data_reg1);
	if (ret < 0)
		return ret;
...
static int32_t sd7601_set_ir_comp(struct sd7601_chip *chip, uint32_t mohm)
{
#if 0
	uint8_t data_reg = 0;
	uint32_t data_temp = 0;
	...
	return sd7601_update(chip, SD7601_R0D, data_reg, COND_BAT_COMP_MASK, COND_BAT_COMP_SHIFT);
#endif
	return 0;
}

static int32_t sd7601_get_ir_comp(struct sd7601_chip *chip)
{
	uint8_t data_reg = 0;
	int32_t ret = 0;

	ret = sd7601_read_byte(chip, SD7601_R0D, &data_reg);
	if (ret < 0)
		return ret;
...
static int32_t sd7601_set_comp_max(struct sd7601_chip *chip, uint32_t mv)
{
#if 0
	uint8_t data_reg = 0;
	uint32_t data_temp = 0;
	...
	return sd7601_update(chip, SD7601_R0D, data_reg, COND_COMP_MAX_MASK, COND_COMP_MAX_SHIFT);
#endif
	return 0;
}

static int32_t sd7601_get_comp_max(struct sd7601_chip *chip)
{
	uint8_t data_reg = 0;
	int32_t ret = 0;
...
static int32_t sd7601_set_rechg_volt(
		struct sd7601_chip *chip, uint32_t rechg_volt)
{
	uint8_t data_reg = 0;
...
{
	return sd7601_update(chip, SD7601_R05, !!enable,
...
static int32_t sd7601_set_wd_timer(struct sd7601_chip *chip, uint32_t second)
{
	...
	return sd7601_update(chip, SD7601_R05, data_reg,
...
static int32_t sd7601_get_vlimit(struct sd7601_chip *chip)
{
	...
	if (data_reg & (CON10_HV_VINDPM_MASK << CON10_HV_VINDPM_SHIFT)) {
		data_reg = (data_reg & (CON10_VLIMIT_MASK << CON10_VLIMIT_SHIFT)) >>
				CON10_VLIMIT_SHIFT;
		...
		data_reg = (data_reg & (CON6_VINDPM_MASK << CON6_VINDPM_SHIFT)) >>
				CON6_VINDPM_SHIFT;
...
static int32_t sd7601_get_adc_value(struct sd7601_chip *chip, enum sd7601_adc_channel adc)
{
	...
	if (chip->component_id != SD7601AD_COMPINENT_ID &&
			chip->component_id != SD7601AD1_COMPINENT_ID &&
			chip->component_id != SD7601AD2_COMPINENT_ID)
		return -1;
	...
	sd7601_dbg("%s[0x%02X]: Regval = 0x%02X, Adcval = %d PG=%d\n",
		adc_channel[adc], SD7601_ADC_CHANNEL_START + adc, val, adc_value, !!pg_good);
...
static int32_t sd7601_get_vbus_voltage(struct sd7601_chip *chip)
{
	int tmout = 0;
	int32_t ret;
...
static int32_t sd7601_dump_msg(struct sd7601_chip *chip)
{
	...
	sd7601_dbg("[0x00] = 0x%02X, [0x01] = 0x%02X, [0x02] = 0x%02X, [0x03] = 0x%02X, [0x04] = 0x%02X, [0x05] = 0x%02X, [0x06] = 0x%02X, [0x07] = 0x%02X\n",
			 data[0x00], data[0x01], data[0x02], data[0x03],
			 data[0x04], data[0x05], data[0x06], data[0x07]);

	sd7601_dbg("[0x08] = 0x%02X, [0x09] = 0x%02X, [0x0A] = 0x%02X, [0x0B] = 0x%02X, [0x0C] = 0x%02X, [0x0D] = 0x%02X, [0x0E] = 0x%02X, [0x0F] = 0x%02X\n",
			data[0x08], data[0x09], data[0x0A], data[0x0B],
			data[0x0C], data[0x0D], data[0x0E], data[0x0F]);

	sd7601_dbg("[0x10] = 0x%02X\n", data[0x10]);

	//charge parameter
	sd7601_dbg("vchg:%d, ichg:%d, ilimit:%d, iterm %d, vlimit %d, sftimer %d, iboost %d\n",
	...
	//fault and state
	sd7601_dbg("hiz:%d, chg_en:%d, chg_state:%s, vbus_type:%s, pg:%d, thm:%d, vsys_state:%d\n",
		sd7601_get_en_hiz(chip),
		sd7601_get_charger_en(chip),
		charge_state[(data[SD7601_R08] & (CON8_CHRG_STAT_MASK << CON8_CHRG_STAT_SHIFT)) >>
					CON8_CHRG_STAT_SHIFT],
		vbus_type[(data[SD7601_R08] & (CON8_VBUS_STAT_MASK << CON8_VBUS_STAT_SHIFT)) >>
					CON8_VBUS_STAT_SHIFT],
		(data[SD7601_R08] & (CON8_PG_STAT_MASK << CON8_PG_STAT_SHIFT)) >>
				CON8_PG_STAT_SHIFT,
		(data[SD7601_R08] & (CON8_THM_STAT_MASK << CON8_THM_STAT_SHIFT)) >>
				CON8_THM_STAT_SHIFT,
		(data[SD7601_R08] & (CON8_VSYS_STAT_MASK << CON8_VSYS_STAT_SHIFT)) >>
				CON8_VSYS_STAT_SHIFT
	);

	sd7601_dbg("fault:%d, vbus_gd:%d, vdpm:%d, idpm:%d, acov:%d\n",
		data[SD7601_R09],
		(data[SD7601_R0A] & (CONA_VBUS_GD_MASK << CONA_VBUS_GD_SHIFT)) >>
				CONA_VBUS_GD_SHIFT,
		(data[SD7601_R0A] & (CONA_VINDPM_STAT_MASK << CONA_VINDPM_STAT_SHIFT)) >>
					CONA_VINDPM_STAT_SHIFT,
		(data[SD7601_R0A] & (CONA_IDPM_STAT_MASK << CONA_IDPM_STAT_SHIFT)) >>
					CONA_IDPM_STAT_SHIFT,
		(data[SD7601_R0A] & (CONA_ACOV_STAT_MASK << CONA_ACOV_STAT_SHIFT)) >>
...
static int32_t sd7601_set_dpdm(
	struct sd7601_chip *chip, uint8_t dp_val, uint8_t dm_val)
{
	uint8_t data_reg = 0;
...
static int32_t sd7601_charging_set_hvdcp20(
		struct sd7601_chip *chip, uint32_t vbus_target);
static int sd7601_bc12_postprocess(struct sd7601_chip *chip,bool force)
{
	int ret = 0;
	bool attach = false, inform_psy = true, usbdev_state = false;
	u8 port = SD7601_PORTSTAT_NO_INPUT;

	if (chip->component_id != SD7601D_COMPINENT_ID &&
				chip->component_id != SD7601AD_COMPINENT_ID &&
				chip->component_id != SD7601AD1_COMPINENT_ID &&
				chip->component_id != SD7601AD2_COMPINENT_ID)
		return -ENODEV;
	...
	if (!force && chip->attach == attach) {
		sd7601_dbg("attach(%d) is the same\n", attach);
		inform_psy = !attach;
		goto out;
	}
	chip->attach = attach;
//	dev_info(chip->dev, "attach = %d\n", attach);
	sd7601_set_vlimit(chip, DEFAULT_VLIMIT);

	if (!attach) {
		chip->port = SD7601_PORTSTAT_NO_INPUT;
		chip->chg_type = POWER_SUPPLY_TYPE_UNKNOWN;
//		sd7601_set_charger_en(chip, 0);
		sd7601_extcon_clear_types(chip);
		usbdev_state = false;
		goto out;
	}
...
//		sd7601_set_charger_en(chip, 0);
	...
	case SD7601_PORTSTAT_SDP:
		...
		sd7601_extcon_notify(chip, EXTCON_CHG_USB_SDP, true);
		break;
	case SD7601_PORTSTAT_CDP:
		chip->chg_type = POWER_SUPPLY_TYPE_USB_CDP;
		sd7601_extcon_notify(chip, EXTCON_CHG_USB_CDP, true);
		break;
	case SD7601_PORTSTAT_DCP:
		chip->chg_type = POWER_SUPPLY_TYPE_USB_DCP;
		sd7601_set_vlimit(chip, DEFAULT_DCP_VLIMIT);
		sd7601_extcon_notify(chip, EXTCON_CHG_USB_DCP, true);
		break;
	case SD7601_PORTSTAT_HVDCP:
		chip->chg_type = POWER_SUPPLY_TYPE_USB_HVDCP;
//		sd7601_set_vlimit(chip, 8500);
		sd7601_charging_set_hvdcp20(chip, 5);
//		msleep(1800);
		sd7601_extcon_notify(chip, EXTCON_CHG_USB_FAST, true);
//		sd7601_charging_set_hvdcp20(chip, 9);
//		msleep(200);
		break;
	case SD7601_PORTSTAT_NON_STANDARD:
	case SD7601_PORTSTAT_UNKNOWN:
		chip->chg_type = POWER_SUPPLY_TYPE_USB_FLOAT;
		sd7601_extcon_notify(chip, JLQ_EXTCON_CHG_USB_FLOAT, true);
...
out:
	if (chip->chg_type == POWER_SUPPLY_TYPE_USB ||
		chip->chg_type == POWER_SUPPLY_TYPE_USB_CDP)
		usbdev_state = true;
	sd7601_extcon_notify(chip, EXTCON_USB, usbdev_state);
...
#ifdef TRIGGER_CHARGE_TYPE_DETECTION
#define SD7601_VBUS_CHECK_RETRY  7
static void sd7601_chg_type_hvdcp_work(struct work_struct *work)
{
	struct sd7601_chip *chip =
		container_of(work, struct sd7601_chip, hvdcp_work.work);
	int ret;
	int retry_cnt;
	...
	sd7601_charging_set_hvdcp20(chip, 9);
	for (retry_cnt = 0; retry_cnt < SD7601_VBUS_CHECK_RETRY; retry_cnt++) {
		msleep(300);
		ret = sd7601_get_vbus_voltage(chip);
		if (ret < 0)
			continue;

		if (ret > 8000 && ret < 9500)
			break;
	}
	if (retry_cnt >= SD7601_VBUS_CHECK_RETRY) {
		sd7601_charging_set_hvdcp20(chip, 0);
		sd7601_dbg("Set 9V.Not in Rangs[%dmv]", ret);
		goto done;
	}
	sd7601_dbg("HVDCP detect Done.");
//	sd7601_charging_set_hvdcp20(chip, 9);
	chip->chg_type = POWER_SUPPLY_TYPE_USB_HVDCP;
	chip->port = SD7601_PORTSTAT_HVDCP;
	sd7601_extcon_notify(chip, EXTCON_CHG_USB_FAST, true);
	sd7601_extcon_notify(chip, EXTCON_CHG_USB_DCP, false);
	sd7601_psy_notify(chip);
done:
	chip->hvdcp_deting = false;
	pm_relax(chip->dev);
}

static int32_t sd7601_chg_type_det(struct sd7601_chip *chip)
{
	int32_t ret = 0;
	uint8_t val = 0;
	int32_t wait_cnt = 0;
...
//	dev_info(chip->dev, "wait cnt %d\n", wait_cnt);
	if (wait_cnt < 300) {
		...
		chg_type = (val & (CON8_VBUS_STAT_MASK << CON8_VBUS_STAT_SHIFT)) >>
				CON8_VBUS_STAT_SHIFT;
		if (chg_type == SD7601_PORTSTAT_DCP) {
			if (chip->hvdcp_deting)
				cancel_delayed_work(&chip->hvdcp_work);
			else
				pm_stay_awake(chip->dev);
			chip->hvdcp_deting = true;
			ret = sd7601_set_dpdm(chip, CONC_DP_DM_VOL_0P6, CONC_DP_DM_VOL_HIZ);
			sd7601_dbg("Start HVDCP deting\n");
			schedule_delayed_work(&chip->hvdcp_work, msecs_to_jiffies(1400));
			ret = 0;
		}
		sd7601_info("chg_type %s\n", vbus_type[chg_type]);
	} else {
		ret = -ETIMEDOUT;
		goto out;
	}
...
static int sd7601_bc12_en_kthread(void *data)
{
	int ret = 0;
	...
	while (1) {
		...
		sd7601_extcon_clear_types(chip);
		msleep(500);
		ret = sd7601_set_en_hiz(chip, false);
		if (ret < 0)
			dev_info(chip->dev, "dis hz fail(%d)\n", ret);

		ret = sd7601_chg_type_det(chip);
		if (ret < 0)
			dev_info(chip->dev, "fail(%d)\n", ret);
		if (ret != -EBUSY)
			sd7601_bc12_postprocess(chip,true);
...
int32_t sd7601_charge_type_detection(struct sd7601_chip *chip)
{
	if (chip->component_id != SD7601D_COMPINENT_ID &&
				chip->component_id != SD7601AD_COMPINENT_ID &&
				chip->component_id != SD7601AD1_COMPINENT_ID &&
...
static void sd7601_inform_psy_dwork_handler(struct work_struct *work)
{
	...
	sd7601_info("attach = %d, type = %d\n", attach, chg_type);
	sd7601_psy_notify(chip);
}
static void sd7601_unkown_type_dwork_handler(struct work_struct *work)
{
	int32_t ret = 0;
	struct sd7601_chip *chip = container_of(work, struct sd7601_chip, unkown_type_work.work);
	pm_stay_awake(chip->dev);
#ifdef TRIGGER_CHARGE_TYPE_DETECTION
	ret = sd7601_set_dpdm(chip, CONC_DP_DM_VOL_0P6, CONC_DP_DM_VOL_0P0);
	sd7601_charge_type_detection(chip);
#endif
	pm_relax(chip->dev);
}
		...
		struct sd7601_chip *chip, uint32_t vbus_target)
{
	...
	if (!mutex_trylock(&chip->bc12_lock))
		return -EBUSY;
	if(chip->port == SD7601_PORTSTAT_DCP)
		vlimit_mv = DEFAULT_DCP_VLIMIT;
	else
		vlimit_mv = DEFAULT_VLIMIT;
	sd7601_info("Set vbus target %dv\n", vbus_target);
	switch (vbus_target) {
	case 5:
		ret = sd7601_set_dpdm(chip, CONC_DP_DM_VOL_0P6, CONC_DP_DM_VOL_0P0);
		sd7601_set_vlimit(chip, vlimit_mv);
		break;
	case 9:
		sd7601_set_vlimit(chip, 9000 - chip->vlimit_offset_mv);
		ret = sd7601_set_dpdm(chip, CONC_DP_DM_VOL_3P3, CONC_DP_DM_VOL_0P6);
		break;
	case 12:
		sd7601_set_vlimit(chip, 12000 - chip->vlimit_offset_mv);
...
int32_t sd7601_charging_enable_hvdcp30(struct sd7601_chip *chip, bool enable)
{
//	sd7601_dbg("enable %d\n", enable);
	//enter continuous mode DP 0.6, DM 3.3
	return sd7601_set_dpdm(chip, CONC_DP_DM_VOL_0P6, CONC_DP_DM_VOL_3P3);
}

int32_t sd7601_charging_set_hvdcp30(struct sd7601_chip *chip, bool increase)
{
	int32_t ret = 0;

	mutex_lock(&chip->bc12_lock);
//	sd7601_dbg("increase %d\n", increase);
	if (increase) {
		//DP 3.3, DM 3.3
		ret = sd7601_set_dpdm(chip, CONC_DP_DM_VOL_3P3, CONC_DP_DM_VOL_3P3);
		if (ret < 0)
			goto out;
		//need test
		msleep(100);
		//DP 0.6, DM 3.3
		ret = sd7601_set_dpdm(chip, CONC_DP_DM_VOL_0P6, CONC_DP_DM_VOL_3P3);
		if (ret < 0)
			goto out;
		msleep(100);
	} else {
...
static int32_t sd7601_hw_component_detect(struct sd7601_chip *chip)
{
	...
	sd7601_info("chip_id:0x%02X, version_id:0x%02X, component_id:0x%04X\n",
			chip_id, version_id, chip->component_id);

	if ((chip->component_id != SD7601_COMPINENT_ID) &&
			(chip->component_id != SD7601D_COMPINENT_ID) &&
			(chip->component_id != SD7601AD_COMPINENT_ID) &&
			(chip->component_id != SD7601AD1_COMPINENT_ID) &&
			(chip->component_id != SD7601AD2_COMPINENT_ID))
		return -1;
	...
	if ((chip->component_id == SD7601AD_COMPINENT_ID) ||
			(chip->component_id == SD7601AD1_COMPINENT_ID)) {
		sd7601_enter_debug_mode(chip, 1);
		sd7601_enable_sysovp(chip, 0);
		sd7601_enter_debug_mode(chip, 0);
	}
	return 0;
}

static void charging_base_hw_init(struct sd7601_chip *chip)
{
		//sd7601_set_charger_en(chip, 0);
		//set ilimit 500ma as default
		sd7601_write_byte(chip, SD7601_R00, 0x04);
			//set as default, chg enable, vsys 3500mv, otg 2.8v falling
		sd7601_write_byte(chip, SD7601_R01, 0x0A);
		//set ichg 2040ma as default
		sd7601_write_byte(chip, SD7601_R02, 0xA2);
		//set pre_chg and iterm current 180ma
		sd7601_write_byte(chip, SD7601_R03, 0x22);
		//set as default, cv 4208mv
		sd7601_write_byte(chip, SD7601_R04, 0x58);
		//enable iterm, disable wdt, enable charge timer, chg timer 10hrs, jeita 20% ichg
		sd7601_write_byte(chip, SD7601_R05, 0x8F);
		//set vindpm 4500mv as default
		sd7601_write_byte(chip, SD7601_R06, 0xE6);
		//set as default
		sd7601_write_byte(chip, SD7601_R07, 0x4C);
		//disable iindpm interrupt & Vindpm interrupt
		sd7601_write_byte(chip, SD7601_R0A, 0x03);
		//disable hvdcp
		sd7601_write_byte(chip, SD7601_R0C, 0x00);
		//set bat comp as 40 mohm
		sd7601_write_byte(chip, SD7601_R0D, 0x00);
		//enable hv_vindpm
		sd7601_write_byte(chip, SD7601_R10, 0x87);
		sd7601_set_wd_reset(chip);

	return;
}

static void charging_profile_hw_init(struct sd7601_chip *chip)
{
	...
	sd7601_set_en_term_chg(chip, 1);
	sd7601_set_wd_timer(chip, 0);
	sd7601_set_en_chg_timer(chip, 1);
	if (chip->calibrated) {
		sd7601_set_ir_comp(chip, chip->bat_ir_mohm);
		sd7601_set_comp_max(chip, chip->vchg_comp_max_mv);
	}

	if (chip->vbus_bypass) {
		chip->discard = 1;
		sd7601_set_en_hiz(chip, 1);
		sd7601_set_charger_en(chip, 0);
	}
...
static void charging_hw_init(struct sd7601_chip *chip)
{
	sd7601_set_sys_min_volt(chip, chip->vsysmin_mv);
	sd7601_set_iprechg_current(chip, chip->pre_ma);
	sd7601_set_iterm(chip, chip->eoc_ma);
...
static void sd7601_work_func(struct work_struct *work)
{
	...
	sd7601_dump_msg(chip);
	schedule_delayed_work(&chip->sd7601_work, msecs_to_jiffies(2000));
}

static void sd7601_battery_profile_parse(struct work_struct *work)
{
	struct sd7601_chip *chip = container_of(work, struct sd7601_chip, sd7601_batt_pro_work.work);
	struct power_supply *main_psy;
//	struct jlq_charger_manager *jcm;
	...
	//	dev_info(chip->dev, "Not Find battery psy-node(%s),retry!.", chip->battery_profile_psy);
		schedule_delayed_work(&chip->sd7601_batt_pro_work, 2*HZ);
		return ;
	}
...
static int32_t sd7601_parse_dt(struct sd7601_chip *chip, struct device *dev)
{
	...
	if (of_property_read_string(np, "battery-profile-psy", &chip->battery_profile_psy) < 0) {
		...
		INIT_DELAYED_WORK(&chip->sd7601_batt_pro_work, sd7601_battery_profile_parse);
		schedule_delayed_work(&chip->sd7601_batt_pro_work, 4 * HZ);
	}
...
static ssize_t registers_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (chip->component_id == SD7601AD_COMPINENT_ID ||
				chip->component_id == SD7601AD1_COMPINENT_ID ||
				chip->component_id == SD7601AD2_COMPINENT_ID) {
		ret = sd7601_update(chip, SD7601_R11, 1,
		...
		const char *buf, size_t size)
{
	...
	dev_info(chip->dev, "write sd7601 reg 0x%02X with value 0x%02X !\n",
...
static int32_t sd7601_bat_fault_irq(struct sd7601_chip *chip, uint8_t data)
{
	uint8_t ret = 0;
	uint8_t val = 0;
...
static int32_t	sd7601_vlimit_irq(struct sd7601_chip *chip, uint8_t data)
{
	...
	return 0;
	if (data) {
		chg_stat = sd7601_get_charger_en(chip);
		if (chg_stat)
			return 0;
		sd7601_set_charger_en(chip, 0);
		input_curr_lim =  sd7601_get_input_curr_lim(chip);
		if (input_curr_lim <= INPUT_CURRT_MIN)
			return 0;
		input_curr_lim -= INPUT_CURRT_STEP;
		sd7601_set_input_curr_lim(chip, input_curr_lim);
		msleep(20);
...
#if 0
static int32_t	sd7601_vbus_state_irq(struct sd7601_chip *chip, uint8_t data)
{
...
static int32_t	sd7601_pg_state_irq(struct sd7601_chip *chip, uint8_t data)
{
	...
	mutex_unlock(&chip->bc12_lock);
	sd7601_bc12_postprocess(chip,false);
	sd7601_psy_notify(chip);
...
static int32_t	sd7601_vbus_good_irq(struct sd7601_chip *chip, uint8_t data)
{
	...
	if (data) {
		if (chip->phy_regulator) {
			sd7601_info("regulator_enable phy_regulator\n");
			/*
		...
		if (chip->phy_regulator) {
			sd7601_info("regulator_disable phy_regulator\n");
			regulator_disable(chip->phy_regulator);
		}
	}
...

{
	SD7601_R08, 0, 0,
	{
		...
		{
			.irq_name	 = "vbus state",
			.irq_func	 = sd7601_vbus_state_irq,
...
static int32_t sd7601_process_irq(struct sd7601_chip *chip)
{
	...
	for (i = 0; i < ARRAY_SIZE(irq_handles); i++) {
		...
		sd7601_info("reg 0x%02X, value now 0x%02X, value pre 0x%02X\n",
			irq_handles[i].reg, irq_handles[i].value, irq_handles[i].pre_value);

		if (ret < 0) {
			sd7601_info("Couldn't read reg 0x%02X ret = %d\n",
...
//				dev_info(chip->dev, "%s\n", irq_handles[i].irq_info[j].irq_name);
				...
				if (irq_handles[i].irq_info[j].irq_func) {
					...
					if (ret < 0)
						sd7601_warn("Couldn't handle %d irq for reg 0x%02X ret = %d\n",
...
static irqreturn_t sd7601_irq_handler(int irq, void *data)
{
...
int sd7601_otg_vbus_enable(struct regulator_dev *dev)
{
	struct sd7601_chip *chip = (struct sd7601_chip *)rdev_get_drvdata(dev);

	return sd7601_set_otg_en(chip, 1);
}

int sd7601_otg_vbus_disable(struct regulator_dev *dev)
{
	struct sd7601_chip *chip = (struct sd7601_chip *)rdev_get_drvdata(dev);

	return sd7601_set_otg_en(chip, 0);
}
int32_t sd7601_set_boost_ilim_1_2A_enable(struct regulator_dev *dev)
{
	struct sd7601_chip *chip = (struct sd7601_chip *)rdev_get_drvdata(dev);

	return sd7601_set_boost_ilim(chip, 1200);
}

int32_t sd7601_set_boost_ilim_1_2A_disable(struct regulator_dev *dev)
{
	struct sd7601_chip *chip = (struct sd7601_chip *)rdev_get_drvdata(dev);
...
int32_t sd7601_get_boost_ilim_1_2A_status(struct regulator_dev *dev)
{
	struct sd7601_chip *chip = (struct sd7601_chip *)rdev_get_drvdata(dev);
	int32_t boost_ilim;

	boost_ilim  = sd7601_get_boost_ilim(chip);
	return boost_ilim == 1200 ? 1 : 0;
}

static int sd7601_enchg_enable(struct regulator_dev *dev)
{
	struct sd7601_chip *chip = (struct sd7601_chip *)rdev_get_drvdata(dev);

	sd7601_dbg("dev->name:%s enable_mask:%d,enable_reg:%d shift:%d",
			dev->desc->name, dev->desc->enable_mask,
			dev->desc->enable_reg, MASK_TO_SHIFT(dev->desc->enable_mask));
	return sd7601_set_charger_en(chip, 1);
}

static int sd7601_enchg_disable(struct regulator_dev *dev)
{
	struct sd7601_chip *chip = (struct sd7601_chip *)rdev_get_drvdata(dev);
	...
	sd7601_dbg("dev->name:%s enable_mask:%d,enable_reg:%d shift:%d",
			dev->desc->name, dev->desc->enable_mask,
			dev->desc->enable_reg, MASK_TO_SHIFT(dev->desc->enable_mask));
	return sd7601_set_charger_en(chip, 0);
}

static int sd7601_regulator_enable(struct regulator_dev *dev)
{
	struct sd7601_chip *chip = (struct sd7601_chip *)rdev_get_drvdata(dev);
	...
	sd7601_dbg("dev->name:%s enable_mask:%d,enable_reg:%d shift:%d",
			dev->desc->name, dev->desc->enable_mask,
			dev->desc->enable_reg, MASK_TO_SHIFT(dev->desc->enable_mask));
	return sd7601_update(chip, dev->desc->enable_reg, 1,
		dev->desc->enable_mask >> MASK_TO_SHIFT(dev->desc->enable_mask),
		MASK_TO_SHIFT(dev->desc->enable_mask));
}

static int sd7601_regulator_disable(struct regulator_dev *dev)
{
	struct sd7601_chip *chip = (struct sd7601_chip *)rdev_get_drvdata(dev);
	...
	sd7601_dbg("dev->name:%s enable_mask:%d,enable_reg:%d shift:%d",
			dev->desc->name, dev->desc->enable_mask,
			dev->desc->enable_reg, MASK_TO_SHIFT(dev->desc->enable_mask));
	return sd7601_update(chip, dev->desc->enable_reg, 0,
		dev->desc->enable_mask >> MASK_TO_SHIFT(dev->desc->enable_mask),
		MASK_TO_SHIFT(dev->desc->enable_mask));
}

static int sd7601_regulator_is_enable(struct regulator_dev *dev)
{
	struct sd7601_chip *chip = (struct sd7601_chip *)rdev_get_drvdata(dev);
	uint8_t data_reg = 0;

	sd7601_read_byte(chip, dev->desc->enable_reg, &data_reg);
	sd7601_dbg("dev->name:%s enable_mask:0x%x,enable_reg:%d shift:%d :0x%x",
			dev->desc->name, dev->desc->enable_mask,
			dev->desc->enable_reg, MASK_TO_SHIFT(dev->desc->enable_mask), data_reg);
	return !!(data_reg & dev->desc->enable_mask) ;
}

static int sd7601_otg_reg_enable(struct regulator_dev *dev)
{
	struct sd7601_chip *chip = (struct sd7601_chip *)rdev_get_drvdata(dev);

	atomic_set(&chip->boost_fault_cnt, 0);
	return sd7601_regulator_enable(dev);
}

static int sd7601_otg_reg_disable(struct regulator_dev *dev)
{
	struct sd7601_chip *chip = (struct sd7601_chip *)rdev_get_drvdata(dev);

	atomic_set(&chip->boost_fault_cnt, 0);
	return sd7601_regulator_disable(dev);
}

static int sd7601_regulator_set_cur_uA(
	struct regulator_dev *dev, int min_uA, int max_uA)
{
	...
	sd7601_dbg("dev->name:%s uA:%d.sel:%d",
		dev->desc->name, min_uA, selector);
	return sd7601_update(chip, dev->desc->csel_reg, selector,
				dev->desc->csel_mask >> MASK_TO_SHIFT(dev->desc->csel_mask),
				MASK_TO_SHIFT(dev->desc->csel_mask));
}

static int sd7601_regulator_get_cur_uA(struct regulator_dev *dev)
{
	struct sd7601_chip *chip = (struct sd7601_chip *)rdev_get_drvdata(dev);
	uint8_t data_reg = 0;
	int32_t ret = 0;
	...
	ret = sd7601_read_byte(chip, dev->desc->csel_reg, &data_reg);
	if (ret < 0)
		return ret;
	selector = (data_reg & dev->desc->csel_mask) >>
		MASK_TO_SHIFT(dev->desc->csel_mask);
	sd7601_dbg("dev->name:%s .sel:%d .uA:%d",
		dev->desc->name, selector, dev->desc->curr_table[selector]);
	return dev->desc->curr_table[selector];
}

static int sd7601_regulator_set_volt_sel(
	struct regulator_dev *dev, unsigned int selector)
{
	struct sd7601_chip *chip = (struct sd7601_chip *)rdev_get_drvdata(dev);

	sd7601_dbg("dev->name:%s data_reg:[0x%x] Mask:%x selector:0x%x",
		dev->desc->name, dev->desc->vsel_reg, dev->desc->vsel_mask,
			selector);
	return sd7601_update(chip, dev->desc->vsel_reg, selector,
			dev->desc->vsel_mask >> MASK_TO_SHIFT(dev->desc->vsel_mask),
			MASK_TO_SHIFT(dev->desc->vsel_mask));
}

static int sd7601_regulator_get_volt_sel(struct regulator_dev *dev)
{
	struct sd7601_chip *chip = (struct sd7601_chip *)rdev_get_drvdata(dev);
	uint8_t data_reg = 0;
	int32_t ret = 0;

	ret = sd7601_read_byte(chip, dev->desc->vsel_reg, &data_reg);
	if (ret < 0)
		return ret;
	sd7601_dbg("dev->name:%s data_reg:[0x%x]->%x Mask:%x selector:0x%x",
...
static int sd7601_regulator_set_vreg_sel(
	struct regulator_dev *dev, unsigned int selector)
{
	struct sd7601_chip *chip = (struct sd7601_chip *)rdev_get_drvdata(dev);

	sd7601_dbg("dev->name:%s selector:%d-> %dmv [R:0x%02x S:%d]:%d [R:0x%02x S:%d]:%d",
			dev->desc->name, selector,
			VREG_VOL_MIN + (selector * VREG_VOL_STEP / 2),
			SD7601_R04, CON4_VREG_SHIFT, selector >> 1,
			SD7601_R0D, COND_CV_SPP_SHIFT, selector & 1);
	sd7601_update(chip, SD7601_R04, selector >> 1,
			CON4_VREG_MASK, CON4_VREG_SHIFT);
	return sd7601_update(chip, SD7601_R0D, selector & 1,
			COND_CV_SP_MASK, COND_CV_SPP_SHIFT);
}

static int sd7601_regulator_get_vreg_sel(struct regulator_dev *dev)
{
	struct sd7601_chip *chip = (struct sd7601_chip *)rdev_get_drvdata(dev);
	uint8_t data_reg = 0;
	uint8_t data_reg1 = 0;
	...
	if (ret < 0)
		return ret;
	sd7601_dbg("dev->name:%s data_reg:[0x%02x]:%x data_reg:[0x%02x]:%x ",
		dev->desc->name, SD7601_R04, data_reg,SD7601_R0D, data_reg1);
	data_reg =(data_reg >> CON4_VREG_SHIFT) & CON4_VREG_MASK;
	data_reg <<= 1;
	data_reg1 = (data_reg1 >> COND_CV_SPP_SHIFT) & COND_CV_SP_MASK;
	data_reg += data_reg1;
	sd7601_dbg("dev->name:%s selector:%d -> %dmv",
...
static int sd7601_regulator_set_vbus_input_volt(
	struct regulator_dev *dev, int min_uV, int max_uV, unsigned int *selector)
{
	struct sd7601_chip *chip = (struct sd7601_chip *)rdev_get_drvdata(dev);
	unsigned int index;

	if (chip->port != SD7601_PORTSTAT_HVDCP && chip->port != SD7601_PORTSTAT_NO_INPUT)
		return 0;
	...
	sd7601_dbg("dev->name:%s Set:%d V", dev->desc->name, dev->desc->volt_table[index] / 1000000);
	return sd7601_charging_set_hvdcp20(chip, dev->desc->volt_table[index] / 1000000);
}

static int sd7601_regulator_get_vbus_input_volt(struct regulator_dev *dev)
{
	struct sd7601_chip *chip = (struct sd7601_chip *)rdev_get_drvdata(dev);
	int32_t ret = 0;
	...
	if (ret < 0)
		return ret;
	sd7601_dbg("dev->name:%s Volt:%d mV", dev->desc->name, ret);
	return ret * 1000;
}

static struct regulator_init_data sd7601_input_initdata = {
	.constraints = {
		.name = "Iuput",
		.min_uV = 100000,
		.max_uV = 12000000,
...
static const int sd7601_input_volt_table[] = {
	100000, 5000000, 9000000, 12000000
};

static const int sd7601_input_current_limts[] = {
...
static const struct regulator_ops sd7601_input_charger_ops = {
	.list_voltage = regulator_list_voltage_table,
	.set_voltage = sd7601_regulator_set_vbus_input_volt,
	.get_voltage = sd7601_regulator_get_vbus_input_volt,
	.set_current_limit = sd7601_regulator_set_cur_uA,
	.get_current_limit = sd7601_regulator_get_cur_uA,
//	.map_voltage = regulator_map_voltage_ascend,
//	.list_voltage = regulator_list_voltage_table,
};

static struct regulator_desc sd7601_input_charger_desc = {
	.name = "SD7601-input-charger",
	.of_match = "SD7601-input-charger",
	.type = REGULATOR_VOLTAGE,
	.owner = THIS_MODULE,
	.ops = &sd7601_input_charger_ops,
	.csel_reg = SD7601_R00,
	.csel_mask = CON0_IINLIM_MASK << CON0_IINLIM_SHIFT,
	.volt_table = sd7601_input_volt_table,
	.n_voltages = ARRAY_SIZE(sd7601_input_volt_table),
	.curr_table = sd7601_input_current_limts,
	.n_current_limits = ARRAY_SIZE(sd7601_input_current_limts),
	.linear_min_sel = 0,
};

static struct regulator_init_data sd7601_charger_initdata = {
	.constraints = {
		.name = "charger",
...
static const int sd7601_charger_current_limts[] = {
...
static const struct regulator_ops sd7601_charger_ops = {
	.disable = sd7601_enchg_disable,
	.enable = sd7601_enchg_enable,
	.is_enabled = sd7601_regulator_is_enable,
	.set_voltage_sel = sd7601_regulator_set_vreg_sel,
	.get_voltage_sel = sd7601_regulator_get_vreg_sel,
	.set_current_limit = sd7601_regulator_set_cur_uA,
	.get_current_limit = sd7601_regulator_get_cur_uA,
...
static struct regulator_desc sd7601_charger_desc = {
	.name = "SD7601-charger",
	.of_match = "SD7601-charger",
	.supply_name = "charger",
	.type = REGULATOR_VOLTAGE,
	.owner = THIS_MODULE,
	.ops = &sd7601_charger_ops,
	.min_uV = VREG_VOL_MIN * 1000,
	...
	.curr_table = sd7601_charger_current_limts,
	.n_current_limits = ARRAY_SIZE(sd7601_charger_current_limts),
	.enable_reg = SD7601_R01,
	.enable_mask = CON1_CHG_CONFIG_MASK << CON1_CHG_CONFIG_SHIFT,
	.linear_min_sel = 0,
};

#ifdef SD7601_OTG
static struct regulator_init_data sd7601_otg_vbus_initdata = {
	.constraints = {
		.name = "otg",
		.min_uV = 4850000,
		.max_uV = 5300000,
...
static const int sd7601_boost_current_limts[] = {
	500000, 1200000
};
static const struct regulator_ops sd7601_otg_vbus_ops = {
	.enable = sd7601_otg_reg_enable,
	.disable = sd7601_otg_reg_disable,
	.is_enabled = sd7601_regulator_is_enable,
	.set_voltage_sel = sd7601_regulator_set_volt_sel,
	.get_voltage_sel = sd7601_regulator_get_volt_sel,
	.map_voltage = regulator_map_voltage_linear,
	.list_voltage = regulator_list_voltage_linear,
	.set_current_limit = sd7601_regulator_set_cur_uA,
	.get_current_limit = sd7601_regulator_get_cur_uA,
...
static struct regulator_desc sd7601_otg_vbus_desc = {
	.name = "SD7601-otg-vbus",
	.of_match = "SD7601-otg-vbus",
	.supply_name = "otg",
	.type = REGULATOR_VOLTAGE,
	.owner = THIS_MODULE,
	.ops = &sd7601_otg_vbus_ops,
	.min_uV = 4850000,
	.uV_step = 150000,
	.n_voltages = 4,
	.vsel_reg = SD7601_R06,
	...
	.curr_table = sd7601_boost_current_limts,
	.n_current_limits = ARRAY_SIZE(sd7601_boost_current_limts),
	.csel_reg = SD7601_R02,
...
//		val->intval = (chip->chg_type != POWER_SUPPLY_TYPE_UNKNOWN);
	...
	case	POWER_SUPPLY_PROP_QUICK_CHARGE_TYPE:
		...
		ret = sd7601_get_main_psy_prop(chip, POWER_SUPPLY_PROP_TEMP, &val_temp);
		if (!ret) {
	...
	case	POWER_SUPPLY_PROP_PRECHARGE_CURRENT:
		...
		ret = sd7601_get_iprechg_current(chip);
		val->intval = (ret < 0) ? 0 : (ret * 1000);
	break;
	...
	default:
		sd7601_err("get prop %d is not supported in SD7601\n", psp);
		ret = -EINVAL;
		break;
	}

	if (ret < 0) {
		sd7601_err("Couldn't get prop %d rc = %d\n", psp, ret);
		return -ENODATA;
	}
	return 0;
}

static int sd7601_set_prop(
	...
	const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:	//vbus in uv
		...
		break;
	case POWER_SUPPLY_PROP_QUICK_CHARGE_TYPE:
		ret = sd7601_charge_type_detection(chip);
		break;
	case POWER_SUPPLY_PROP_INPUT_VOLTAGE_LIMIT:
		if(val->intval == VILIMIT_VOL_MAX * 1000) {
			chip->discard = 1;
			sd7601_set_en_chg_timer(chip, 0);
			msleep(100);
			sd7601_set_en_chg_timer(chip, 1);
			ret = sd7601_set_vlimit(chip, VILIMIT_VOL_MAX);
		}
			...
			ret = sd7601_set_vlimit(chip, val->intval / 1000);
		break;
	case POWER_SUPPLY_PROP_CALIBRATE:
		chip->calibrated = val->intval;
		if (chip->calibrated) {
			sd7601_set_ir_comp(chip, chip->bat_ir_mohm);
			sd7601_set_comp_max(chip, chip->vchg_comp_max_mv);
		} else {
			sd7601_set_ir_comp(chip, 0);
			sd7601_set_comp_max(chip, 0);
		}
		break;
	default:
		sd7601_err("set prop %d is not supported\n", psp);
		ret = -EINVAL;
		break;
	}
...
static int sd7601_prop_is_writeable(
	struct power_supply *psy, enum power_supply_property psp)
{
	switch (psp) {
...
static struct power_supply_desc sd7601_psy_desc = {
	.name = "sd7601",
...
static enum power_supply_property sd7601_usb_props[] = {
	POWER_SUPPLY_PROP_PRESENT,
	POWER_SUPPLY_PROP_ONLINE,
	POWER_SUPPLY_PROP_QUICK_CHARGE_TYPE,
};

static int sd7601_get_usb_prop(
		struct power_supply *psy,
		enum power_supply_property psp,
		union power_supply_propval *val)
{
...
static int sd7601_set_usb_prop(
	struct power_supply *psy, enum power_supply_property psp,
	const union power_supply_propval *val)
{
	return -EINVAL;
...
static int sd7601_usb_prop_is_writeable(
	struct power_supply *psy, enum power_supply_property psp)
{
	return 0;
}
static struct power_supply_desc sd7601_usb_psy_desc = {
	.name = "usb",
	.type = POWER_SUPPLY_TYPE_USB_PD,
	.properties = sd7601_usb_props,
	.num_properties = ARRAY_SIZE(sd7601_usb_props),
	.get_property = sd7601_get_usb_prop,
	.set_property = sd7601_set_usb_prop,
	.property_is_writeable = sd7601_usb_prop_is_writeable,
};

static ssize_t sd7601_sysfs_ibat_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	int ret;
	struct sd7601_chip *chip
		= container_of(attr, struct sd7601_chip, attr_ibat);
...
static ssize_t sd7601_sysfs_vbat_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	int ret;
	struct sd7601_chip *chip
		= container_of(attr, struct sd7601_chip, attr_vbat);
	ret = sd7601_get_adc_value(chip, vbat_channel);
	if (ret < 0)
		return sprintf(buf, "%s\n", "Error");
	return sprintf(buf, "%d mV\n", ret);
}

static ssize_t sd7601_sysfs_vsys_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	int ret;
	struct sd7601_chip *chip
		= container_of(attr, struct sd7601_chip, attr_vsys);
	ret = sd7601_get_adc_value(chip, vsys_channel);
	if (ret < 0)
		return sprintf(buf, "%s\n", "Error");
	return sprintf(buf, "%d mV\n", ret);
}

static ssize_t sd7601_sysfs_usb_type_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct sd7601_chip *chip
		= container_of(attr, struct sd7601_chip, attr_usb_type);
	switch (chip->port) {
	case SD7601_PORTSTAT_NO_INPUT:
		return sprintf(buf, "%s\n", "No input");
	break;
...
static ssize_t sd7601_sysfs_usb_type_store(struct device *dev,
...
static ssize_t sd7601_sysfs_usb_discard_store(struct device *dev,
				struct device_attribute *attr, const char *buf,
				size_t count)
{
	struct sd7601_chip *chip
		= container_of(attr, struct sd7601_chip, attr_usb_discard);
	int ret;
	int discard;
	...
	sd7601_set_en_chg_timer(chip, 1);
	if(chip->discard)
		sd7601_set_vlimit(chip, VILIMIT_VOL_MAX);
	else
		sd7601_charge_type_detection(chip);
//	sd7601_set_en_hiz(chip, chip->discard);
	return count;
}

static ssize_t sd7601_sysfs_usb_discard_show(struct device *dev,struct device_attribute *attr, char *buf)
{
  struct sd7601_chip *chip
    = container_of(attr, struct sd7601_chip, attr_usb_discard);

//  chip->discard = sd7601_get_en_hiz(chip);
  return sprintf(buf, "%d\n", !!chip->discard);
}

static ssize_t sd7601_sysfs_dbg_mask_store(struct device *dev,
				struct device_attribute *attr, const char *buf,
				size_t count)
{
	struct sd7601_chip *chip
		= container_of(attr, struct sd7601_chip, attr_dbg_mask);
	int ret;
	int debug_mask;
...
static ssize_t sd7601_sysfs_dbg_mask_show(struct device *dev,struct device_attribute *attr, char *buf)
{
  struct sd7601_chip *chip
    = container_of(attr, struct sd7601_chip, attr_dbg_mask);
  return sprintf(buf, "%d\n", chip->debug_mask);
...
static int32_t sd7601_sysfs_init(struct sd7601_chip *chip)
{
	int i = 0;
	...
	chip->attr_usb_type.show
			= sd7601_sysfs_usb_type_show;
	chip->attr_usb_type.store
			= sd7601_sysfs_usb_type_store;
	...
	chip->attr_usb_discard.show
			= sd7601_sysfs_usb_discard_show;
	chip->attr_usb_discard.store
			= sd7601_sysfs_usb_discard_store;
	...
	chip->attr_dbg_mask.show
			= sd7601_sysfs_dbg_mask_show;
	chip->attr_dbg_mask.store
			= sd7601_sysfs_dbg_mask_store;
...
static int32_t sd7601_power_init(struct sd7601_chip *chip)
{
	...
	if (!sd7601_sysfs_init(chip))
		cfg.attr_grp = chip->sysfs_groups;
	sd7601_psy_desc.name = chip->chg_psy_name;

	chip->sd7601_psy = devm_power_supply_register(chip->dev, &sd7601_psy_desc, &cfg);

	if (IS_ERR(chip->sd7601_psy)) {
		dev_err(chip->dev, "SD7601:charger Couldn't register power supply\n");
		return PTR_ERR(chip->sd7601_psy);
	}

	cfg.attr_grp = NULL;
	chip->sd7601_usb_psy = devm_power_supply_register(chip->dev, &sd7601_usb_psy_desc, &cfg);

	if (IS_ERR(chip->sd7601_usb_psy)) {
		dev_err(chip->dev, "SD7601:usb Couldn't register power supply\n");
	}

	return 0;
}

static int32_t sd7601_regulator_init(struct sd7601_chip *chip)
{
	...
	if (chip->otg_regulator_name) {
		config.init_data = &sd7601_otg_vbus_initdata;
		sd7601_otg_vbus_initdata.constraints.name = chip->otg_regulator_name;
		sd7601_otg_vbus_desc.name = chip->otg_regulator_name;
		rdev = devm_regulator_register(chip->dev,
						&sd7601_otg_vbus_desc, &config);
	}
#endif
	if (chip->input_regulator_name) {
		config.init_data = &sd7601_input_initdata;
		sd7601_input_charger_desc.name = chip->input_regulator_name;
		sd7601_input_initdata.constraints.name = chip->input_regulator_name;
		rdev = devm_regulator_register(chip->dev,
						&sd7601_input_charger_desc, &config);
	}
	if (chip->chg_regulator_name) {
		config.init_data = &sd7601_charger_initdata;
		sd7601_charger_desc.name = chip->chg_regulator_name;
		sd7601_charger_initdata.constraints.name = chip->chg_regulator_name;
		rdev = devm_regulator_register(chip->dev,
						&sd7601_charger_desc, &config);
	}
	return 0;
}

static int32_t sd7601_extcon_init(struct sd7601_chip *chip)
{
	chip->edev = devm_extcon_dev_allocate(chip->dev,
									sd7601_extcon_cables);
	if (IS_ERR(chip->edev)) {
		chip->edev = NULL;
		return -1;
	}
	devm_extcon_dev_register(chip->dev, chip->edev);
	dev_info(chip->dev, "Register Extcon(%s) Success.",
		extcon_get_edev_name(chip->edev));
	if (chip->vbus_bypass)
...
static int32_t sd7601_register_irq(struct sd7601_chip *chip)
{
	...
	ret = devm_gpio_request_one(chip->dev, chip->irq_gpio, GPIOF_DIR_IN,
					...
					"sd7601_irq_gpio.%s", dev_name(chip->dev)));
	if (ret < 0) {
		sd7601_err("gpio request fail(%d)\n", ret);
		return ret;
	}

	chip->irq = gpio_to_irq(chip->irq_gpio);
	if (chip->irq < 0) {
		sd7601_err("gpio2irq fail(%d)\n", chip->irq);
		return chip->irq;
	}

	sd7601_info("irq gpio:%d irq = %d\n", chip->irq_gpio, chip->irq);
	...
	if (ret < 0) {
		sd7601_err("request threaded irq fail(%d)\n", ret);
		return ret;
	}
	enable_irq_wake(chip->irq);
	return ret;
}
static int sd7601_reboot_notify(struct notifier_block *this, unsigned long code,
			  void *unused)
{
	struct sd7601_chip *chip = container_of(this, struct sd7601_chip, reboot_nb);
	chip->discard = 0;
	sd7601_set_otg_en(chip, 0);
	sd7601_set_ichg_current(chip, 500);
	sd7601_set_input_curr_lim(chip, 500);
	sd7601_set_vlimit(chip, DEFAULT_VLIMIT);
	sd7601_set_vreg_volt(chip, DEFAULT_CV);
	sd7601_set_charger_en(chip, 1);
	sd7601_info("diable OTG, set chg(4400mV/500mA)\n");
	return NOTIFY_DONE;
}

static int32_t sd7601_register_reboot_nb(struct sd7601_chip *chip)
{
	chip->reboot_nb.notifier_call = sd7601_reboot_notify;
	chip->reboot_nb.priority = INT_MAX;
	register_reboot_notifier(&chip->reboot_nb);
	return 0;
}

static int32_t sd7601_driver_probe(
	struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	//chip->debug_mask = SD7601_PR_ERR | SD7601_PR_INFO | SD7601_PR_WARN | SD7601_PR_DBG;
	...
	ret = sd7601_hw_component_detect(chip);
	if (ret < 0) {
	...
	if (ret < 0)
		dev_err(chip->dev, "sd7601_parse_dt failed ret %d\n", ret);

	//charging_hw_init(chip);
	atomic_set(&chip->boost_fault_cnt, 0);
//	INIT_DELAYED_WORK(&chip->sd7601_work, sd7601_work_func);
//	schedule_delayed_work(&chip->sd7601_work, 0);
	...
	if ((chip->component_id == SD7601D_COMPINENT_ID) ||
		...
		INIT_DELAYED_WORK(&chip->psy_dwork,
				sd7601_inform_psy_dwork_handler);
#ifdef TRIGGER_CHARGE_TYPE_DETECTION
		...
		INIT_DELAYED_WORK(&chip->hvdcp_work,
					sd7601_chg_type_hvdcp_work);
		INIT_DELAYED_WORK(&chip->unkown_type_work,
				sd7601_unkown_type_dwork_handler);
#endif
	}
	...
	ret = sd7601_regulator_init(chip);
	if (ret < 0) {
		dev_err(chip->dev, "register regulator fail(%d)\n", ret);
		goto err_power;
	}
	ret = sd7601_extcon_init(chip);
	if (ret < 0) {
		dev_err(chip->dev, "register extcon fail(%d)\n", ret);
		goto err_power;
	}
	ret = sd7601_register_reboot_nb(chip);
	if (ret < 0) {
...
static int32_t sd7601_resume(struct device *dev)
{
	...
	if (atomic_read(&chip->vbus_gd) &&
			(chip->port == SD7601_PORTSTAT_UNKNOWN ||
			chip->port == SD7601_PORTSTAT_NON_STANDARD))
		schedule_delayed_work(&chip->unkown_type_work,
...
{
	struct sd7601_chip *chip = i2c_get_clientdata(client);
	sd7601_reboot_notify(&chip->reboot_nb, 0, NULL);
	sd7601_dump_msg(chip);
}
...
static const struct of_device_id sd7601_of_match[] = {
	{.compatible = "jlq,sd7601"},
	{.compatible = "jlq,sd7601d"},
...
{
	if (i2c_add_driver(&sd7601_driver) != 0)
		pr_err("failed to register sd7601 i2c driver.\n");
	else
		pr_info("Success to register sd7601 i2c driver.\n");
...
MODULE_SOFTDEP("pre: rpm-smd-regulator");
MODULE_LICENSE("GPL");
