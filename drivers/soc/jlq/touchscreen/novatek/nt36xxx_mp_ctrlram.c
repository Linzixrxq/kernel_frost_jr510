...
#define FW_RAWDATA_CSV_FILE "/data/vendor/touchpad/NVT_FWRawdataTest.csv"
...
#define PEN_FW_RAW_TEST_CSV_FILE "/data/vendor/touchpad/NVT_PenFWRawTest.csv"
#define PEN_NOISE_TEST_CSV_FILE "/data/vendor/touchpad/NVT_PenNoiseTest.csv"
...
static int32_t c_show_selftest(struct seq_file *m, void *v)
{
	...
	print_selftest_result(m, TestResult_FW_Rawdata, RecordResult_FW_Rawdata, RawData_FW_Rawdata, X_Channel, Y_Channel);
		...
		if (TestResult_FWMutual == -1) {
			...
			print_selftest_result(m, TestResult_FWMutual, RecordResult_FW_Rawdata, RawData_FW_Rawdata, X_Channel, Y_Channel);
		}
...
int lct_nvt_tp_selftest_callback(unsigned char cmd)
{
	...
	if (nvt_read_baseline(RawData_FW_Rawdata) != 0) {
		...
		TestResult_FWMutual = RawDataTest_SinglePoint_Sub(RawData_FW_Rawdata, RecordResult_FW_Rawdata, X_Channel, Y_Channel,
	...
	if ((TestResult_FW_Rawdata == 0) || (TestResult_FW_Rawdata == 1)) {
		 print_selftest_result_on_log(TestResult_FWMutual, RecordResult_FW_Rawdata, RawData_FW_Rawdata, X_Channel, Y_Channel);
	} else { // TestResult_FW_Rawdata is -1
...
EXPORT_SYMBOL(nvt_mp_proc_init);
...
EXPORT_SYMBOL(nvt_mp_proc_deinit);
