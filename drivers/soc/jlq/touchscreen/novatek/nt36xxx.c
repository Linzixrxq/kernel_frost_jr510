...
bool nvt_usb_plug_in = false; //for usb mode
extern bool nvt_usb_plug_in; //for usb mode

#if WAKEUP_GESTURE
extern void notify_lcd_gesture_state(bool en);
...
const uint16_t gesture_key_array[] = {
	...
	KEY_POWER,  //for tp_palm only using
};
#endif
...
EXPORT_SYMBOL(nvt_bld_crc_enable);
...
EXPORT_SYMBOL(nvt_fw_crc_enable);
...
EXPORT_SYMBOL(nvt_tx_auto_copy_mode);
...
EXPORT_SYMBOL(nvt_check_spi_dma_tx_info);
...
EXPORT_SYMBOL(nvt_bootloader_reset);
...
EXPORT_SYMBOL(nvt_clear_fw_status);
...
EXPORT_SYMBOL(nvt_check_fw_status);
...
EXPORT_SYMBOL(nvt_check_fw_reset_state);
...
*******************************************************/
#ifdef CONFIG_OF
static struct extcon_dev *nvt_charger_extcon_scan(
	struct device *dev,
...
static int nvt_extcon_usb_notifier_callback(struct notifier_block *self,
		unsigned long event, void *data)
{
	struct nvt_ts_data *ts =
		container_of(self, struct nvt_ts_data, usb_notif);

	if (!ts->edev) {
		nvt_usb_plug_in = false;
		goto exit;
	}

	if ((extcon_get_state(ts->edev, EXTCON_CHG_USB_CDP) > 0) ||
		(extcon_get_state(ts->edev, EXTCON_CHG_USB_FAST) > 0) ||
		(extcon_get_state(ts->edev, EXTCON_CHG_USB_DCP) > 0) ||
		(extcon_get_state(ts->edev, EXTCON_CHG_USB_SDP) > 0) ||
		(extcon_get_state(ts->edev, JLQ_EXTCON_CHG_USB_FLOAT) > 0))
		nvt_usb_plug_in = true;
  	else
    		nvt_usb_plug_in = false;
exit:
	if (nvt_usb_plug_in) {
		NVT_LOG("nvt usb plug in\n");
	} else {
		NVT_LOG("nvt usb plug out\n");
	}

	nvt_ts_usb_plugin_work(nvt_usb_plug_in);
	return 0;
}
...
static int32_t nvt_parse_dt(struct device *dev)
{
	...
	ret = of_property_read_u16(np, "novatek,resolution-width", &ts->abs_x_max);
	if (ret) {
		NVT_ERR("error reading novatek,resolution-width. ret=%d\n", ret);
		return ret;
	} else {
		NVT_LOG("resolution-width=0x%06X\n", ts->abs_x_max);
	}

	ret = of_property_read_u16(np, "novatek,resolution-height", &ts->abs_y_max);
	if (ret) {
		NVT_ERR("error reading novatek,resolution-height. ret=%d\n", ret);
		return ret;
	} else {
		NVT_LOG("resolution-height=0x%06X\n", ts->abs_y_max);
	}

	ret = of_property_read_u32(np, "novatek,longv-mode", &ts->longv_mode);
	if (ret) {
		NVT_ERR("error reading novatek,longv-mode. ret=%d\n", ret);
		ts->longv_mode = 0;
	} else {
		NVT_LOG("ts->longv_mode=0x%06X\n", ts->longv_mode);
	}
...
//2020.2.28 longcheer taocheng add for pocket mode endif

int32_t nvt_ts_usb_plugin_work(bool en)
{
	uint8_t buf[8] = {0};
	int32_t ret = 0;
	if ( !bTouchIsAwake ) {
	...
	if (nvt_usb_plug_in) {
		buf[1] = 0x53;// power plug ac on
  		NVT_LOG("enter charger\n");
        } else {
		buf[1] = 0x51;// power plug off
  		NVT_LOG("exit charger\n");
        }
	ret = CTP_SPI_WRITE(ts->client, buf, 2);
	if (ret < 0) {
...
static int nvt_ts_check_dt(struct device_node *np)
{
	...
	if (count == disabled_count) {
		NVT_ERR("no drm_panel is found\n");
		return -ENODEV;
	} else {
		NVT_ERR("defer probe drm_panel\n");
		return -EPROBE_DEFER;
	}
}
#endif
...
//create longcheer procfs node
	...
	ts->resume_wq = alloc_workqueue("nvt_ts_resume_wq", WQ_UNBOUND | WQ_MEM_RECLAIM | WQ_HIGHPRI, 1);
	if (!ts->resume_wq) {
			NVT_ERR("allocate resume workqueue failed.\n");
			goto err_alloc_resume_wq;
	}
	...
	ts->edev = nvt_charger_extcon_scan(&client->dev, "usb-dev-extcons");
	if (IS_ERR_OR_NULL(ts->edev)) {
		dev_err(&client->dev, "Can't found usb-dev-extcon\n");
		ts->edev = NULL;
	} else {
		ts->usb_notif.notifier_call = nvt_extcon_usb_notifier_callback;
		err  = extcon_register_notifier(ts->edev, EXTCON_CHG_USB_CDP, &ts->usb_notif);
		err |= extcon_register_notifier(ts->edev, EXTCON_CHG_USB_FAST, &ts->usb_notif);
          	err |= extcon_register_notifier(ts->edev, EXTCON_CHG_USB_DCP, &ts->usb_notif);
          	err |= extcon_register_notifier(ts->edev, EXTCON_CHG_USB_SDP, &ts->usb_notif);
          	err |= extcon_register_notifier(ts->edev, JLQ_EXTCON_CHG_USB_FLOAT, &ts->usb_notif);
		if(err < 0) {
			dev_err(&client->dev, "Cannot register notifier EXTCON_USB for %s\n",
					extcon_get_edev_name(ts->edev));
		} else {
			dev_err(&client->dev, " Register notifier EXTCON_USB for %s success!\n",
					extcon_get_edev_name(ts->edev));
		}
	}
...
#endif
#endif
  	nvt_extcon_usb_notifier_callback(&ts->usb_notif, 0, NULL);
	nvt_ts_resume(&ts->client->dev);
...
int lct_nvt_tp_gesture_callback(bool flag)
{
	...
	if (flag) {
		...
		notify_lcd_gesture_state(true);
		NVT_LOG("enable gesture mode\n");
	} else {
		ts->is_gesture_mode = false;
		notify_lcd_gesture_state(false);
...
MODULE_LICENSE("GPL");
MODULE_SOFTDEP("pre: komeda spi-jlq sd7601 sgm4154x_charger");
