...
struct upgrade_module ft_module_list[] = {
...
struct upgrade_setting_nf upgrade_setting_list[] = {
    ...
    {0xF0, 0xC6, 0, (84 * 1024), (128 * 1024), 0xA5, 0x01, 8,  0, 1, 0, 1},
};
...
{
    int i = 0;
    struct upgrade_module *info = &ft_module_list[0];
...
int fts_fwupg_init(struct fts_ts_data *ts_data)
{
    ...
    if (ts_data->pdata->longv_mode)
        fwupgrade->module_id = FTS_MODULE2_ID;
    else
        fwupgrade->module_id = FTS_MODULE_ID;

    FTS_INFO("fwupgrade->module_id=0x%x\n", fwupgrade->module_id);
