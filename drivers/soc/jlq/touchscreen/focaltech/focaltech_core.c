...
bool fts_usb_plug_in = false; //for usb mode
extern bool fts_usb_plug_in; //for usb mode
...
int lct_fts_tp_gesture_callback(bool flag)
{
	...
	if (ts_data->suspended) {
		delay_gesture = true;
		FTS_INFO("The gesture mode will be %s the next time you wakes up.",
...
static struct extcon_dev *fts_charger_extcon_scan(
	struct device *dev,
...
static int fts_extcon_usb_notifier_callback(struct notifier_block *self,
		unsigned long event, void *data)
{
  	struct fts_ts_data  *ts =
		container_of(self, struct fts_ts_data, usb_notif);

	if (!ts->edev) {
		fts_usb_plug_in = false;
		goto exit;
	}
  	...
  	if ((extcon_get_state(ts->edev, EXTCON_CHG_USB_CDP) > 0) ||
		...
		fts_usb_plug_in = true;
  	else
		fts_usb_plug_in = false;

exit:
	if (fts_usb_plug_in) {
		FTS_ERROR("fts usb plug in");
	} else {
		FTS_ERROR("fts usb plug out");
	}

	lct_fts_set_charger_mode(fts_usb_plug_in);
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
    ...
    ret = of_property_read_u32(np, "focaltech,longv-mode", &pdata->longv_mode);
    if (ret) {
        FTS_ERROR("Unable to read focaltech,longv-mode\n");
        pdata->longv_mode = 0;
    } else {
        FTS_INFO("pdata->longv_mode=0x%06X\n", pdata->longv_mode);
    }
...
static int drm_check_dt(struct device_node *np)
{
	...
	if (count == disabled_count) {
		...
		FTS_ERROR("defer probe drm_panel\n");
		return -EPROBE_DEFER;
	}
}
...
static int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
	...
	ts_data->edev = fts_charger_extcon_scan(ts_data->dev, "usb-dev-extcons");
	if (IS_ERR_OR_NULL(ts_data->edev)) {
		dev_err(ts_data->dev, "Can't found usb-dev-extcon\n");
		ts_data->edev = NULL;
	} else {
		ts_data->usb_notif.notifier_call = fts_extcon_usb_notifier_callback;
		err  = extcon_register_notifier(ts_data->edev, EXTCON_CHG_USB_CDP, &ts_data->usb_notif);
		err |= extcon_register_notifier(ts_data->edev, EXTCON_CHG_USB_FAST, &ts_data->usb_notif);
		err |= extcon_register_notifier(ts_data->edev, EXTCON_CHG_USB_DCP, &ts_data->usb_notif);
		err |= extcon_register_notifier(ts_data->edev, EXTCON_CHG_USB_SDP, &ts_data->usb_notif);
		err |= extcon_register_notifier(ts_data->edev, JLQ_EXTCON_CHG_USB_FLOAT, &ts_data->usb_notif);
		if(err < 0) {
			dev_err(ts_data->dev, "Cannot register notifier EXTCON_USB for %s\n",
					extcon_get_edev_name(ts_data->edev));
		} else {
			dev_err(ts_data->dev, " Register notifier EXTCON_USB for %s success!\n",
					extcon_get_edev_name(ts_data->edev));
		}
	}

   fts_extcon_usb_notifier_callback(&ts_data->usb_notif, 0, NULL);
    FTS_FUNC_EXIT();
    return 0;
...
#if defined(CONFIG_DRM_PANEL)
    if (spi->dev.of_node) {
        ret = drm_check_dt(spi->dev.of_node);
        if (ret) {
...
#ifdef MODULE
MODULE_SOFTDEP("pre: komeda sd7601 sgm4154x_charger");
#endif
