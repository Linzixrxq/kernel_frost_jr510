...
void jlq_slim_submod_register_noti(void);
...
int jlq_slim_rx_queue_reset(struct jlq_slim_ctrl *dev)
{
    unsigned long flags;

    spin_lock_irqsave(&dev->rx_lock, flags);
    dev->tail = dev->head = 0;
    memset((u8 *)dev->rx_msgs[dev->head], 0, JLQ_SLIM_MSGQ_BUF_LEN);
    spin_unlock_irqrestore(&dev->rx_lock, flags);
    return 0;
}
...
static int jlq_xfer_msg(struct slim_controller *ctrl, struct slim_msg_txn *txn)
{
    ...
    */
    ...
    /*reconfiguration*/
        ...
        uint8_t ch;
        MTDR_SubframeMode sfm;
        MTDR_ClockGear cg;

        /*with wcn3950*/
        ch = txn->wbuf[0];
        if((ch == 160 || ch == 161) && (presenceRate == MTDR_PR_48K)) {
            sfm = MTDR_SM_3_CSW_32_SL;
            cg = MTDR_CG_8;
        } else if((ch == 157 || ch == 159) && (presenceRate == MTDR_PR_16K)) {
            sfm = MTDR_SM_3_CSW_24_SL;
            cg = MTDR_CG_6;
        } else if((ch == 157 || ch == 159) && (presenceRate == MTDR_PR_8K)) {
            sfm = MTDR_SM_3_CSW_24_SL;
            cg = MTDR_CG_5;
        } else {
            pr_err(LOGTAG"[slim chl]chl param error(chl: %d, presenceRate: %d)\n", ch, presenceRate);
            ret = -1;
        }

        if(!ret) {
            pr_info(LOGTAG"[slim chl]chl: %d, presenceRate: %d, SubframeMode: %d, ClockGear: %d\n", ch, presenceRate, sfm, cg);

            m_MTDR_MsgNextSubframeMode(sfm);
            m_MTDR_MsgNextClockGear(cg);
        ...
        if(!ret) {
            if(ch == 160) {
                ...
                pr_err(LOGTAG"[slim chl]chl error(chl: %d)\n", ch);
                ret = -1;
            }

            if(!ret) {
                pr_info(LOGTAG"[slim chl]chl: %d, tp: %d, sd: 0x%x\n", ch, tp, sd);
                ret = m_MTDR_MsgNextDefineChannel(txn->wbuf[0], tp, sd, sd_len);
            }
        }
                         ...
                         u8 elen, u8 laddr)
{
    ...
    if (ret) {
        ...
        pr_info(LOGTAG"jlq_set_laddr succ, ea: %2x %2x %2x %2x %2x %2x, la: %2x\n",
...
/*audio subsys rereset*/
static void jlq_slim_rereset(void)
{
    struct jlq_slim_ctrl *dev;
    ...
    /*reset rx queue*/
    jlq_slim_rx_queue_reset(dev);

    /*jr510 slimbus dereset*/
    m_slimbus_enable(SBCtrlBase);
    //pr_info(LOGTAG"dereset slimbus ok\n");
    ...
    //pr_info(LOGTAG"%s, end\n", __func__);

}

static int jlq_slim_submod_noti_cb(struct notifier_block *this, unsigned long event, void *ptr)
{
    pr_info(LOGTAG"%s, event: %u\n", __func__, event);

    if(event == ADSP_STAT_RERESET)
        jlq_slim_rereset();

    return NOTIFY_OK;
}

static struct notifier_block jlq_slim_submod_notifier = {
    .notifier_call = jlq_slim_submod_noti_cb,
};
...
void jlq_slim_submod_register_noti(void)
{
    ...
    adsp_submod_notif_register("slim_submod", &jlq_slim_submod_notifier);
}

/*slimbus frm clk enable/disable*/
static atomic_t frm_clk_count = ATOMIC_INIT(0);

int jlq_slim_frm_clk_enable(void)
{
#ifdef FRM_CLK_DISABLE
    struct jlq_slim_ctrl *dev;
    unsigned long freq;
    int ret;

    if(atomic_read(&frm_clk_count) > 0) {
        atomic_inc(&frm_clk_count);
        return 0;
    } else {
        atomic_inc(&frm_clk_count);
    }
    ...
    if (IS_ERR(dev->rclk)) {
        pr_err(LOGTAG"slimbus_frm_clk_i is error\n");
        return -1;
    }

    ret = clk_prepare_enable(dev->rclk);
    if(ret != 0) {
        pr_err(LOGTAG"%s, enable fail!\n", __func__);
        return -1;
    }

    freq = clk_get_rate(dev->rclk);
    pr_info(LOGTAG"check slimbus_frm_clk_i, %lu\n", freq);
#endif
    return 0;
}
EXPORT_SYMBOL(jlq_slim_frm_clk_enable);
...
EXPORT_SYMBOL(jlq_slim_frm_clk_disable);
