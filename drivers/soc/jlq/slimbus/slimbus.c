...
static int __init slimbus_init(void)
{
	...
	PR_INFO(LOGTAG"slimbus_init(bus init)\n");
    ...
    PR_INFO(LOGTAG"slimbus_init(bus init), succ\n");
...
static int slim_drv_probe(struct device *dev)
{
	...
	PR_INFO(LOGTAG"slim_drv_probe(bus)\n");
...
{
    PR_INFO(LOGTAG"slim_driver_register\n");
...
{
    PR_INFO(LOGTAG"slim_add_device(name: %s, eaddr: %x %x %x %x %x %x)\n", 
        sbdev->name, sbdev->e_addr[0], sbdev->e_addr[1], sbdev->e_addr[2],
				...
				struct slim_boardinfo *bi)
{
	...
	PR_INFO(LOGTAG"slim_match_ctrl_to_boardinfo\n");
	...
	if (ret != 0)
		dev_err(ctrl->dev.parent, LOGTAG"can't create new device for %s\n",
			bi->slim_slave->name);
}
...
int slim_register_board_info(struct slim_boardinfo const *info, unsigned int n)
{
	...
	PR_INFO(LOGTAG"slim_register_board_info\n");
...
void slim_ctrl_add_boarddevs(struct slim_controller *ctrl)
{
	...
	PR_INFO(LOGTAG"slim_ctrl_add_boarddevs\n");
...
static int slim_register_controller(struct slim_controller *ctrl)
{
	...
	PR_INFO(LOGTAG"slim_register_controller\n");
	...
	dev_dbg(&ctrl->dev, LOGTAG"Bus [%s] registered\n", ctrl->name);
...
void slim_remove_device(struct slim_device *sbdev)
{
	...
	PR_INFO(LOGTAG"slim_remove_device(%s)\n", sbdev->name);
...
int slim_add_numbered_controller(struct slim_controller *ctrl)
{
	...
	PR_INFO(LOGTAG"slim_add_numbered_controller\n");
	...
	if (id < 0)
	{
	    PR_INFO(LOGTAG"idr_alloc, fail(%d)\n", id);
		return id;
	}
...
void slim_msg_response(struct slim_controller *ctrl, u8 *reply, u8 tid, u8 len)
{
	...
	if (txn == NULL || txn->rbuf == NULL) {
		...
		if (txn == NULL)
			dev_err(&ctrl->dev, LOGTAG"Got response to invalid TID:%d, len:%d\n",
				tid, len);
		else
			dev_err(&ctrl->dev, LOGTAG"Invalid client buffer passed\n");
		return;
	}
	async = txn->async;
	for (i = 0; i < len; i++)
				...
				u8 e_len, u8 *laddr, bool valid)
{
	...
	dev_dbg(&ctrl->dev, LOGTAG"setting slimbus l-addr:%x\n", *laddr);
ret_assigned_laddr:
	mutex_unlock(&ctrl->m_ctrl);
	if (exists || ret)
		return ret;

	PR_INFO(LOGTAG"slimbus:%d laddr:0x%x, EAPC:0x%x:0x%x\n", ctrl->nr, *laddr,
				e_addr[2], e_addr[3]);
	mutex_lock(&ctrl->m_ctrl);
	list_for_each_safe(pos, next, &ctrl->devs) {
		sbdev = list_entry(pos, struct slim_device, dev_list);
		if (memcmp(sbdev->e_addr, e_addr, 6) == 0) {
			...
			PR_INFO(LOGTAG"device, name: %s, la: %2x\n", sbdev->name, sbdev->laddr);
			...
			const u8 *wbuf, u8 len)
{
	...
	dev_dbg(&ctrl->dev, LOGTAG"SB xfer msg:os:%x, len:%d, MC:%x, sl:%x\n",
	...
	if (rbuf) {
		...
		if (!ret && !msg->comp) {
			...
			if (!ret) {
				dev_err(&ctrl->dev, LOGTAG"slimbus Read timed out\n");
				spin_lock_irqsave(&ctrl->txn_lock, flags);
			...
			dev_err(&ctrl->dev, LOGTAG"slimbus Read error\n");
			spin_lock_irqsave(&ctrl->txn_lock, flags);
			/* Invalidate the transaction */
			ctrl->txnt[txn->tid] = NULL;
			spin_unlock_irqrestore(&ctrl->txn_lock, flags);
			...
			int (*comp_cb)(void *ctx, int err), void *ctx)
{
	...
	if (!sb->ctrl->xfer_bulk_wr) {
		pr_warn(LOGTAG"controller does not support bulk WR, serializing\n");
		for (i = 0; i < n; i++) {
...
int slim_dealloc_mgrports(struct slim_device *sb, u32 *hdl, int nports)
{
	...
	for (i = 0; i < nports; i++) {
		...
		if (pn >= ctrl->nports || ctrl->ports[pn].state == SLIM_P_CFG) {
			...
			if (pn >= ctrl->nports) {
				dev_err(&ctrl->dev, LOGTAG"invalid port number\n");
				ret = -EINVAL;
			} else {
				dev_err(&ctrl->dev,
					LOGTAG"Can't dealloc connected port:%d\n", i);
				ret = -EISCONN;
			}
			for (j = i - 1; j >= 0; j--) {
...
int slim_connect_src(struct slim_device *sb, u32 srch, u16 chanh)
{
	...
	PR_INFO(LOGTAG"slim_connect_src(slim_dev: %s, porth: %.8x, chanh: %.8x)\n", sb->name, srch, chanh);
...
int slim_connect_sink(struct slim_device *sb, u32 *sinkh, int nsink, u16 chanh)
{
	...
	PR_INFO(LOGTAG"slim_connect_sink(slim_dev: %s, porth: %.8x, chanh: %.8x)\n", sb->name, *sinkh, chanh);
				...
				struct completion *comp)
{
	...
	dev_dbg(&ctrl->dev, LOGTAG"port xfer: num:%d", pn);
			...
			phys_addr_t *done_buf, u32 *done_len)
{
	...
	dev_dbg(&ctrl->dev, LOGTAG"get status port num:%d", pn);
	/*
...
static int slim_nextdefine_ch(struct slim_device *sb, u8 chan)
{
	...
	dev_dbg(&ctrl->dev, LOGTAG"ch:%d, chan PR rate:0x%x\n", chan, slc->prrate);
...
query_out:
	mutex_unlock(&ctrl->sched.m_reconf);
	dev_dbg(&ctrl->dev, LOGTAG"query ch:%d,hdl:%d,ref:%d,ret:%d",
...
int slim_dealloc_ch(struct slim_device *sb, u16 chanh)
{
	...
	PR_INFO(LOGTAG"slim_dealloc_ch(chan: %d)\n", chan);
	...
	if (slc->ref > 1) {
		slc->ref--;
		mutex_unlock(&ctrl->sched.m_reconf);
		dev_dbg(&ctrl->dev, LOGTAG"remove chan:%d,hdl:%d,ref:%d",
					slc->chan, chanh, slc->ref);
		return 0;
	}
	if (slc->state >= SLIM_CH_PENDING_ACTIVE) {
		dev_err(&ctrl->dev, LOGTAG"Channel:%d should be removed first\n",
								chan);
		mutex_unlock(&ctrl->sched.m_reconf);
		return -EISCONN;
	}
			...
			u8 nchan, bool grp, u16 *grph)
{
	...
	for (i = 0; i < nchan; i++) {
		...
		dev_dbg(&ctrl->dev, LOGTAG"define_ch: ch:%d, state:%d", chan,
...
err_define_ch:
	dev_dbg(&ctrl->dev, LOGTAG"define_ch: ch:%d, ret:%d", *chanh, ret);
	mutex_unlock(&ctrl->sched.m_reconf);
	return ret;
}
			...
			u32 *ctrlw, u32 *subfrml)
{
	...
	if (bias == SLIM_COEFF_1) {
		...
		while ((coeff1 < ctrl->sched.num_cc1) || (curintr > 24)) {
			...
			if (opensl1[0] <= 0) {
				dev_dbg(&ctrl->dev, LOGTAG"reconfig failed:%d\n",
					__LINE__);
				return -EXFULL;
			}
			curexp++;
				...
				u32 subfrml, u32 clkgear)
{
	...
	dev_dbg(&ctrl->dev, LOGTAG"Clock gear is:%d\n", clkgear);
	...
	while (cc1 < ctrl->sched.num_cc1) {
		...
		if (slc == NULL) {
			dev_err(&ctrl->dev, LOGTAG"SLC1 null in verify: chan%d\n",
				cc1);
			return -EIO;
		}
		dev_dbg(&ctrl->dev, LOGTAG"chan:%d, offset:%d, intr:%d, seglen:%d\n",
	...
	while (cc3 < ctrl->sched.num_cc3) {
		...
		if (slc == NULL) {
			dev_err(&ctrl->dev, LOGTAG"SLC3 null in verify: chan%d\n",
				cc3);
			return -EIO;
		}
		dev_dbg(&ctrl->dev, LOGTAG"chan:%d, offset:%d, intr:%d, seglen:%d\n",
...
static int slim_allocbw(struct slim_device *sb, int *subfrmc, int *clkgear)
{
	...
	*clkgear = SLIM_MAX_CLK_GEAR;
	...
	dev_dbg(&ctrl->dev, LOGTAG"used sl:%u, availlable sl:%u\n", usedsl, availsl);
	dev_dbg(&ctrl->dev, LOGTAG"pending:chan sl:%u, :msg sl:%u, clkgear:%u\n",
...
int slim_reconfigure_now(struct slim_device *sb)
{
	...
	PR_INFO(LOGTAG"slim_reconfigure_now\n");
	...
	if (sb->pending_msgsl == sb->cur_msgsl &&
		...
		if (list_empty(&sb->mark_removal)) {
			PR_INFO(LOGTAG"SLIM_CL: skip reconfig sequence\n");
			return 0;
		}
	}
	...
	if (!ret) {
		ret = slim_processtxn(ctrl, &txn, false);
		dev_dbg(&ctrl->dev, LOGTAG"sending begin_reconfig:ret:%d\n", ret);
	}
	...
	if (!ret && subframe != ctrl->sched.subfrmcode) {
		...
		txn.wbuf = wbuf;
		ret = slim_processtxn(ctrl, &txn, false);
		dev_dbg(&ctrl->dev, LOGTAG"sending subframe:%d,ret:%d\n",
				(int)wbuf[0], ret);
	}
	if (!ret && clkgear != ctrl->clkgear) {
		...
		txn.wbuf = wbuf;
		ret = slim_processtxn(ctrl, &txn, false);
		dev_dbg(&ctrl->dev, LOGTAG"sending clkgear:%d,ret:%d\n",
				(int)wbuf[0], ret);
	}
	if (ret)
	...
	list_for_each_entry(pch, &sb->mark_define, pending) {
		...
		txn.rl = 7;
		txn.wbuf = wbuf;
		dev_dbg(&ctrl->dev, LOGTAG"define content, activate:%x, %x, %x, %x\n",
        ...
        PR_INFO(LOGTAG"slim_reconfigure_now, SLIM_MSG_MC_NEXT_ACTIVATE_CHANNEL(chan: %d)\n", slc->chan);
	...
	list_for_each_entry(pch, &sb->mark_removal, pending) {
		...
		dev_dbg(&ctrl->dev, LOGTAG"remove chan:%x\n", pch->chan);
		wbuf[0] = slc->chan;
	...
	list_for_each_entry(pch, &sb->mark_suspend, pending) {
		...
		dev_dbg(&ctrl->dev, LOGTAG"suspend chan:%x\n", pch->chan);
		wbuf[0] = slc->chan;
		PR_INFO(LOGTAG"slim_reconfigure_now, SLIM_MSG_MC_NEXT_DEACTIVATE_CHANNEL(chan: %d)\n", slc->chan);
	...
	for (i = 0; i < ctrl->sched.num_cc1; i++) {
		...
		expshft = SLIM_MAX_CLK_GEAR - clkgear;
		dev_dbg(&ctrl->dev, LOGTAG"new-intr:%d, old-intr:%d, dist:%d\n",
				slc->newintr, slc->interval, segdist);
		dev_dbg(&ctrl->dev, LOGTAG"new-off:%d, old-off:%d\n",
	...
	txn.wbuf = NULL;
	ret = slim_processtxn(ctrl, &txn, false);
	dev_dbg(&ctrl->dev, LOGTAG"reconfig now:ret:%d\n", ret);
	if (!ret) {
	...
	PR_INFO(LOGTAG"slim_reconfigure_now CMD error\n");

revert_reconfig:
    PR_INFO(LOGTAG"slim_reconfigure_now flow error, revert\n");
			...
			enum slim_ch_control chctrl, bool commit)
{
	...
	PR_INFO(LOGTAG"slim_control_ch(chan: %d, control: %d)\n", chan, chctrl);
	...
	do {
		...
		slc = &ctrl->chans[chan];
		dev_dbg(&ctrl->dev, LOGTAG"chan:%d,ctrl:%d,def:%d", chan, chctrl,
					slc->def);
		if (slc->state < SLIM_CH_DEFINED) {
			ret = -ENOTCONN;
			break;
...
int slim_reservemsg_bw(struct slim_device *sb, u32 bw_bps, bool commit)
{
	...
	dev_dbg(&ctrl->dev, LOGTAG"request:bw:%d, slots:%d, current:%d\n", bw_bps, sl,
						sb->cur_msgsl);
	sb->pending_msgsl = sl;
	if (commit) {
		mutex_lock(&ctrl->sched.m_reconf);
...
int slim_ctrl_clk_pause(struct slim_controller *ctrl, bool wakeup, u8 restart)
{
	...
	for (i = 0; i < ctrl->last_tid; i++) {
		if (ctrl->txnt[i]) {
			ret = -EBUSY;
			PR_INFO(LOGTAG"slim_clk_pause: txn-rsp for %d pending\n", i);
			mutex_unlock(&ctrl->m_ctrl);
			return -EBUSY;
		}
	}
	...
	if (ctrl->sched.usedslots) {
		PR_INFO(LOGTAG"slim_clk_pause: data channel active\n");
		ret = -EBUSY;
		goto clk_pause_ret;
	}
