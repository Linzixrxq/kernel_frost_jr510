...
#define QPNP_PON_GP2_RESET_S1_TIMER(pon)	((pon)->base + 0x50)
...
struct jlq_pon_reason {
	int id;
	char *str;
};

struct jlq_pon_reason powerup_reasons[] = {
	{0, "reboot"},
	{1, "reboot"},
	{0xa, "kpanic"},
};
...
{
	unsigned int pu_reason = 0;
	//int rc, index;
	int rc, index, i;

	rc = qpnp_pon_read(g_pon, QPNP_POFF_REASON1(g_pon), &pu_reason);
	pr_err("[yxw test0305] pu_reason = 0x%04x\n", pu_reason);
	index = ffs(pu_reason) - 1;
	pr_err("[yxw test0305] index = 0x%04x\n", index);
	if (index == 7)
		return sprintf(buf, "%s\n", "long_power_key");
	rc = qpnp_pon_read(g_pon, QPNP_PON_REASON1(g_pon), &pu_reason);
        pr_err("[yxw test0311] pu_reason = 0x%04x\n", pu_reason);
	index = ffs(pu_reason) - 1;
	//return sprintf(buf, "%s\n", qpnp_pon_reason[index]);
        pr_err("[yxw test0311] index = 0x%04x\n", index);
	//	if (index == 7 || index == 2)
	//	return sprintf(buf, "%s\n", qpnp_pon_reason[index]);
        if (index == 7)
		return sprintf(buf, "%s\n", "keypad");
	...
	rc = qpnp_pon_read(g_pon, QPNP_PON_GP2_RESET_S1_TIMER(g_pon), &pu_reason);
	for(i=0; i < sizeof(powerup_reasons); i++) {
		if (pu_reason == powerup_reasons[i].id) {
			index = i;
			break;
		}
	}
	return sprintf(buf, "%s\n", powerup_reasons[index].str);
}
...
static int qpnp_pon_read_hardware_info(struct qpnp_pon *pon, bool sys_reset)
{
	...
	bool cold_boot;
	u16 poff_sts = 0;
	unsigned int off_sts = 0;
	unsigned int fault_sts[2] = {0};
	int rc, index;
	...
	if (index >= ARRAY_SIZE(qpnp_pon_reason) || index < 0) {
		...
		dev_info(dev, "PMIC@SID%d Power-on reason: %s and '%s' boot, pon_sts=0x%x\n",
			 to_spmi_device(dev->parent)->usid,
			 qpnp_pon_reason[index],
	...
	if (index >= ARRAY_SIZE(qpnp_poff_reason) || index < 0 ||
		...
		dev_info(dev, "PMIC@SID%d: Power-off reason: %s, poff_sts=0x%x\n",
			 to_spmi_device(dev->parent)->usid,
	...
	/* OFF reason */
	rc = qpnp_pon_read(pon, QPNP_PON_OFF_REASON(pon), &off_sts);
	if (rc)
		return rc;

	/* Fault reason */
	rc = regmap_bulk_read(pon->regmap, QPNP_FAULT_REASON1(pon), &fault_sts[0], 2);
	if (rc)
		return rc;

	dev_info(dev, "PMIC@SID%d: off_sts=0x%x, fault_sts[0]=0x%x, fault_sts[1]=0x%x\n",
		to_spmi_device(dev->parent)->usid, off_sts, fault_sts[0], fault_sts[1]);
	...
	if (to_spmi_device(dev->parent)->usid == 0) {
		...
		rc = sysfs_create_group(bootinfo_kobj, &attr_group);
		if (rc)
