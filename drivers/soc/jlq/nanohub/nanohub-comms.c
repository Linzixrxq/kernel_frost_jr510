		    ...
		    int timeout)
{
	...
	const int max_size = sizeof(struct nanohub_packet) + MAX_UINT8 +
	    sizeof(struct nanohub_packet_crc);
	unsigned long end = msecs_to_jiffies(READ_ACK_TIMEOUT_MS);

	spin_lock_irqsave(&data->notify_wait.lock, data->notify_flags);
	end = wait_event_interruptible_timeout_locked(
			data->notify_wait,
			 nanohub_notify_fired(data),
			 end, data->notify_flags);
	spin_unlock_irqrestore(&data->notify_wait.lock, data->notify_flags);
	if (end <= 0) {
		ret = ERROR_NACK;
		pr_info("nanohub: cnt:%d %s: timeout\n",
			atomic_read(&data->notify_cnt), __func__);
	} else {
		ret = bridge_read(data->ch, (uint8_t *)response, max_size);
		if (ret == 0) {
			pr_info("nanohub: cnt:%d %s: empty packet\n",
				atomic_read(&data->notify_cnt), __func__);
			ret = ERROR_NACK;
		} else if (ret < sizeof(struct nanohub_packet)) {
			pr_info("nanohub: %s: too small\n", __func__);
			ret = ERROR_NACK;
		} else if (ret <
			   sizeof(struct nanohub_packet) + response->len +
			   sizeof(struct nanohub_packet_crc)) {
			pr_info("nanohub: %s: too small length\n", __func__);
			ret = ERROR_NACK;
		} else {
			atomic_dec(&data->notify_cnt);
			if (atomic_read(&data->notify_cnt))
				pr_debug("nanohub:%s: cnt:%d len[%d %lu]\n",
				      __func__,
				      atomic_read(&data->notify_cnt), ret,
				      NANOHUB_PACKET_SIZE(response->len));
		}
	}

	if (ret < 0) {
		pr_info("nanohub: %s: dbg cnt:%d MBOX INTR:0x%x REG[0x%x 0x%x 0x%x]\n",
		...
		if ((atomic_read(&data->notify_cnt) == 0) &&
			(readl(data->pdata->top_mbox_base + MBOX_CM42AP_NINTR_STA) & 0x1))
			writel(0x1, data->pdata->top_mbox_base + MBOX_CM42AP_NINTR_STA);
	}
		    ...
		    int ret_ack)
{
	...
	const int max_size = sizeof(struct nanohub_packet) + MAX_UINT8 +
	    sizeof(struct nanohub_packet_crc);
	unsigned long end = msecs_to_jiffies(READ_MSG_TIMEOUT_MS);
	struct nanohub_packet *msg_resp;

	ret = ret_ack - NANOHUB_PACKET_SIZE(response->len);
	if (ret > 0) {
		msg_resp = (struct nanohub_packet *)((uint8_t *)response +
			    NANOHUB_PACKET_SIZE(response->len));
		memcpy(response, (uint8_t *)msg_resp, ret);
	} else {
		spin_lock_irqsave(&data->notify_wait.lock, data->notify_flags);
		end = wait_event_interruptible_timeout_locked(
				data->notify_wait,
		...
		if (end <= 0)
			pr_info("nanohub: cnt:%d %s: empty packet\n",
				atomic_read(&data->notify_cnt), __func__);
		spin_unlock_irqrestore(&data->notify_wait.lock,
				       data->notify_flags);
		ret = bridge_read(data->ch, (uint8_t *)response, max_size);
	}
	if (ret == 0) {
		pr_info("nanohub: %s: empty packet\n", __func__);
		ret = ERROR_NACK;
	} else if (ret < sizeof(struct nanohub_packet)) {
		pr_info("nanohub: %s: too small\n", __func__);
		ret = ERROR_NACK;
		...
		ret = ERROR_NACK;
		   ...
		   sizeof(struct nanohub_packet) + response->len +
		   sizeof(struct nanohub_packet_crc)) {
		pr_info("nanohub: %s: wrong length\n", __func__);
		ret = ERROR_NACK;
	} else
		pr_debug("nanohub:%s: cnt:%d len[%d %lu]\n",
			      __func__,
		     ...
		     uint32_t seq)
{
	...
	ret = read_ack(data, response, 272);
		...
		if (response->reason == CMD_COMMS_ACK) {
			if (response->len == sizeof(data->interrupts))
				memcpy(data->interrupts, response->data,
				       response->len);
			ret = read_msg(data, response, ret);
			if (ret < 0)
				ret = ERROR_NACK;
		} else {
			int i;
			       ...
			       uint32_t seq, uint8_t *rx, size_t rx_len)
{
	...
	ret = bridge_write(data->ch, (const char *)&pad->packet, packet_size);
	if (ret == packet_size) {
