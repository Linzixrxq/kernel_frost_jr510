...
struct dmas_ch_cfg {
	...
	/*
	 * For rx channel only. Timeout counter base on 1us, when timeout,
	 * DMAS will do flush; if this value==0 that means disable
	 * timeout. MAX=0xFFF.
	 */
	int rx_timeout;
...
extern int jlq_dmas_request(const char *name, unsigned int ch);
extern int jlq_dmas_free(unsigned int ch);
extern int jlq_dmas_config(unsigned int ch, struct dmas_ch_cfg *cfg);
extern int jlq_dmas_get(unsigned int ch, unsigned int *addr);
extern int jlq_dmas_state(unsigned int ch);
extern int jlq_dmas_start(unsigned int ch);
extern int jlq_dmas_stop(unsigned int ch);
extern int jlq_dmas_intr_enable(unsigned int ch, unsigned int intr);
extern int jlq_dmas_intr_disable(unsigned int ch, unsigned int intr);
extern void jlq_dmas_dump(unsigned int ch);

enum of_dmas_flags {
	OF_DMAS_FLAG_RESERVED = 0x1,
};

extern int of_get_named_dmas_flags(struct device_node *np, const char *propname,
			   int index, enum of_dmas_flags *flags);

static inline int of_get_named_dmas_channel(struct device_node *np,
					    const char *propname, int index)
{
	return of_get_named_dmas_flags(np, propname, index, NULL);
}
